/**
 * The Verifier catch-rate ground truth, in one place.
 *
 * The published verifier benchmark asks how often it catches a fatal defect in
 * wrong-but-faithfully-sourced material. Every number answering it — local qwen
 * ~1-in-3 (n=3), `gemini-3.5-flash-lite` 2.33/4 (n=3), the deep tier at n=0
 * until the rerun — is measured against THIS section and THESE four probes.
 *
 * It used to exist twice. `verifier-tier-test.mjs` ran its Ollama loop on import
 * and exported nothing, so `gemini-benchmark.mjs` carried a second copy, and
 * `GEMINI_BENCHMARK_2026-08-20.md` §9.4 flagged it: two copies of a ground truth
 * drift, and a drifted ground truth reports a catch rate against a section
 * nobody has read. The measurements above are only comparable to each other
 * while the fixture is one object.
 *
 * So: data and pure functions only. Nothing here calls a model, opens a socket,
 * reads a file or runs a loop at import time, because a module that does work on
 * import is exactly what forced the copy the first time.
 *
 * ---
 *
 * The section below SHIPPED in REFERENCE_SESSION v1 with four independently
 * confirmed fatal defects. It is faithfully sourced — every claim traces to the
 * material — and wrong anyway, which is the hard case: a Verifier that only
 * catches unsourced invention manufactures confidence on everything else.
 */
