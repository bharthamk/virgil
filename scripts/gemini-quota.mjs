 ...
 * landed with the ADK merge and does exactly what the quota-degradation contract
 ...
 * nothing here was counting. The usage-accounting contract says to count: **three identical
 * consecutive failures and the run stops issuing requests.**
 *
 * The usage-accounting contract also settles the accounting those seventeen requests raised. A
...
/**
 * Identical consecutive failures before the run gives up. The usage-accounting contract.
 *
 ...
 *
 * Status and kind, which is what the ruling names, and deliberately **not** the
 ...
 *
 * The usage-accounting contract: *transport-level 503s that arrive WITHOUT a provider exhaustion
...
export function makeLedger({ dayCap = 20, reserve = 3, breaker = BREAKER_THRESHOLD } = {}) {
  return {
    ...
    fastSpent: 0,
    /**
     * Requests presumed to have been counted by the provider (the usage-accounting contract).
     *
     ...
     * one case the ruling says the day did not pay for.
     */
    deepBilled: 0,
    fastBilled: 0,
    /** The attempts the ruling says never reached the model. The difference, itemised. */
    unbilled: [],
    deepExhausted: false,
    fastExhausted: false,
    ...
    stoppedAt: null,
    /** Identical consecutive failures the breaker gives the provider. The usage-accounting contract. */
    breaker,
 ...
 * documents why the adapter does not retry — D18 keeps nightly cost and latency
...
export function guard(inner, ledger, opts = {}) {
  ...
  const call = async (method, req, isRetry = false) => {
    ...
    try {
      ...
      // The usage-accounting contract's accounting, applied to this one request.
      ...
      //
      //    A wait the harness already handles does not count toward
