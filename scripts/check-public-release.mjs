...
const privateWorkspacePatterns = [
  /\/Users\/mb(?:\/|$)/g,
  /\bRUI workspace\b/gi,
  /(?:^|[\/"'`])Core\/Agents\//gm,
  /_seed\/breadcrumbs\.md/g,
  /\bTEN\/CX\b/g,
  /(?:^|[\/"'`])Projects\/virgil\/artifacts\/submission\//gm,
];
for (const file of candidates) {
  ...
  if (file !== 'scripts/check-public-release.mjs') {
    ...
    for (const pattern of privateWorkspacePatterns) {
      if (pattern.test(source)) problems.push(`${file}: private workspace marker`);
      pattern.lastIndex = 0;
    }
  }
...
if (existsSync('.git')) {
  ...
  for (const record of records) {
    ...
    if (existsSync('virgil-source-package.json')
        && [authorEmail, committerEmail].some((value) => /@(?:gmail|googlemail)\.com$/i.test(value))) {
      problems.push(`${hash.trim()}: personal email appears in public snapshot metadata`);
    }
