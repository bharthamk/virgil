...
for (let i = 2; i < process.argv.length; i += 1) {
  ...
  if (token === '--allow-dirty') { flags.add('allow-dirty'); continue; }
...
const requestedChromeClientId = args.get('google-extension-client-id')?.trim() || null;
if (!serviceValue || !outValue) {
  throw new Error('Usage: npm run package:extension -- --service https://YOUR-SERVICE --out PATH [--google-extension-client-id ID] [--source-ref REF | --allow-dirty]');
}

const sourceRef = args.get('source-ref')?.trim() || null;
...
if (dirtyLines.length && !allowDirty) {
  throw new Error('Refusing release packaging from a dirty tree; use a clean worktree/ref or --allow-dirty for an explicitly non-release package');
}
if (sourceRef) {
...
if (requestedChromeClientId && !/^[A-Za-z0-9._-]+\.apps\.googleusercontent\.com$/.test(requestedChromeClientId)) {
...
// Rebuilding a finished watched package must not silently turn it back into a
// preparation package. The OAuth client is public manifest configuration, and
// belongs to the stable extension identity already proven by this package.
const chromeClientId = requestedChromeClientId || existing?.chromeClientId || null;
...
try {
  ...
  const files = packageFiles(stage).filter((file) => file !== 'virgil-package.json');
  const runtimePaths = [
    'manifest.json', 'dist/service.js', 'dist/panel.js', 'session-bridge-content.js',
  ].filter((file) => existsSync(join(stage, file)));
  const metadata = {
    schema: 'virgil-extension-package-v2',
    extensionId,
    ...
    files: files.map((file) => fileReceipt(stage, file)),
...
function existingPackage(path) {
  ...
  try {
    ...
    if (!['virgil-extension-package-v1', 'virgil-extension-package-v2'].includes(metadata.schema) || !key
      || metadata.extensionId !== idFromKey(key)) throw new Error('receipt mismatch');
    const chromeClientId = typeof manifest.oauth2?.client_id === 'string'
      && manifest.oauth2.client_id ? manifest.oauth2.client_id : null;
    if (Boolean(metadata.googleExtensionClientConfigured) !== Boolean(chromeClientId)) {
      throw new Error('OAuth receipt mismatch');
    }
    if (metadata.schema === 'virgil-extension-package-v2') validateInventory(path, metadata);
    return { key, chromeClientId };
  } catch {
...
function packageFiles(root, relativeDirectory = '') {
  const files = [];
  for (const entry of readdirSync(join(root, relativeDirectory), { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) files.push(...packageFiles(root, relativePath));
...
function validateInventory(root, metadata) {
  if (!Array.isArray(metadata.files) || metadata.fileCount !== metadata.files.length
      || !/^[a-f0-9]{40}$/.test(metadata.sourceCommit)
      || !/^[a-f0-9]{40}$/.test(metadata.sourceTree)) throw new Error('bad portable receipt');
  const expected = new Set(['virgil-package.json']);
  for (const entry of metadata.files) {
    if (!entry || typeof entry.path !== 'string' || !/^[a-f0-9]{64}$/.test(entry.sha256)) {
      throw new Error('bad portable receipt entry');
    }
    const target = resolve(root, entry.path);
    if (target === root || !target.startsWith(`${root}${sep}`) || expected.has(entry.path)) {
      throw new Error('unsafe or duplicate portable receipt path');
    }
    const actual = fileReceipt(root, entry.path);
    if (actual.sha256 !== entry.sha256 || actual.bytes !== entry.bytes) throw new Error('package bytes changed');
    expected.add(entry.path);
  }
  const actual = new Set(packageFiles(root));
