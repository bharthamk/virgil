...
try {
  // Probe every corpus up front. A partly warm cache must not make the check
  // start the Nomic half only to fail later when it reaches an uncached pin.
  await nomicVectors([...seedPins, ...syntheticPins, ...scalePins], { allowUnavailable: true });
  nomicAvailable = true;
} catch (err) {
