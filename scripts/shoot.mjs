...
const seed = arg('seed', '');
/**
 * An expression evaluated on the settled surface, printed next to the shot.
 *
 * Added because a screenshot answers "does this look wrong" and never "why".
 * A masthead whose padding had silently fallen back to zero looked, in a PNG,
 * exactly like a masthead with a layout bug in it, and the difference between
 * the two is one `getComputedStyle` call that used to mean writing a fifth
 * copy of the launch-Chrome rig.
 */
