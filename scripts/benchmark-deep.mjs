...
async function preflight() {
  ...
  // 18, not 17. `learner-action` was added after a real five-minute session
  // passed every shape check while offering nothing the learner could answer.
...
function deltas(card, base, basis = null, baseBasis = null) {
  ...
  /**
   * The Composer material-budget contract gave the scorer two budget bases, and a delta taken across them
...
async function comparabilityNotes(threeRegisterBoard) {
  ...
  notes.push({
    ...
    detail: 'The verifier-withholding contract added `closing-note-withheld`. Every check is addressed by name and nothing '
  ...
  notes.push({
    ...
    detail: 'The verifier-withholding contract also changed the product: `stripWithheldTopics` rewrites the note in the verify '
  ...
  /**
   * The Composer material-budget contract split the budget into two bases, and the delta columns straddle
  ...
  notes.push({
    ...
    detail: 'The Composer material-budget contract made a section\'s budget `min(registerBudget, max(150, materialWords × 3.5))` and '
  ...
  notes.push({
    ...
    detail: 'The floor-2026-08-20 and three-register-attempt-1 columns are computed by scoring their '
      ...
      + 'and the comparison is sound. But the sessions themselves were composed against the earlier register-only budget '
  ...
  notes.push({
    what: 'the Composer material-budget contract changes what the Composer is asked for, not which agents are called',
    detail: 'A thin topic now gets a shorter section by construction. That moves session-words, '
      ...
      + 'three-register board has exactly one, which is the case that motivated this constraint. It does '
  ...
  notes.push({
    ...
    detail: 'provider refusal handling, verifier withholding, learn-now, longer toast dwell time, and the '
   ...
   *
   * The prose above quotes 0.12 vs 0.44 from the fixlet lane's thin board. This
...
const llm = guard(baseLlm, ledger, {
  onEvent: (e) => {
    ...
    if (e.kind === 'tripped') {
      ...
      say(`     on the ${e.tier} tier. The run stops issuing requests, by every tier, now (the usage-accounting contract).`);
    }
...
for (const s of plan.stages) {
  ...
  if (ledger.tripped) {
    ...
    actual[s.id] = {
      ...
      note: `${ledger.tripped.count} identical ${ledger.tripped.signature} failure(s) in a row`
        + ` at "${ledger.tripped.stage}" — the run stopped issuing requests (the usage-accounting contract)`,
    };
    say('');
    continue;
  }
  ...
  if (s.kind === 'catch-rate') {
    ...
    //
    // The usage-accounting contract amends it: only budget an earlier stage genuinely did not need
    ...
    if (freed > 0) {
      ...
      say(`      fewer. That budget is not absorbed here (the usage-accounting contract) and stays unspent.`);
    }
...
if (ledger.tripped) {
  ...
  say(`  request(s) were attempted and ${ledger.deepBilled} are presumed billed (the usage-accounting contract).`);
}
say('');
...
function renderMarkdown(r) {
  ...
  o.push(`**Scope:** Gemini use in this lane is limited to benchmarking.`);
  ...
  for (const [id, s] of Object.entries(r.scored)) {
    ...
    o.push('');
    o.push(`Budget basis: \`${s.basis}\` (the Composer material-budget contract).`);
