#!/usr/bin/env bash
#
# The names deployment needs, in one place.
#
...
# project.
#
# **Nothing in this directory creates anything without VIRGIL_DEPLOY=yes.**
# Plans are safe by default; cloud mutations require an explicit operator opt-in.
...
# The secrets. Both are created by whoever holds them, on deployment.
...
# The runtime identity. Needs, on deployment:
...
This script creates or bills Google Cloud resources. Re-run with an explicit
