...
# SB_LLM=local is named, not inherited: unset routes to Gemini because Cloud
# is the product default, and this suite found that out by dying
# on LlmCredentialMissing inside a container pointed at an Ollama-shaped stub.
# The opt-in is named for the same reason: the stub answers on a docker network
# hostname, which is a remote endpoint to the container, and the endpoint guard
# (added after this suite was written) otherwise degrades every model stage to
# `no-session:model-failed` with zero calls made. The network is `--internal`,
# so the opt-in still cannot reach off this machine.
...
# An empty board is the honest empty night: the three-state batch-result and verifier-withholding contracts say this is a run
...
# A temporary shared secret for the exposed-service authentication check. It exists for the length of this
