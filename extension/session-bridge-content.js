...
void (() => {
  ...
  /** Chrome leaves a content script's DOM listeners behind when an unpacked
   * extension is reloaded, but invalidates every extension API behind them.
   * A later hosted-session publication must retire that stale listener before
   * it can touch storage; the refreshed extension/page will install a current
   * bridge in its own context. */
  const contextAlive = () => {
    try { return typeof chrome.runtime?.id === 'string' && chrome.runtime.id !== ''; }
    catch { return false; }
  };
  ...
  function onMessage(event) {
    // A DOM listener has no consumer for a rejected promise. Fail closed on a
    // bridge/storage error, and retire this copy when the context disappeared
    // between the check above and an awaited Chrome call.
    void receive(event).catch((error) => {
      const message = error instanceof Error ? error.message : String(error);
      if (message.includes('Extension context invalidated')) stop();
    });
  }
