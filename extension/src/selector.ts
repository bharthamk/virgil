/**
 * THE SELECTOR — pick what to pin, when nothing is highlighted (`mode-select`).
 *
 * With no selection this product used to swallow the whole page, and the first
 * real pin anybody made showed exactly what that costs: the material was an AI
 * notice, a "Show Transcript" button and a task list, and the take written
 * from it was as thin as its input. Whole-page capture is still available and
 * is now something a learner chooses (`mode-page`); this is the mode for the
 * far more common case, which is that the thing worth keeping is one or two
 * blocks on a page full of furniture.
 *
 * ## It sets the selection and lets capture do the work
 *
 * The obvious build is a second capture path that reads text out of the picked
 * elements. That would be a second answer to "what is the heading path", "what
 * is the surrounding text", "what are the parts", and the two would drift.
 *
 * So the picker does not extract anything. For each thing picked it puts the
 * page's own selection around that element and calls the shipped `capture`,
 * which is then doing precisely what it does for a hand-made highlight. A pin
 * made here and a pin made by dragging across the same paragraph are the same
 * pin, by construction rather than by inspection.
 *
 * ## Multi-pick is the point
 *
 * A page usually has more than one thing worth keeping and making somebody
 * repeat the whole gesture per paragraph is how a product teaches people not
 * to bother. Each pick becomes its own pin, because two paragraphs about two
 * things are two things to learn, and the clusterer is what decides whether
