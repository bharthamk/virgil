 ...
 *  the whole instruction ( the interface-affordance contract), and where there is nothing to offer
...
export interface Section {
  ...
  /** The learner-lineup contract: why the ranker chose this, in its own words. */
  why?: string | null;
  ...
  summary?: string | null;
  /** The interface-affordance contract: what this lesson is part of, and what it moves forward. Both
...
export function lessonSurfaces(
  ...
  // , and wired before the done short-circuit on purpose: checking where a
  ...
  // skipping shortens the session rather than backfilling. Backfilling
  ...
  /**
   * , collapsed to the nuance one button can promise.
   *
...
function answerControl(
  ...
  q.querySelector('[data-answer]')!.addEventListener('click', async () => {
    ...
    if (!passed) {
      ...
      //. Offered only where the agent can actually have been wrong about
...
/**
 *  — Virgil's teaching can be challenged at the claim, not only at the
...
/**
 *  — where every claim in this lesson came from, in one tap that writes
