...
/**
 * Service worker: the capture path.
 *
 * SB-08 — full capture capability with the panel closed. The panel is for
 * consumption, not capture, so nothing here opens or requires it.
 * SB-47 — capture must never depend on the network. Pins queue locally and
 * sync later; the toast degrades to the raw selection rather than failing.
 *
 * What is left in this file is the shell: chrome listeners, injection, fetch.
 * The decisions it used to make inline now live in `prefs.ts`, `pin-body.ts`
 * and `queue.ts` where they can be tested without a browser.
 */
...
chrome.runtime.onInstalled.addListener(() => {
  ...
  // click goes to capture, because SB-03 calls capture the single most
  // important interaction in the product and SB-08 says the panel is for
...
// The toolbar-popup ownership contract put a pin on this click, and it is off it again: see
...
/** SB-11's refusal, said the same way from every path that can hit it. */
...
async function finishSelection(tabId: number, chosen: readonly unknown[]): Promise<void> {
  ...
  for (const envelope of chosen) {
    ...
    // Capture ends at a receipt even when there is exactly one passage. A pin
    // id makes Virgil capable of writing a lesson; it does not authorise the
    // paid call or choose it over Gemini, Notebook, or leaving the pin alone.
    // The browser-session shortcut below is the durable route to those choices.
    landed += 1;
    first ??= e;
  }
  ...
  await announceSelection({ kind: SELECT_STATUS, tabId, state: 'saved', count: landed, queued });
  if (newestSessionPin) {
...
/**
 * Pull the service's prefs into the local cache (SB-40/41, SB-16).
 *
...
async function pin(
  ...
  // The deliberate-capture precedence: a pin is a deliberate gesture and outranks both the exclusion
  ...
  // SB-11: the one capture we refuse rather than store. Said before the
  ...
  // SB-09: the bytes, now, while the page the learner is looking at is
  ...
  if (announce) {
    await chrome.scripting.executeScript({
      target: { tabId: tab.id },
      // SB-59: the confirmation grows one affordance, and only when there is
...
/**
 * Post one pin. Says whether the service took it *and* what Scout called it —
 * two different questions that used to be answered by one nullable string. The
 * drain read a label-less success as a failure and would have sent it again on
 * every alarm from then on.
 */
...
/**
 * The re-read detector's only route in and out (SB-15/16).
 *
...
chrome.runtime.onMessage.addListener((message, _sender, respond) => {
  ...
  /**
   * SB-59 — the tap on the toast, on its way to the panel.
   *
   ...
   *
   * No pause check, deliberately: the manual-capture pause exemption. A pause stops what is watched, and
 ...
 *
 * SB-16 is the rejection count; SB-40/41 are pause and exclusions. All three now
...
/** A candidate, never a pin. The learner confirms it in the panel (SB-15). */
...
// Both branches name their alarm. The drain used to be the `else` — every alarm
// that was not the prefs one — which is exact with two alarms and silently
// wrong the moment there is a third: whatever it was for, it would also post
// the queue. An alarm this worker did not create is nothing to do here.
