...
export const EXTERNAL_ADD = 'Add outside learning';
 ...
 */
function addControl(
  deps: ExternalFaceDeps, added: (entry: ExternalEntryView) => void,
): HTMLElement {
  ...
  const body = deps.el(`<div class="external-add-body"></div>`);
  ...
  open.addEventListener('click', () => {
    ...
    const form = deps.el(`<div class="external-add-form">
      <label class="field"><span class="what-label"></span>
        <input class="what" type="text">
        <span class="meta input-limit"></span></label>
      <label class="field"><span class="where-label"></span>
        <input class="where" type="text"></label>
      <div class="row"><button data-external-save></button></div>
    ...
    const where = form.querySelector('.where') as HTMLInputElement;
    ...
    save.addEventListener('click', async () => {
      ...
      save.disabled = true;
      const entry = await deps.add(label, where.value.trim());
      save.disabled = false;
...
// ------------------------------------------- the board's own send (slice 2)

/** Board cards expose separate Learn and Send controls; send copy lives here. */
export const BOARD_LEARN = 'Learn';
export const BOARD_RUN_THEN_LEARN = 'Run then learn';
export const BOARD_SEND = 'Send';
export const BOARD_SEND_TITLE = 'Takes this to Gemini. You pick where it goes.';
export const BOARD_SEND_HEADING = 'Take this to Gemini';

/** Which of the two payloads the popup is holding, said in one line so nobody
 *  has to press a destination to find out what it would carry. */
export const SEND_CARRIES_LESSON = 'Carrying the lesson I have lined up on this.';
export const SEND_CARRIES_SAVED = 'Carrying what you saved on this.';
export const SEND_CARRIES_NOTHING =
  'There is nothing saved on this one yet, so there is nothing to carry.';

/**
 * WHY A SUBJECT WITH NO LESSON IS OFFERED THREE DOORS AND NOT FOUR.
 *
 * The learn now document is about the lesson in front of the learner. It is
 * written from the current session and nothing else, so a push from a card the
 * session is not teaching would rewrite a document about a different subject
 * and then open it, which is the one failure a hand-off cannot see from the
 * inside. Truthfulness beats symmetry, so the fourth destination is absent and
 * the popup says why rather than leaving a gap somebody has to explain.
 */
export const SEND_NO_NOTEBOOK =
