 ...
 *
 *  is why this is fat: re-fetch fails on gated pages, so what we grab here
 ...
 * the Analyst is told to reason about a *delta* from. When there is a
...
export interface CapturedEnvelope {
  ...
  /** . Null on every page with nothing playing, which is most of them. */
  ...
  /** which page of the PDF, where the viewer says. Usually it does not. */
  pdfPage: number | null;
}

/**
 * reviewer-boundary constraint, the wider form of it: Chrome serialises an injected function across the
...
export function capture(
  ...
  /** Whole-page pins () are legitimate and common — we must not push the
  ...
  /**
   *  — the moment, if the learner was watching something.
   *
  ...
  /**
   *  — is this a PDF, and which page of it?
   *
   ...
   * binding would be `undefined` on the other side (reviewer-boundary constraint). `selection-memory.
  ...
  return {
    ...
    // capture-envelope constraint: Forager re-fetches this page hours later and must be able to tell it
    // got the same language back. Load-bearing, not decoration.
    contentLanguage: document.documentElement.lang || null,
    // captured now because by the nightly the tab is closed and the
    // playhead is nowhere.
