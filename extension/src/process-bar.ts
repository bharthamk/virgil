...
export async function processControl(
  ...
  const paint = (next: BatchState): void => {
    ...
    /**
     * A built lesson nobody has begun owns the strip.
     *
     * The rule is the one written a few lines down for the run receipt — a
     * checked lesson must not compete with a fresh seven-call build before it
     * has been opened — and it used to hang off that receipt, which is gone by
     * the next visit. So the lesson stayed ready and the strip went back to
     * offering the build over the top of it. It hangs off the lesson itself
     * now, and holds for as long as the lesson is untouched.
     */
