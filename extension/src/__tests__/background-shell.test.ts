...
test('two rejections on a site quiet it, and leave its neighbours alone (SB-16)', async (t) => {
  const c = await wake(t);
...
test('a candidate is posted as a suggestion, never as a pin (SB-15)', async (t) => {
  const c = await wake(t, {}, (stub) => {
...
// ----------------------------------------------- the shared secret (the exposed-service authentication boundary)

test('every request the worker makes carries the secret, when the learner has set one', async (t) => {
  /**
   * The exposed-service authentication boundary's extension half. The deployed service is private and answers 401
...
test('an alarm nobody named does nothing at all', async (t) => {
  // The handler used to treat every alarm that was not `sb-prefs` as the drain.
  // With two alarms registered that is exact, and it stops being exact the
  // moment somebody adds a third — which would then post the offline queue as a
  // side effect of whatever it was actually for. Both branches name their
  // alarm; an alarm this worker did not create is a no-op.
  const c = await wake(t, { store: { sb_pending_pins: [{ type: 'interest' }] } }, (stub) => {
...
/**
 * SB-59, the half that had never been run: the tap opening the panel.
 *
