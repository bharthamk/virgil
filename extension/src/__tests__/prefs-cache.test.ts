...
/**
 * The synced cache, and the direction it fails in (SB-40/41, SB-16).
 *
...
test('the rejection counts ride the same cache, and two of them silence the site', () => {
  ...
  assert.equal(mayObserve(twice, SITE, NOW), true, 'SB-40/41 have no objection');
  assert.equal(detectorMayObserve(twice, SITE, NOW), false, 'and SB-16 does');
