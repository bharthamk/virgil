...
/**
 * capture must never depend on the network. Everything the offline queue
...
test('a pin captured while a drain is in flight is not thrown away', async () => {
  ...
  // silently.  says the network must never be able to cost a capture.
...
test('an envelope survives the queue with every field intact', async () => {
  ...
  // contentLanguage above all (capture-envelope constraint).
  const body = {
    type: 'struggle',
    envelope: {
