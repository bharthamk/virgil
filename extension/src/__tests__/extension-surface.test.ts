...
test('the hosts the extension may reach are its own service and its sign-in, and nothing else', () => {
   ...
   *
   * It was one entry until the learner-identity contract gave learners accounts. The only remote
...
test('every request to the service goes through the one place that carries identity', () => {
  /**
   * **The service-protection contract, as a structural rule rather than as three call sites that
...
test('the extension is not available in incognito at all', () => {
  //  third demand: "incognito is excluded by default, always". Chrome's
  ...
  assert.equal(manifest.incognito, 'not_allowed',
    'the manifest does not exclude incognito, so a learner could enable it there and the product contract\'s promise is false');
});
