...
test('default guide copy is a natural sentence for the named target', async (t) => {
  const dom = installGuideDom(t);
  dom.app.attrs.set('data-room', 'privacy');
  dom.target.attrs.set('data-guide-target', 'customize-settings');
  await guideVirgilView({
    surface: 'customize', target: 'customize-settings', refresh: true,
  });
  assert.equal(dom.body.querySelector('.guide-message')?.textContent,
    'These are Virgil’s settings.');
  assert.doesNotMatch(dom.body.querySelector('.guide-message')?.textContent ?? '', /This is the Customize/);
});
