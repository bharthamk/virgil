 ...
 * what `acrossExecuteScriptBoundary` reproduces (reviewer-boundary constraint).
 */
...
export interface NodeInit {
   ...
   * `clientWidth`. A `<video>` element's state is not in its markup, and
...
export interface PageInit {
  ...
  hostname?: string;
  lang?: string;
  /** `document.contentType`. `application/pdf` for a tab showing a PDF (). */
  contentType?: string;
...
export function installPage(page: PageInit): () => void {
  ...
  // it says anything at all , so it is split off the url the way the
 ...
 * module scope — only globals. reviewer-boundary constraint is the toast half of this lesson; the same
