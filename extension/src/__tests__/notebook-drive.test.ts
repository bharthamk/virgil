...
test('hosted Drive creates three native Docs with one account choice, then rewrites them', async () => {
  ...
  try {
    ...
    assert.equal(tokenRequests, 1, 'one page session asked the learner to choose an account twice');
    ...
    assert.ok(requests.every((request) => request.authorization === 'Bearer short-lived-access-token'));
