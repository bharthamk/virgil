...
import { disposeWebMcp, virgilTools } from '../webmcp.js';
import {
...
test('the exported registry carries all five bounded tools before a page host is present', async () => {
  disposeWebMcp();
  const tools = virgilTools();
  assert.equal(tools.length, 5);
  assert.equal(new Set(tools.map((tool) => tool.name)).size, 5);
  const refused = await tools.find((tool) => tool.name === TOOL_DRAFT_INTAKE)!.execute(null);
  assert.match(refused, /Nothing was sent/);
});
