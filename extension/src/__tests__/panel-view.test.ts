...
test('an honest empty state, rather than a session manufactured to look busy ()', () => {
...
test('source availability says what the learner confirmed, not that the words were verified', () => {
...
test('a pinned source says it was the learner\'s own page, and when they saved it', () => {
  // the product contract's own words: "From the page you pinned on 14 Aug".
  assert.equal(
...
test('a pinned video says which moment of it the learner kept', () => {
  // the product contract's step 3 — "the session later opens at that moment" — needs the
...
test('the moment reads as a person would say it, at any length', () => {
...
test('a moment it cannot read is a clause it does not write', () => {
...
test('a pinned PDF says which page of it, so a 90-page paper is followable', () => {
  assert.equal(
    sourceLine({
...
test('a page number it cannot read is a clause it does not write', () => {
...
test('an agent-sourced reference is marked as not the learner\'s', () => {
...
test('a date it cannot read is a clause it does not write', () => {
  assert.equal(
...
test('a source whose origin is neither is not described at all', () => {
...
test('only an http(s) link is offered as a link', () => {
...
test('a reference that could not be resolved is said out loud, not dropped in silence', () => {
...
test('the concession is offered where the agent can be wrong, and nowhere else ()', () => {
...
/**
 * The controls the product contract assumed and the panel never had.
 *
...
test('the timed pauses are bounded, and one choice is not timed at all (the processing-pause contract)', () => {
...
test('the paused banner says what stopped and what did not, in one line each', () => {
  // The processing-pause contract’s mitigation, and the learner-action contract’s exemption, on the one surface that is
...
test('the off-limits screen admits that pinning by hand still works there', () => {
  // The learner-confirmation contract: manual capture outranks the list. A privacy screen that let
...
test('§5: the card heading names the session, and every other state names itself', () => {
  ...
  // The manual-processing contract: no run is coming on its own, so a heading telling somebody to
...
test('§5: a flagged row says who flagged it and when, in a person\'s words', () => {
  ...
  //. In the learner's own voice like the two above it, because that is
...
test('course search includes the planned work already projected into that course', () => {
...
test('an all-search miss names its complete scope without claiming an empty board', () => {
  ...
  // The headings name the two places and count nothing in them.
...
test('the lesson offers the one nuance a single button can promise', () => {
 ...
 * quietly broken.
 *
 * The reviewer boundary: this reviews the learner's own work and never
...
test('the screen says what it will not do, before it is asked', () => {
  assert.equal(
    reviewFramingLine(),
...
test('a draft too short to judge is refused before the request, not after', () => {
...
test('each of the four answers is a different sentence', () => {
...
test('an outcome this build does not know says the failure, not the success', () => {
...
test('a finding that touches the board says which topic, in the learner’s own label', () => {
...
test('the labels on the Check screen are names, and the voice is in the line below', () => {
  ...
  // the learner has to work out they have already arrived at.  shortened
...
test('a scanned rubric is told why pixels will not do, and offered the way round it', () => {
  /**
   * The criteria-extraction contract is the reason this box is different from the one above it. The
...
test('repeated document markers repair title and wrapped-line damage', () => {
  const extracted = [
...
test('unstructured imported criteria remain exactly as extracted', () => {
...
test('labelled and bullet lists repair only when their own structure repeats', () => {
...
test('a finding can be kept whether or not the model supplied a board label', () => {
  // The learner-confirmation contract: a suggestion the user confirms, never a silent write. The line
...
test('the take is subordinate by what it shows, not by a caption', () => {
  assert.equal(quickTakeStandingLine(), '');
});

test('three answers close it, and asking is not one of them', () => {
  /**
   * §3 said *"Closes with one tap: Got it / Still shaky. No essay, no rating
   * scale"*, and this asserted two labels.  walkthrough found the
...
test('the receipts say what happens next, not how the learner is doing', () => {
  ...
  // the deferral names the window the service actually applied, rather
...
test('each answer says what happened to the pin, and none is a verdict on them', () => {
...
test('each way a take can fail is said plainly, and only one of them is us', () => {
...
test('the model-label contract: a runaway topic label is cut at render, and only at render', () => {
  // The label is model output over pinned text, rendered in a narrow
  // single-column panel. The model-label contract: cap at the untrusted boundary and at
...
// ---------------------------------- the copy under "what you pinned"

test('a selection is called the learner’s, a whole page is not', () => {
...
test('only a whole-page pin gets the sentence explaining itself', () => {
...
test('the passage folds at a word, and collapses the page’s own whitespace', () => {
...
test('the model-label contract still holds, and now cuts at a word: a runaway label is cut at render', () => {
...
test('what is next is stated as a fact, and never as a count of what is left', () => {
  ...
  //  and : no backlog, no number of its own, nothing to clear. The
 ...
 * - **Unsized calls stay outside every total.** The quota-accounting contract presumes them billed
...
test('a cell carries the thing itself and never a number', () => {
   ...
   * The moment it draws "4" instead,  has been broken on the one surface
...
test('a lean night is said once, and only on a night that was actually lean', () => {
  /**
  ...
  // The learner-lineup contract’s copy law, on a sentence a learner reads about their own night.
...
test('a refused build names the pause rather than guessing at a status', () => {
  // The processing-pause contract: a pause is a pause and a button is not a way round it, so the
...
test('the two banned dashes are normalised, even when a model wrote them', () => {
  // It is learner-facing copy the moment it is drawn, and the learner-lineup contract’s rule
...
test('only immediately openable material is a learn-now alternative', () => {
  /**
...
// ============================= The interface-affordance contract: show, do not tell
...
test('the model notice says what a model wrote and what to do about it', () => {
  ...
  //  guard bans the vocabulary of a backlog everywhere, and does not
...
// ------------------------------------------- The session-navigation contract: the session room

test('the session rail names lessons and counts nothing', () => {
  // nothing on any screen is a tally the learner can fall behind on, and
...
test('the four Learn faces are named for the material each one shows', () => {
  ...
  // explaining itself, which the interface-affordance contract bans.
