...
/**
 * The capture envelope is the only thing that survives a gated page  and
 * the only record of what language the learner actually read (capture-envelope constraint). It is also
...
test('capture survives the executeScript boundary — no closure, no module scope', () => {
  // reviewer-boundary constraint, generalised. Chrome serialises the function and evaluates the source in
...
test('a whole-page pin is a first-class case, not an error ()', () => {
  const body = node({
    tag: 'body',
...
test('a whole-page pin carries the readable blocks, in document order ()', () => {
  const body = node({
    tag: 'body',
...
test('every part capture emits is a passage, and never a role it cannot know', () => {
  ...
  // delta between. Nothing on a page says which half was the learner's,
...
test('a selection under headings carries the path down to it ()', () => {
...
test('the content language is captured, because the re-fetch depends on it (capture-envelope constraint)', () => {
 ...
 * the only part of the product contract's step 2 that a content script can honestly get.
 *
...
test('a PDF a viewer does let us read is captured like any other selection', () => {
  ...
  // case the product contract describes, on the viewers that permit it.
