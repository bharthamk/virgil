...
test('a candidate is a suggestion and never a pin, and nothing is written anywhere ()', async (t) => {
  const c = await worker(t);
  const target = passage('Session state is held per user.');
...
test('a site the learner has turned down twice is not watched at all ()', async (t) => {
...
test('an off-limits site is not watched, and neither is a paused learner ()', async (t) => {
  for (const prefs of [
