 ...
 * sequence is where a pin is lost, and losing a pin is the one failure SB-47
...
test('the struggle key is a different pin and a different sentence (SB-13)', async (t) => {
  const c = await wake(t, {}, serviceIsUp);
...
test('capture happens with the panel shut, and does not open it (SB-08)', async (t) => {
  const c = await wake(t, {}, serviceIsUp);
  await c.fire.command('pin-interest', tab());
  await settle();
  assert.equal(pins(c).length, 1);
...
test('a deliberate pin on an off-limits, paused site still lands (The deliberate-capture precedence)', async (t) => {
...
test('a pin the service cannot take is queued, and the toast promises rather than fails (SB-47)', async (t) => {
  const c = await wake(t, {}, serviceIsDown);
  await c.fire.command('pin-interest', tab());
  await settle();
...
test('a success with no label is neither queued again nor called offline', async (t) => {
  // `postPin` deliberately separates "the service took it" from "here is what
  // Scout called it" — the drain used to read a label-less 200 as a failure and
  // resend forever. The toast read that same 200 as offline, so a learner whose
  // service answered without a label was told their pin would sync later when
  // it already had. Both halves now say the same true thing.
  const c = await wake(t, {}, (stub) => {
...
// ------------------------------------------- SB-59: the escalation on the toast
...
test('SB-59: a pin the service filed is offered a take, on the toast, after capture', async (t) => {
  const c = await wake(t, {}, serviceNamesTheId);
  await c.fire.command('pin-interest', tab());
  await settle();
...
test('SB-59: an offline pin is promised, never offered a take it cannot have', async (t) => {
  // SB-47 keeps capture off the network; the take is a live call and cannot
...
test('the manual-capture pause exemption: the take is offered on a paused, off-limits site like the pin is', async (t) => {
...
test('SB-59: the tap writes the hand-off first and asks for the panel second', async (t) => {
...
test('SB-59: a panel Chrome will not open leaves the take waiting rather than lost', async (t) => {
 ...
 */
test('SB-59: the menu route opens the panel first and fills the pin in after', async (t) => {
  const c = await wake(t, { gestureBound: true }, serviceNamesTheId);
...
test('SB-59: the menu route does not also raise a toast offering the same take', async (t) => {
...
test('SB-59: a pin the service refuses still ends the panel’s wait', async (t) => {
  ...
  assert.equal((c.store[QUEUE_KEY] as unknown[] | undefined)?.length, 1,
    'and the pin itself is not lost: it queues like any other (SB-47)');
});
...
test('Standard is where the struggle signal lives now', async (t) => {
  // The menu item that used to carry it is gone. This is the surface that
  // replaced it, and the signal is an input to the comfort map, so this is
  // the assertion standing between the ruling and a thinner learner model.
  const c = await wake(t, {}, serviceNamesTheId);
  await c.send({
    kind: COMPOSE_SAVE,
...
test('Standard’s pin queues when the service is down, like every other pin (SB-47)', async (t) => {
  const c = await wake(t, {}, serviceIsDown);
  await c.send({
    kind: COMPOSE_SAVE,
    envelope: { selection: 'x', pageTitle: 'A Page' },
...
test('one picker passage ends at its receipt and authorises no lesson', async (t) => {
  const c = await wake(t, {}, serviceNamesTheId);
  await c.send({
    kind: SELECT_SAVE,
  ...
  assert.equal(pendingTake(c.store[HANDOFF_KEY], Date.now()), null,
    'saving one passage wrote the paid quick-take handoff');
  const status = c.runtimeMessages
    .filter((m) => (m as { kind?: string }).kind === SELECT_STATUS).at(-1) as Record<string, unknown>;
  assert.deepEqual(status,
    { kind: SELECT_STATUS, tabId: 7, state: 'saved', count: 1, queued: 0 });
  assert.deepEqual((c.sessionStore[CAPTURE_SESSION_KEY] as CaptureSessionPin[])
    .map((pin) => pin.pinId), ['p-42'], 'the receipt lost its durable session shortcut');
  assert.equal(c.runtimeMessages.some((message) =>
    (message as { kind?: string; pinId?: string }).kind === CAPTURE_SESSION_ADDED
      && (message as { pinId?: string }).pinId === 'p-42'), true,
    'an already-open panel was not told to show the receipt');
});

test('one refused pick costs that pick and not the batch', async (t) => {
  // A PDF frame among four paragraphs. SB-11 refuses the one it cannot read
...
test('a batch the service cannot take queues, every one of them (SB-47)', async (t) => {
  const c = await wake(t, {}, serviceIsDown);
  await c.send({
    kind: SELECT_SAVE,
    envelopes: [
