...
/**
 * The wiring that took the detector from written to running (SB-15), and the
...
test('a site the learner has quieted still starts, and starts silent (SB-16)', async () => {
...
test('a raised candidate is the only thing that leaves the page (SB-15)', async () => {
  const h = harness({ quieted: false });
  await h.run();
...
test('an unreachable worker goes quiet rather than loud', async () => {
  ...
  // has, because a fetch timed out, is the exact failure SB-16 exists to stop.
...
test('the re-read detector is registered as a content script (SB-15)', () => {
