 ...
 * (SB-40/41), and what the toast says about it (SB-03, SB-13, SB-47).
 */
...
test('a deliberate pin on an off-limits site still lands, and the detector still does not watch it (The deliberate-capture precedence)', () => {
  // The ruling, stated where it can be read: the exclusion list says "do not
...
test('a pause silences the detector and not the pin the learner just asked for (SB-40)', () => {
...
test('a page no extension may script is refused here rather than by the injection failing', () => {
  // The toolbar capture contract puts a second capture affordance on the toolbar button, and the
  // toolbar button is present on every page including the ones an extension is
  // not allowed near. Until now the refusal was implicit — `executeScript`
  // rejects, the catch shrugs, and nothing is pinned — which is the right
  // outcome reached by an accident of Chrome's, on a path that also swallows
  // real failures. Named schemes, checked before anything is attempted.
  for (const url of [
    'chrome://settings',
...
test('an ordinary page, and a local file, are the two answers that are not obvious', () => {
  ...
  // a PDF opened from disk (SB-11) is a file url. Scripting one needs "Allow
  ...
  assert.equal(capturePermitted('file:///Users/someone/paper.pdf'), false);
});

// ---------------------------------------------------------------------- toast

test('the toast says which kind of pin it was before the service answers (SB-13)', () => {
...
test('a pin that did not reach the service is a promise rather than an error (SB-47)', () => {
...
test('the posted body carries the envelope through untouched', () => {
  ...
  assert.equal(body.envelope.contentLanguage, 'en', 'D9: the re-fetch has nothing to check against without it');
});
 ...
 * whole-page pin of a thin page, which SB-07 says is legitimate and which the
...
test('an ordinary page is never refused, however thin it is', () => {
  // SB-07: whole-page pins are legitimate and common, and the Forager re-fetches
