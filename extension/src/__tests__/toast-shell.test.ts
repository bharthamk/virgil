 ...
 *
 * SB-03 makes that a real failure and not a cosmetic one: the toast is the
...
class FakeEl {
  ...
  removed = false;
  /** SB-59's affordance is the first thing in this toast a person can press,
   *  so the fake page has to be able to press it. The toast-reachability contract adds the two
...
interface ToastPage {
   ...
   *  The toast-reachability contract's explicit dismiss is a key, so it has to be one of these — and
...
function installToastPage(options: {
  chrome?: boolean; reply?: unknown; reduceMotion?: boolean; throwMessage?: boolean;
} = {}): ToastPage {
  ...
  if (options.chrome === false || options.chrome === undefined) delete g['chrome'];
    ...
    if (options.throwMessage) throw new Error('extension context invalidated');
    sent.push(m);
    reply?.(options.reply);
  } } };
 ...
 *  gets no closure and no module scope either (D3). Run it the way it runs. */
...
test('the exported toast and its serialized browser copy mount the same initial receipt', () => {
  const direct = installToastPage();
  try {
    showToast('Pinned, working it out…');
    assert.equal(direct.documentElement.children[0]?.shadow?.children[0]?.textContent,
      'Pinned, working it out…');
  } finally { direct.undo(); }

  const serialized = installToastPage();
  try {
    inject('Pinned, working it out…');
    assert.equal(serialized.documentElement.children[0]?.shadow?.children[0]?.textContent,
      'Pinned, working it out…');
  } finally { serialized.undo(); }
});

test('the exported toast wires its quote, lesson door and undo receipt without serialization help', () => {
  const page = installToastPage({ chrome: true, reply: { ok: true } });
  try {
    showToast('Pinned, working it out…');
    const bubble = page.documentElement.children[0]!.shadow!.children[0]!;
    const finish = page.win['__sbFinishToast'] as (
      text: string, offer: ToastOffer, saved: { quote: string; wholePage: boolean; pageNote: string },
      undo: ToastUndo,
    ) => void;
    finish('Pinned: gradients',
      { label: 'Learn it now?', pinId: 'p-direct', pinLabel: 'Gradients' },
      { quote: '“The exact saved passage.”', wholePage: true, pageNote: 'Saved from the whole page.' },
      { label: 'Undo', pinId: 'p-direct', ownerUid: 'owner' });
    assert.equal(bubble.children.length, 4);
    bubble.children[2]!.click();
    bubble.children[3]!.click();
    assert.deepEqual(page.sent.map((message) => (message as { kind: string }).kind),
      ['sb-learn-now', PIN_UNDO]);
    assert.equal(bubble.textContent, 'Removed. It is no longer on your board.');
  } finally { page.undo(); }
});

test('the toast host is still fixed and on top after its own `all` reset (SB-03)', () => {
  const page = installToastPage();
  try {
    inject('Pinned, working it out…');
...
test('SB-59: the confirmation grows one affordance, and only when there is one', () => {
...
test('SB-59: no offer means no affordance at all — not a disabled one', () => {
  const page = installToastPage({ chrome: true });
  try {
    inject('Pinned, working it out…');
...
test('SB-112: an online pin offers one exact brief Undo', () => {
  const page = installToastPage({ chrome: true });
  try {
    inject('Pinned, working it out…');
    const bubble = page.documentElement.children[0]!.shadow!.children[0]!;
...
test('SB-112: Undo sends the exact pin and owner and speaks successful removal', () => {
  const page = installToastPage({ chrome: true, reply: { ok: true } });
  try {
    inject('Pinned, working it out…');
    const bubble = page.documentElement.children[0]!.shadow!.children[0]!;
...
test('SB-112: a refused Undo leaves the receipt honest', () => {
...
test('SB-112: an invalidated extension context leaves the Undo receipt honest', () => {
  const page = installToastPage({ chrome: true, throwMessage: true });
  try {
    inject('Pinned, working it out…');
    const bubble = page.documentElement.children[0]!.shadow!.children[0]!;
    finishWithUndo(page, 'Pinned: Firestore indexes', {
    ...
    assert.doesNotThrow(() => bubble.children[0]!.click());
    assert.equal(bubble.textContent, 'I could not remove it. It is still on your board.');
    assert.equal(page.timers.at(-1)?.delay, 4000);
  } finally { page.undo(); }
});

// ------------------------------------- the toast-reachability contract: the window a hand can reach


test('the toast-reachability contract: the toast carrying the affordance dwells long enough to be reachable', () => {
  const page = installToastPage({ chrome: true });
  try {
    inject('Pinned, working it out…');
    finishWith(page, 'Pinned: Firestore indexes', { label: 'Learn it now?', pinId: 'p-1', pinLabel: 'Firestore indexes' });
...
test('the toast-reachability contract: the plain toast times out exactly as it did', () => {
  // Offline, the SB-11 refusal, an unnamed success — every toast with no take
...
test('the toast-reachability contract: the plain toast grows no behaviour either — no listeners, on it or on the page', () => {
...
test('the toast-reachability contract: a pointer resting on the toast holds it open, and leaving restarts the dwell', () => {
...
test('the toast-reachability contract: focus holds it open the same way a pointer does', () => {
  const page = installToastPage({ chrome: true });
  try {
    inject('Pinned, working it out…');
    finishWith(page, 'Pinned: Firestore indexes', { label: 'Learn it now?', pinId: 'p-1', pinLabel: 'Firestore indexes' });
...
test('the toast-reachability contract: escape takes it away now, and takes its listener off the page with it', () => {
...
test('SB-59: the tap tells the worker which pin, and takes the toast away with it', () => {
  const page = installToastPage({ chrome: true });
  try {
    inject('Pinned, working it out…');
    finishWith(page, 'Pinned: Firestore indexes', { label: 'Learn it now?', pinId: 'p-1', pinLabel: 'Firestore indexes' });
 ...
 */
test('SB-59: the tap carries what the toast called it, so the panel has a heading', () => {
  const page = installToastPage({ chrome: true });
  try {
    inject('Pinned, working it out…');
    finishWith(page, 'Pinned: Firestore indexes', { label: 'Learn it now?', pinId: 'p-1', pinLabel: 'Firestore indexes' });
...
test('SB-59: a pin the Scout could not name sends no name, rather than an empty one', () => {
  const page = installToastPage({ chrome: true });
  try {
    inject('Pinned, working it out…');
...
test('the imported toast function mounts the complete saved-material receipt', () => {
  // Most tests deliberately execute a serialised copy because that is Chrome's
  // boundary. This companion check calls the imported function itself so the
  // module export is also proved as a normal callable surface, with the richest
  // receipt shape the worker can hand it.
  const page = installToastPage({ chrome: true, reply: { ok: true } });
  try {
    showToast('Pinned, working it out…');
    const finish = page.win['__sbFinishToast'] as (
      text: string, offer: ToastOffer, saved: Saved, undo: ToastUndo,
    ) => void;
    finish(
      'Pinned: Firestore indexes',
      { label: 'Learn it now?', pinId: 'p-1', pinLabel: 'Firestore indexes' },
      {
        quote: '“the exact passage that was saved”', wholePage: true,
    ...
    const bubble = page.documentElement.children[0]!.shadow!.children[0]!;
    assert.equal(bubble.textContent, 'Pinned: Firestore indexes');
    assert.deepEqual(chosenLines(bubble), [
      '“the exact passage that was saved”',
...
test('a toast that quotes dwells long enough to read it', () => {
  // A plain four-word confirmation needs only 1500ms because there is nothing
  // to press. A
  ...
  // branch carrying a decision, preserving the intended ordering.
  const page = installToastPage();
  try {
    inject('Pinned, working it out…');
    finishQuoting(page, 'Pinned: Firestore indexes',
...
test('a toast with nothing to read still leaves at 1500ms', () => {
