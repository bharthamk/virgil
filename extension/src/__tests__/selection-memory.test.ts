...
test('capture reads the key this module writes, and the same window', () => {
  ...
  // no imports (D3), so it spells both out. Two copies of a rule is two rules
 ...
 * out inside `capture` (D3). These are the assertions that keep them one.
 */
...
test('the manifest declares them early, and before the detector', () => {
  ...
  // nicety a pause may quiet, and a pin outranks what watches (The deliberate-capture precedence).
