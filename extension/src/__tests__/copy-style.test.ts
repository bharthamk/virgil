...
const COPY_MODULES = [
  ...
  'pin-modes.ts',
  // the quick take's three closing controls and the receipts they
  ...
  // most noticeable.
  'quick-take-close.ts',
  // the same take's two controls before it is opened. Two labels and
...
test('the copy modules are still the modules that hold copy', () => {
  ...
  assert.deepEqual(seen, [
    'account-scope.ts', 'action-popup-main.ts', 'action-popup.ts', 'arrival.ts',
    ...
    'check-surface.ts', 'course-material.ts',
    ...
    'panel-core.ts',
    // the panel's glyph set, moved out of `panel.ts` when a second
...
/**
 * exact operator constructions that escaped the earlier service-only
