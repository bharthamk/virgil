...
test('the first sighting is not a return, so three returns takes four visits', () => {
  ...
  // which is what the  walkthrough actually describes.
  const t = tracked();
...
test('a rejection reaches the detector, and two of them quiet the site ()', () => {
  const quiet = (): Harness => {
    const raised: Raised[] = [];
    const t = createRereadTracker<string>(mapVisitStore<string>(), (key, visit) => {
      raised.push({ key, visit: { ...visit } });
...
test('the heading path walks back up the document ()', () => {
