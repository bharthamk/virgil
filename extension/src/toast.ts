/**
 * SB-03: capture costs one gesture and zero context switches. The toast is the
...
/**
 * SB-59 — the one affordance the toast grows.
 *
 ...
 * The **plain** toast — offline, the SB-11 refusal, an unnamed success, every
...
export function showToast(initial: string): void {
  ...
  /** The page's, not ours, so it has to come off again (the toast-reachability contract). Null on
  ...
  // One pending dismissal at a time. It used to be one timer per call with the
  // caller cancelling the old one; holding the toast open needs to cancel and
  // re-arm repeatedly, so the bookkeeping lives in here instead.
  ...
  window.__sbFinishToast = (
    ...
    if (hasTake || hasUndo) {
      ...
      // The toast-reachability contract's courtesies belong to every toast action. A learner
    ...
    /**
     * Keep the quotation visible long enough to read without delaying plain status messages.
     *
