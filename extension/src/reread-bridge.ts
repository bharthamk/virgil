 ...
 * answers the  question on the way in.
 */
...
export interface RereadPrefsReply {
  /** this origin has had enough suggestions turned down. */
  quieted: boolean;
}
 ...
 * said no twice because a fetch timed out, and  is the product contract that exists to
...
export async function boot(send: Send, start: StartDetector, origin: string): Promise<() => void> {
  ...
  return start((candidate) => {
    // Never interrupt. The candidate is posted and then nothing happens
