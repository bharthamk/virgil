...
export interface SectionView {
   ...
   *  the list: The interface-affordance contract. */
...
export interface SessionView {
  ...
  /**  resume pointer, as the store keeps it: the first section still
 ...
 * is stated rather than hidden. But `Date.parse` answers NaN for
 ...
 *
 *  ready card set this at 48 and  cut section headings to it for
...
export interface SourceView {
  ...
  at: string | null;
  /** seconds into a video, when the pin was made on one. */
  moment?: number | null;
  /** which page of a PDF, when the viewer said. */
  page?: number | null;
  /**
...
/**
 * , in one line per source: whose page this was.
 *
...
export function sourceLine(source: SourceView): string | null {
  ...
  //  and. The noun changes with the clause: "the page you pinned, at
 ...
 */
/** "page 3", or nothing. Same rule as the moment above — a page number
...
/** a reference that resolved to nothing is said out loud. Dropping it in
...
/**
 * the register, as the learner reads it.
 *
...
/** topics are the display unit; pins are the evidence unit. Confusing
 ...
 *
 * There is no comfort here, and there is no percentage.  is that comfort
 ...
 *
 * The interface-affordance contract: no instruction copy, and the affordances ARE the instruction.
 ...
 *
 * A visible label would be the interface explaining itself, which the interface-affordance contract
 ...
 * turn collection off — so the product contract's promise was enforceable and unreachable
...
export interface PrefsView {
   ...
   *  for never — which is the default. The manual-processing contract. */
 ...
 *
 * The processing-pause contract put a persistent paused state on the main screen; the learner-action contract keeps
 ...
 * control stops a pin the learner makes by hand — that is The learner-confirmation contract, manual
...
/**
 * The reviewer boundary is visible on the screen rather than hidden in code.
 *
 ...
 * attribution is what goes, on the same rule  applies to a source it
 * cannot trace.
 */
...
export function cardHeading(card: SessionCardView | null): string {
  switch (card?.state) {
     ...
     */
    // The manual-processing contract: there is no run coming on its own, so telling somebody to
...
export interface LineupItem {
   ...
   *  sentence: see The interface-affordance contract. */
...
/** Not "completed", not a count. : nothing on any screen is a tally the
...
/** the rail on a genuinely new board no longer says one sentence about
 ...
 */
export const MOMENTUM_HEADING = 'What moved';
...
/**  four, in the milestone voice. A badge this build does not know is
...
export function flaggedLine(row: FlaggedRowView, now: number): string | null {
  ...
  // , and §5 lists it first. Said in the learner's own voice like the two
...
/** A door, not a view. The count is of topics — the display unit () —
...
//
//  holds: the headings are the names of the two places, never a tally of
...
/** the award moment is session end. An empty list is empty — there is
 ...
 *
 * The model-label contract: the label is model output over pinned text and is uncapped end to
 ...
 * `quick-take-close.ts` in , when the close grew its third answer.
...
/**
 *  route back to the pin mechanic, kept because it is the half of that
 ...
 * an empty body as teaching.  is explicit that the escalation coming to
...
export function quickTakeFailedLine(cause: GuideFailure = 'model'): string {
  switch (cause) {
    ...
    case 'unverified':
      return 'I could not verify that draft against your source, so I held it back.';
    case 'model':
    default:
...
// ------------------------------------------- what the take was written from
 ...
 *
 * A pause is a pause and a button is not a way round it (the processing-pause contract), so the
...
/**
 * The control that replaced the hour of the day — the manual-processing contract.
 *
 ...
 *  has kept out of every other room (Open Question 7's proposed bound,
 * ruled and accepted).
 */
 ...
 *
 * The separator is the house middot rather than the em-dash the contract wrote
 ...
 *
 * The same inversion My studies took : a room whose first screen is a
 ...
 * they are the same place.  shortened the door to the one word the top
 ...
 *
 * The criteria-extraction contract: the criteria are split out of the pasted text in code, verbatim,
...
/**
 *  — a finding, offered to the board.
 *
 ...
 * The learner-confirmation contract is the rule — a suggestion the user confirms — and a check that
