...
import {
  ...
  // The board-as-Learn-face layout: the board became a face of Learn rather than a room.
...
import {
  ...
  // The learner-controlled lineup contract: tonight's lineup, its five controls, and their sentences.
...
import {
  externalFace, BOARD_LEARN, BOARD_VIEW_PINS, BOARD_RUN_THEN_LEARN,
...
import { DEMO_WORKSPACE_ID, dataTransferMarkup, demoAccessCard, demoEntryRequested,
  demoModelReceipt, readDemoAccess, sharedDemoDataMarkup, signInMarkup, wireSignInAction,
  type DemoAccessView } from './demo-mode.js';
...
type NotebookOpen = 'tab' | 'same-page' | 'failed';

/**
 * Notebook follows a Drive write and may therefore outlive the browser's
 * short popup gesture window. Prefer a new tab; on the full hosted page, a
 * same-page navigation is the reliable last door. The extension panel cannot
 * navigate its own chrome-extension page away, so it keeps the visible link.
 */
async function openNotebookDestination(url: string): Promise<NotebookOpen> {
  try {
    await openBrowserTab(url);
    return 'tab';
  } catch {
    if (SURFACE !== 'page') return 'failed';
    const pageLocation = globalThis.location as Location | undefined;
    if (typeof pageLocation?.assign !== 'function') return 'failed';
    try {
...
async function openAddSourcePage(): Promise<void> {
  if (SURFACE === 'page') {
    ...
    return renderCourses(
      null, true, null, null, false, '.source-text', null, false, false, null, null,
      'add-source',
    );
  }
  try {
...
async function renderCaptureReceipt(pinId: string, fallbackLabel: string | null): Promise<void> {
  ...
  const choose = el(`<section class="capture-next" aria-label="What to do next">
    ...
    <p class="meta">Uses one Virgil model call.</p>
  </section>`);
...
function captureForwardRoutes(
  ...
  return materialForwardRoutes(label, pin.topicId, async () => pin.source.text ?? '');
}

/**
 * The one free handoff component used by Pins and Board.
 *
 * Pins already have their source body in hand. Board topics read their saved
 * pages only on the destination press. The presentation and the destination
 * behaviour stay identical either way: visible Gemini and Notebook doors,
 * the no-model-call boundary, and one receipt region.
 */
function materialForwardRoutes(
  label: string, topicId: string | null, readMaterial: () => Promise<string | null>,
): HTMLElement {
  const node = el(`<div class="gemini-routes capture-routes rail-block" data-zone="capture-routes">
    <span class="alt-label">Use Gemini or Notebook · no Virgil model call</span>
  ...
  for (const [where, routeLabel] of [
    ['tab', `Gemini · ${TUTOR_FORWARD_LABEL}`],
    ['beside', `Gemini · ${TUTOR_BESIDE_LABEL}`],
  ] as const) {
  ...
  notebook.addEventListener('click', async () => {
    ...
    await recordExternal({
      kind: 'material', label, destination: 'notebook', topicId, sessionId: null,
    });
...
interface ActionOptionView {
  ...
  /** `build` since 2026-08-24: a dated thing that names topics and has no
   *  lesson yet is offered the run that would make one. */
  ...
  /** SB-286: how many other topics the ranker has ready behind a quick take's
...
/**
 * Tell the worker its cached prefs are out of date (SB-40/41, SB-16).
 *
...
/** History returns to the exact useful start of an addressable page. Most
 * rooms start at their heading; the explicit source-intake address starts at
 * the field its popup door promised. */
function focusMainPageRouteStart(): void {
  if (SURFACE === 'page'
      && mainPageRoute(typeof location === 'undefined' ? '' : location.hash) === 'add-source') {
    const source = roomContent.querySelector('.source-text') as HTMLElement | null;
...
/** Browser autofill, render hydration and test/setup scripts can all assign a
 * field value. Only a person's native input event turns that assignment into
 * unsent work. The DOM harness has no native `isTrusted` member, so absence is
 * treated as a person there; a real synthetic browser event is explicitly
 * false and remains hydration. */
const trustedUserInput = (event: Event): boolean => event.isTrusted !== false;
const protectUnsentForm = (
  ...
  const touch = (event: Event): void => {
    if (trustedUserInput(event)) touched = true;
  };
  ...
  const entry: UnsentWork = {
    root, label,
    dirty: () => touched && fields.some((field, index) => fieldValue(field) !== baseline[index]),
    discard, focus,
  };
...
function masthead(opts: MastheadOptions = {}): HTMLElement {
  ...
  // on the board is the whole premise of SB-18.
...
/** Google establishes normal identity; an enabled Demo deployment also accepts its private password. */
async function renderSignIn(
  mode: 'sign-in' | 'switch-user' = 'sign-in', notice: string | null = null,
): Promise<void> {
  const switching = mode === 'switch-user';
  const judgeEntry = !switching && demoEntryRequested(SURFACE);
  ...
  const form = el(signInMarkup({
    switching, demo: judgeEntry, currentLabel: learnerLabel(current),
  }));
  ...
  const judgePass = form.querySelector('[data-judge-pass]') as HTMLInputElement | null;
  const signInForm = form.querySelector('[data-signin-form]') as HTMLFormElement | null;

  wireSignInAction({
    demo: judgeEntry, form: signInForm, input: judgePass, button: google, refusal,
    google: signInWithGoogle,
    currentUid: async () => (await readSession())?.uid ?? null,
    adopt: adoptLocalDraftOwner,
    completeDemo: async () => { await renderHome(); focusRoomStart(); },
...
async function renderAccount(): Promise<void> {
  ...
  const demo = session.uid === DEMO_WORKSPACE_ID && session.email === null;
  mountAccountSurface({
    content: owner.content, label: learnerLabel(session) ?? 'This account', demo, make: el,
    onSwitch: () => guardNavigation(
      () => focusAfterRoom(renderSignIn('switch-user'), 'signin'), { includePreserved: true },
    ),
    onSignOut: () => guardNavigation(async () => {
      forgetLocalDrafts();
      await signOut();
      await renderSignIn();
      focusRoomStart();
    }, { includePreserved: true }),
    onData: () => guardNavigation(
...
async function dataSettings(
  ...
  const demoBoard = session?.uid === DEMO_WORKSPACE_ID && session.email === null;
  if (demoBoard) ACCOUNT_BACKUP_DRAFT = null;
  board.replaceChildren();
  if (localBoard) {
    board.append(el(`<p class="meta">This is the one board on this Virgil installation. It does not need a sign-in.</p>`));
  }

  const transfer = el(dataTransferMarkup(demoBoard));
  ...
  if (demoBoard) {
    board.append(el(sharedDemoDataMarkup()));
    return;
  }
...
async function renderHome(
  ...
  const judgeRead = readDemoAccess(browserSession?.uid, apiResult<DemoAccessView>);

  /** First paint distinguishes an absent service from an authentication refusal. */
  const [sessionRead, prefs, todayRead, progression, judgeResult] = await Promise.all([
    apiResult<{
  ...
  // The collection-pause contract, above everything: while collection is off, "is it off?" outranks
  ...
  const modeToggle = learnModeToggle(); owner.content.append(modeToggle);
  if (judge) owner.content.append(demoAccessCard(el, judge));
...
interface LearnMount {
  ...
  /** Swapped in place by SB-286's *show me another*, which re-reads the ranking
  ...
  said: string | null;
  readonly judge: DemoAccessView | null;
...
/**
 * Going where the next move points.
 *
 * `targetId` used to be dropped on the floor here, and the Plan was the room
 * where it cost something: `/today` names a COMMITMENT — "Marketing analysis,
 * due in three days" — and pressing the button opened the plan at the top,
 * leaving the learner to find the row the product had just told them about.
 * The room draws attention to that card now, which is the difference between a
 * next move and a door.
 */
/**
...
function openAction(action: ActionOptionView, session: Session | null): void {
  ...
  // The id used to be dropped here, so a next move that said "Resolve one
  // course question" opened a room of every course and left the learner to
  // find the draft the product had just pointed at.
...
function fillNextAction(
  ...
  /**
   * What the session card said, on the card that now IS the session.
   *
   * §5 asked for the three registers in their three colours and for the
   * Gardener's own reason, and both used to be drawn in a separate zone under
   * this one. That zone was unreachable, and when the session leads it would
   * have been a second card about the same lesson with a second big button on
   * it. So the facts moved up here and the zone went; the accent stays on one
   * control, which is the one immediately below this.
   */
   ...
   *
   * The learner-controlled lineup contract: *"the expected level (the existing register chips)"* is now
  ...
  // SB-286: the one-minute hero's own pre-read controls, under the one accent.
...
/**
 * SB-286 — THE TWO GESTURES ON THE ONE MINUTE HERO.
 *
 ...
 * The learner-controlled lineup contract. *Show me another* writes no mark and moves nothing, so only the
 ...
 * The affordance-first interface contract bans.
 */
...
function lineup(session: Session, items: readonly LineupItem[]): HTMLElement {
  ...
  for (const item of items) {
    ...
    if (item.register) {
      ...
      // title. The ruling asks for the expected level to be shown; a
...
const BOARD_FACE_TITLE = 'Your learning board';
const BOARD_FACE_EXPLAINER =
  'Everything you have pinned, organised by where it is in your learning.';
...
async function drawBoardFace(m: LearnMount): Promise<void> {
  ...
  const wait = thinking(LOADING_HOME);
  const head = el(`<header class="board-face-head">
    <h1></h1><p class="board-face-explainer"></p>
  </header>`);
  (head.querySelector('h1') as HTMLElement).textContent = BOARD_FACE_TITLE;
  (head.querySelector('.board-face-explainer') as HTMLElement).textContent = BOARD_FACE_EXPLAINER;
  host.append(head, wait);
  ...
  // SB-15: the reveal. A proposal the user confirms — never a silent write.
...
/**
 * Show the board, from anywhere.
 *
 * On Learn this is a face change and costs nothing. From another room it is a
 * navigation to Learn that opens on the board face, which is what every path
 * that used to open the board room now does.
 */
...
/**
 * The paused state, where the learner already is (SB-40, the collection-pause contract).
 *
 * Pause used to live entirely on a screen two taps away, which was defensible
 * while every pause expired by itself: an hour later the product simply started
 * working again. "Until I turn it back on" removes that safety net, so the state
 * moves to the front door and stays there for as long as it is true.
 *
 * Three things it deliberately is:
 *
 *  - **Above zone 1.** §5's rule is that nothing may push the session down the
 *    page, and this is the one exception: while collection is off, "is it off?"
 *    outranks "what's on this run", and it is one line rather than a card. When
 *    nothing is paused it renders nothing at all, so the ordinary screen is
 *    exactly the screen §5 describes.
 *  - **One tap from the way back.** The resume button is the same write the
 *    privacy screen makes, so there is no second path to keep in step.
 *  - **Silent when the service is.** A banner is a claim about the authority.
 *    With no answer there is nothing true to say, and guessing either way is
 *    worse than saying nothing: "paused" would stop nothing, "watching" would
 *    hide a pause that is real.
 */
...
async function renderCheck(): Promise<void> {
  ...
  /**
   * The criteria box's own second route (The structured-criteria contract).
   *
...
/**
 * SB-39 — the finding that has become a subject.
 *
 * The Ask room's pattern, and deliberately the same one: an offer line, one
 * control, and nothing happens until it is pressed. The deliberate-capture precedence — a suggestion
...
function markBlocks(m: MarkView | null, limits: CheckLimitsView): HTMLElement[] {
  ...
  if (!m || m.outcome === 'model-failed') {
    // The quarantine survives the failure, because it is the half of the answer
    // that did work. The lines are pulled out of the paste before a model is
    // called, so a mark that could not run still knows that something in the
    // brief tried to tell it what to conclude — and the moment a learner most
    // needs to hear that is the moment nothing came back. This early return
    // used to drop the whole payload on the floor.
    return [
 ...
 */
/**
 * SB-40/41 — the half that had no surface.
 *
 * `GET/PUT /prefs` and the eleven shipped exclusions have been tested since they
 * were written, and until now there was no control anywhere: no pause, no way to
 * see the list, no way to add to it. The detector runs on every page, and the
 * story is explicit that behavioural monitoring is only acceptable *because*
 * these exist — so a list nobody can read is not the guarantee it claims to be.
 *
 * Both writes go to the service, which is the single authority, and then tell
 * the worker to re-read its cache.
 */
...
function processingSettings(prefs: PrefsView): HTMLElement {
  ...
  select.addEventListener('change', async (event) => {
    if (!trustedUserInput(event)) return;
...
function privacySettings(prefs: PrefsView): HTMLElement[] {
  ...
  const authoritativeDomains = (prefs.excludedDomains ?? []).join('\n');
  textarea.value = EXCLUDED_DOMAINS_DRAFT ?? authoritativeDomains;
  textarea.addEventListener('input', (event) => {
    if (trustedUserInput(event)) {
      EXCLUDED_DOMAINS_DRAFT = textarea.value === authoritativeDomains ? null : textarea.value;
    }
  });
...
const modelConnectionCheckRequest = (mode: ModelModeView, draft: ModelRoutingDraft): RequestInit =>
  mode === 'local' && draft.providers.local.setup?.connector !== 'supported'
    ? {
      method: 'POST', headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ endpoint: draft.providers.local.endpoint.trim() }),
    }
    : { method: 'POST' };
...
const stoppedCloudRouteLine = (map: ModelRoutingDraft, afterRemoval = false): string => {
...
async function modelRoutingSettings(host: HTMLElement): Promise<void> {
  ...
  const rememberDraft = (): void => {
    MODEL_ROUTING_MEMORY = sameDraft() ? null : {
      providers: Object.fromEntries(MODEL_MODES.map((mode) => [mode, {
  ...
  for (const mode of ['local', 'cli'] as const) {
    endpointInput(mode).addEventListener('input', (event) => {
      if (!trustedUserInput(event)) return;
  ...
  for (const check of Array.from(section.querySelectorAll<HTMLButtonElement>('[data-check-provider]'))) {
    check.addEventListener('click', async () => {
      ...
      const result = await apiResult<{ provider: ModelModeView; ok: boolean; status: string; detail: string }>(`/model-connections/${mode}/check`, {
        ...modelConnectionCheckRequest(mode, draft),
      });
  ...
  if (cloudKey) {
    ...
    cloudKey.addEventListener('input', (event) => {
      if (trustedUserInput(event)) CLOUD_KEY_DRAFT = cloudKey.value ? cloudKey.value : null;
    });
  }
  ...
  const openCloudKeyRemoval = (): void => {
    ...
    (confirm.querySelector('.impact') as HTMLElement).textContent = stoppedCloudRouteLine(authoritative);
    ...
    action.addEventListener('click', async () => {
      ...
      CLOUD_KEY_REMOVE_CONFIRM = false;
      CLOUD_KEY_REMOVE_NOTICE = `${stoppedCloudRouteLine(routingDraft(modelConfigFrom(result.body)), true)}${
...
async function modelBudgetSettings(host: HTMLElement): Promise<void> {
  ...
  const paint = (reading: BudgetReading, message: string, preserveDraft = false): void => {
    ...
    authoritativeLimit = reading.learnerLimit === null ? '' : String(reading.learnerLimit);
  ...
  limitField.addEventListener('input', (event) => {
    if (trustedUserInput(event)) {
      BUDGET_LIMIT_DRAFT = limitField.value === authoritativeLimit ? null : limitField.value;
    }
  });
...
async function driveSettings(host: HTMLElement): Promise<void> {
  ...
  const paint = (status: DriveStatusView): void => {
    ...
    if (hasClient) {
      // Value first: what the learner gets is the reason anybody reads this
      // block, and the permission reassurance is the line under it rather than
      // the headline over it. Built rather than emptied, so the state with no
      // sign in leaves no gap where four paragraphs used to be.
...
/** Show the shared Demo allowance as a receipt, never an owner control panel. */
async function demoModelSettings(host: HTMLElement): Promise<void> {
  const access = await readDemoAccess((await readSession())?.uid, apiResult<DemoAccessView>);
  host.replaceChildren(access ? demoModelReceipt(el, access)
    : el(`<p class="empty">Virgil could not read this Demo board’s model allowance. No connection or spend claim is being made.</p>`));
}
async function renderSettings(
  ...
  if (active === 'models') {
    const session = await readSession();
    if (session?.uid === DEMO_WORKSPACE_ID && session.email === null) {
      await demoModelSettings(subboard);
      return;
    }
...
/**
 * SB-42 — the learner model, in sentences, editable.
 *
...
async function renderModel(correctionSaved = false): Promise<void> {
  // The door says "Insights"; the screen behind it keeps the longer sentence,
  // which is the rule every other door follows. It used to say "What I've
  // worked out about you" and the door said "About you", so the top of the
  // screen said the same thing twice in two registers.
  ...
  // learner's answer where the preferences door cannot reach it (SB-282).
  ...
  const statementRow = (s: InsightStatementView): HTMLElement => {
    // SB-282: a modality read nobody has answered is a question, and it is the
     ...
     *
     * It writes through the door SB-282 built and marks the row endorsed
 ...
 */
const hasLessonOnBoard = (topicId: string): boolean =>
  !!learnMount?.session?.sections?.some((section) => section.topicId === topicId);

const lessonOnBoard = (topicId: string): boolean => {
  if (!hasLessonOnBoard(topicId)) return false;
 ...
 *
 * Which lesson: the one asked for, else the first still owed. SB-31 had a
...
function paintLesson(m: LearnMount, learning: Learning): void {
  ...
  /**
   * SB-31's other half: a session left long enough to go cold opens with two
 ...
 * SB-18 forbids a tally somebody can fall behind on, so neither list is
...
/** SB-32: three clauses, and the end of the thing. No dashboard, no percentage,
...
/**
 * Prompt forwarding lives in the rail rather than below the lesson.
 *
 * **The on-page brief is gone.** It rendered six statement lines at the foot of
 * every lesson, on the theory that the panel reads the page and the page should
 * therefore say enough. The panel does not read the page until it is pointed
 * there, every time — so the block was six lines of copy under every lesson,
 * paying off only after a manual step most learners will not take, on a screen
 * whose whole design argument is that nothing sits there without earning it.
 * Superseded rather than tidied away: the continuation content still exists,
 * whole, as the forwarded payload. It is just no longer duplicated onto a page
 * where it was cost without payoff.
 *
 * The rail is where a session's own affordances already live — the
 * Notebook hand-off has been there since §5d moved it — and real button
 * semantics are what that block uses. Their visual treatment now comes from
 * the room itself: Whiteboard markers and BlackBoard chalk share the same four
 * theme tokens, so none of the doors needs a generic product-control costume.
 *
 * **It costs nothing to draw**, still: every line of the prompt is computed
 * from the session already in hand, so nothing here makes a request, and the
 * press opens a tab, a window or the clipboard and does nothing else.
 */
interface GeminiRouteOptions {
  readonly externalTopicId?: string | null;
  readonly heading?: string;
  readonly forwards?: readonly ForwardWhere[];
  readonly nameGemini?: boolean;
}

function geminiRoutes(
  section: Section, after: Section | null, sessionId: string | null,
  options: GeminiRouteOptions = {},
): HTMLElement {
  const externalTopicId = options.externalTopicId === undefined
    ? section.topicId : options.externalTopicId;
  const brief = {
  ...
  (node.querySelector('.alt-label') as HTMLElement).textContent = options.heading ?? TUTOR_ROUTES_HEADING;
  const routes = node.querySelector('.routes') as HTMLElement;
  const out = node.querySelector('.routes-out') as HTMLElement;
  ...
  // A lesson already open inside Virgil's side panel must not offer the side
  // panel as a destination. Hosted lessons still offer the clipboard route,
  // because there it genuinely moves the lesson into the browser panel.
  const forwards = options.forwards ?? (SURFACE === 'panel'
    ? ['tab', 'beside'] : ['tab', 'beside', 'copy']);
  for (const [where, label] of forwardLabels.filter(([where]) => forwards.includes(where))) {
    ...
    btn.textContent = options.nameGemini ? `Gemini · ${label}` : label;
...
/**
 * §5d — the hand-off to Gemini Notebook, and the only place it exists.
 *
 * *"From a composed session, and nowhere else."* The affordance lives here
 * because this is where the learner is already holding a specific, finite,
 * self-selected set of material. A board-wide version would be a backlog with a
 * delivery van, and would invert the whole design: the pile the learner is never
 * shown becomes the pile the learner exports. It is banned from the board door,
 * from the capture toast (§2 takes no new decisions) and from every empty state,
 * where it would be a consolation errand.
 *
 * ## What the press does now, and what it used to do
 *
 * It used to copy the session's source URLs to the clipboard and open Notebook's
 * front door, leaving the learner to paste them into Notebook's own *Add
 * sources* step. That whole errand is gone, along with the clipboard, the file
 * offer and the on-screen list: the export seam already keeps documents in the
 * learner's own Drive and Notebook already re-reads them, so the honest press is
 * **rewrite the learn now document, then open the door**.
 *
 * Three properties this function is built around, in the order they bite:
 *
 *  - **It costs nothing to draw.** One request, on the tap, and none on render.
 *    Opening a session is not a build (SB-18), and there is nothing here to ask
 *    for until somebody asks for it.
 *  - **It writes documents and nothing else.** The one request it makes is the
 *    export door, which is pure prose over rows the store already holds: no
 *    signal, no comfort update, no progression event. §5d's ledger law is
 *    absolute and `panel-wiring.test.ts` reads the request log after the tap to
 *    hold it.
 *  - **The write happens before the tab.** A door opened onto a document that
 *    did not get rewritten is the one failure whose label already promised
 *    otherwise, and it is the failure this seam cannot see from the inside. So
 *    the order is: write, and only then open.
 *
 * **Not configured is a usable hand-off, not an error.** A hosted service has
 * nowhere durable to keep Drive OAuth credentials, so a 404 copies the visible
 * lesson and opens Notebook for a paste. The richer rewrite-in-place path stays
 * first wherever Drive is configured.
 *
 * ## It is a destination in the Gemini row now, not a block of its own
 *
 * It was a bordered button with a sentence
 * under it, directly below a group of three bordered buttons that take the same
 * lesson to the same company's products. Four destinations for one lesson is
 * one group.
 *
 * Two consequences, both deliberate:
 *
 *  - **The custody sentence is on the control.** `notebookPushSeamLine` is the
 *    title and the accessible name, which is the idiom the other three doors
 *    already follow, so nothing it said is lost and the group keeps the shape
 *    a cold read of it earned: labels, and no paragraph doing their job.
 *  - **It is lesson-scoped now, where it used to be session-scoped.** The group
 *    is drawn while a lesson is open and unfinished, so this leaves with the
 *    lesson. That is a real narrowing and it is the one the ruling asks for:
 *    the row says *take this lesson to*, the document it rewrites is the learn
 *    now document, and a hand-off offered where no lesson is open was the
 *    session's furniture rather than the lesson's. The empty-state ban §5d
 *    writes down is now structural: a night with nothing to teach draws no
 *    lesson, so it draws no group, so there is nothing to ban.
 */
function notebookRoute(
  ...
  btn.addEventListener('click', async () => {
    ...
    out.replaceChildren();
    try {
    // A foreground lesson has no stored nightly session. On the hosted page,
    // replace the ordinary Learn-now projection with the exact visible lesson
    // before rewriting the same three native Docs. The extension/local route
    // retains the bounded clipboard hand-off because it has no browser-owned
    // Drive grant. Never ask `/notebook/export` to write a different session.
    if (sessionId === null) {
      const title = lessonTitle(section.heading, subjectOf(section));
      const exactLesson = foregroundNotebookDocument(
        title.family, title.area, section.body, section.question?.prompt ?? null,
      );
      // Hosted production holds the durable refresh grant in Secret Manager.
      // Let that credential rewrite the same three sources Notebook already
      // reads. The browser-owned Drive lane remains the self-host fallback.
      const managed = await apiResult<{ ok?: boolean; line?: string; notebookUrl?: unknown }>(
        '/notebook/export',
        {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({ foreground: {
            key: exactLesson.key, title: exactLesson.title, body: exactLesson.body,
          } }),
        },
      );
      if (managed.kind === 'ok' && managed.body.ok === true) {
        const destination = notebookTarget(managed.body.notebookUrl);
        const opening = await openNotebookDestination(destination);
        if (opening === 'tab') {
          await recordExternal({
        ...
        out.append(el(`<div class="meta wrote">${esc(managed.body.line ?? 'I refreshed Virgil’s three Notebook sources in Google Drive.')}</div>`));
        if (opening === 'failed') {
      ...
      if (!(managed.kind === 'refused' && managed.status === 404)) {
        out.append(el(`<div class="meta failed">${esc(notebookPushFailedLine())}</div>`));
        return;
      }
      if (chrome.runtime.id === '') {
        const [setup, learner, origin, sources] = await Promise.all([
          apiResult<{ expectedAccount?: string; notebookUrl?: unknown }>('/notebook/drive/hosted-setup'),
          readSession(),
          serviceBase(),
          apiResult<{ documents?: HostedNotebookDocument[] }>('/notebook/documents'),
        ]);
        if (setup.kind === 'ok' && setup.body.expectedAccount && learner
          && sources.kind === 'ok' && Array.isArray(sources.body.documents)
          && !HOSTED_NOTEBOOK_DOC_KEYS.some((key) => key !== LEARN_NOW_DOC
            && !sources.body.documents?.some((document) => document.key === key))) try {
          const documents = HOSTED_NOTEBOOK_DOC_KEYS.flatMap((key) => key === LEARN_NOW_DOC
            ? [exactLesson]
          ...
          out.append(el(`<div class="meta wrote">${esc(hostedNotebookWrittenLine(
            written.account,
    ...
    if (!opened) {
      ...
      // The whole hand-off is one foreground gesture and stays single-flight until its receipt.
      btn.disabled = false;
    }
  });
  return btn;
 ...
 * one. What SB-18 protects is unchanged and is the reason this is careful: no
 ...
 * The evidence unit is still the evidence unit — SB-33 is untouched, a pin is
...
/**
 * SB-111/SB-115: one pin, before another model call.
 *
...
async function renderPinDetails(
  ...
  // SB-117: availability is what the learner observed after opening the link,
...
async function boardSurface(
  ...
  const draw = (q: string): void => {
    ...
    // The pins nothing has filed yet. They used to head an area of their own
    // directly above the topics; they belong INSIDE "Get Started", because
    // "you pinned this and nothing has begun on it" is the one thing that area
    // is for and two headings saying it is a distinction only the machine cares
    // about. The strip is the board at rest; a search answers with the pins it
    // matched instead, under the group that says where they are.
    ...
    for (const column of boardColumns(topics, new Set(pendingLessons.keys()))) {
      ...
      // Topics first, then the pins. SB-33's order — the display unit before
 ...
 *
 * There is no comfort on the card and no percentage. SB-33 is that comfort is
...
function topicCard(
  ...
  const node = el(`<article class="card" data-state="${esc(pendingLesson ? 'pending' : view.state)}" data-guide-section="topic">
    ...
    <div class="board-card-routes"></div>
    <div class="pins"></div>
    <div class="repair-form"></div>
  </article>`);
  const label = node.querySelector('[data-open]') as HTMLElement;
  label.textContent = view.label;
  // The lineup mark excludes completed sections, but a completed lesson is
  // deliberately still openable for re-reading. Key the door promise to the
  // navigation predicate, not to the narrower "in tonight's lineup" badge.
  const opensLesson = hasLessonOnBoard(t.id);
  /**
   ...
   * about either, so both doors were discovered by pressing. The affordance-first interface contract forbids
   ...
   * different control.
   */
  const door = boardTitleDoorLine(view.label, opensLesson);
  ...
  /**
   * Learn and the free handoffs are explicit controls.
   *
   ...
   *
   * Every other topic exposes its saved evidence rather than a lesson, so its
   * visible action says `View pins`. Calling that disclosure `Learn` promised a
   * lesson the press could not open and made a no-model-call read look like paid
   * generation. The title keeps working because a thing that reads like a link
   * should behave like one; both controls now name the same honest destination.
   *
   * The handoffs are the same component Pins uses: Gemini and Notebook are
   * visible on the card, name the no-model-call boundary, and read the saved
   * material only on the destination press. Board must not make the learner
   * open a legacy popup to discover the product's free routes.
   */
  ...
  if (pendingLesson) {
    ...
    learn.textContent = opensLesson ? BOARD_LEARN : BOARD_VIEW_PINS;
  ...
  const routeHost = node.querySelector('.board-card-routes') as HTMLElement;
  if ((t.pinIds?.length ?? 0) > 0) {
    routeHost.append(materialForwardRoutes(view.label, t.id, () => savedPagesFor(t.id)));
  } else routeHost.remove();
  return node;
}
...
/**
 * SB-59 — the quick take, the "now" moment of the three-moment loop.
 *
 ...
 *
 * Everything here is subordinate to a session on purpose (SB-60). The standing
 ...
 */
/**
 * Wait for the pin the worker is making, and come back with the reason when
 * there is not one.
 *
 * It used to return `string | null`, and every screen read that `null` as "the
 * model could not do it" — which was never true on this path, because on this
 * path the model has not been reached. Two things end the wait now: the id, or
 * the worker saying the post failed. What is left at the timeout is a worker
 * that never came back, which MV3 may do at any moment, and that is its own
 * answer rather than a guess dressed as one.
 */
...
/** Give every quick-lesson surface the same honest lesson handoff payload. */
function quickTakeSection(
  answer: ForegroundQuickTakeReply, pinId: string, shownLabel: string, minutes: number,
): Section {
  ...
  return {
    ...
    // A withheld draft may never become teaching, but the learner's own source
    // can still travel to a destination they explicitly choose. This fallback
    // is used only by those external handoffs; the lesson renderer continues
    // to require a ready, non-empty model answer below.
    body: answer.body || [
      answer.pinned?.pageTitle, answer.pinned?.text, answer.pinned?.url,
    ].filter((part, index, all): part is string =>
      typeof part === 'string' && !!part.trim() && all.indexOf(part) === index).join('\n\n')
      || shownLabel,
    depth,
...
async function renderForegroundQuickTake(
  ...
  const topicId = answer.topicId ?? null;
  const section = quickTakeSection(answer, pinId, shownLabel, minutes);
  const title = lessonTitle(section.heading, subjectOf(section));
  const surfaces = foregroundLessonSurfaces({
    ...
    handoffs: geminiRoutes(section, null, null, { externalTopicId: topicId }),
...
/** Wire the side panel's local way out; it returns to Pins and writes nothing. */
function wireQuickTakePinsExit(node: HTMLElement): void {
  node.querySelector('[data-back-pins]')!
    .addEventListener('click', () => void renderHome(null, null, 'pins'));
}

/** External lesson handoffs shaped for the side panel already being used. */
function quickTakeElsewhere(
  answer: ForegroundQuickTakeReply, pinId: string, pendingLabel: string | null,
  requestedMinutes: AvailableMinutes | null,
): HTMLElement {
  const routes = geminiRoutes(quickTakeSection(
    answer, pinId, shortLabel(answer.label ?? pendingLabel), requestedMinutes ?? 2,
  ), null, null, {
    externalTopicId: answer.topicId ?? null,
    heading: 'Learn elsewhere · no Virgil model call',
    forwards: ['tab', 'beside'],
    nameGemini: true,
  });
  routes.classList.add('quick-take-routes');
  return routes;
}
...
async function renderQuickTake(
  ...
  // before the take lands. Cut at render, the untrusted-label rendering contract: the label is model output
  ...
  const node = el(`<div class="quick-take">
    <nav class="quick-take-nav" aria-label="Quick lesson navigation">
      <button class="link" type="button" data-back-pins>Back to Pins</button>
    </nav>
  ...
  const takePath = `/pins/${encodeURIComponent(pinId)}/quick-take`
    + (requestedMinutes === null ? '' : `?minutes=${requestedMinutes}`);
  const result = await apiResult<ForegroundQuickTakeReply>(takePath, { method: 'POST' });
  if (result.kind !== 'ok') {
    return giveUp(failureOf(result), pending.label, true, pinId, result);
  }
  const answer = result.body;
  // These exits use the saved source when a draft is withheld. They are local
  // until pressed and remain useful precisely when Virgil declines to show its
  // own lesson, so build them before branching on the lesson outcome.
  const elsewhere = quickTakeElsewhere(answer, pinId, pending.label, requestedMinutes);
  ...
  /**
   * SB-283: the close, which is what makes this screen a surface rather than a
  ...
  const ask = async (question: string): Promise<void> => {
    ...
    // SB-30's route back to the pin mechanic, and the half of it worth
  ...
  // After the lesson and its follow-up tools, because it is the answer to
  // something that has to have been read first. Nothing is written until one
  // is pressed: a learner who closes the panel here has answered nothing, and
  // the board records nothing, which is the honest reading of walking away.
  ...
  // External destinations are alternatives to the complete Virgil flow, not a
  // divider inside it. They come last, and the side-panel-only route set never
  // offers to open the side panel the learner is already using.
  node.append(exchange, asker, closing, elsewhere);
}
...
async function renderGuide(pending: Handoff): Promise<void> {
  ...
  /**
   * Say why, and say where the pin is.
   *
   * Every failure on this screen used to be one sentence blaming the model, and
   * it was drawn before anything had been put on screen, so the screen that
   * said *"the pin is safe"* was the only screen that never showed the pin.
   * `saved` is false only where the pin genuinely did not land, which is the
   * one case where promising it is on the board would be a lie.
   */
  const giveUp = (
    cause: GuideFailure, label: string | null, saved: boolean, retryPinId: string | null = null,
    refusal: Unreadable | null = null,
...
async function boot(): Promise<void> {
  ...
  if (SURFACE === 'page') {
    ...
    // The popup's Add it another way door promises the intake itself, not just
    // the room containing it. Browser startup and restored guide state can
    // move focus after the asynchronous course reads finish, so make that
    // promise true at the end of the real boot as well as inside renderCourses.
    if (route === 'add-source' && !document.getElementById('virgil-guide-overlay')) {
      focusMainPageRouteStart();
    }
    return;
  }
...
if (SURFACE === 'page' && typeof window !== 'undefined') {
  ...
  window.addEventListener('popstate', () => {
    if (acceptNextHistoryMove) {
      ...
      void renderMainPageRoute().then(() => focusMainPageRouteStart());
      return;
    }
...
/**
 * THE PLAN — a tutor line, three lanes, and three weeks.
 *
 * The room combines calendar, kanban, and tutor guidance for study planning.
 * Its former shape was a points total, a four-field form, one
 * flat list of every dated thing, and two result surfaces bolted underneath.
 * Every part of that was a data-entry screen wearing a plan's name — the first
 * sentence in the room was a score, the second was a form, and what the learner
 * had actually agreed to came third.
 *
 * What it is now, in the order it is read:
 *
 *  - **the tutor line**, from `GET /today`. One line, never a card: the front
 *    door owns the hero, and a second one here would be the same decision drawn
 *    twice. It is re-read after every tick, reopen and move, so it tracks.
 *  - **three lanes** — Now, This week, Ahead — which are projections of the
 *    service's own state and not buckets anybody drags between. Late leads Now
 *    and the lane is never named after it.
 *  - **three weeks**, with deadlines drawn solid and planned study drawn
 *    lighter. A card dropped on a day moves `plannedFor` and can never move a
 *    due date.
 *  - **Done**, quiet, under both, with the score at its foot. Points still
 *    exist; they have stopped being the room's first sentence.
 *
 * The two rules that did not move: **nothing here punishes** — a late card is
 * stated, ordered first, and still scores — and **nothing here counts
 * anything.** No lane has a number on it, no day cell has a number in it, and
 * the words "overdue" and "remaining" appear nowhere in the room.
 *
 * `focus` is `/today`'s `targetId`, carried in from `openAction`. The card it
 * names is marked rather than the room being scrolled to the top, which is what
 ...
 *
 * SB-74 is the reopen: `POST /commitments/{id}/reopen` was written, tested and
...
function cardMenu(c: CommitmentView, ctx: CardContext, host: HTMLElement): HTMLElement {
   ...
   */
    /**
     * Both halves of "what is this part of", asked once.
     *
     * The action used to set topics alone, and it was gated on the board having
     * some. It sets the COURSE as well now, because the learner-controlled lineup contract
     * made that field the only real join between a topic and the subject shown
     * beside a lesson on tonight's lineup — and every commitment made before
     * course intake existed carries `courseId: null` with no way to give it
     * one. `PUT /commitments/:id` accepted the field and dropped it silently
     * until 2026-08-24, so even the API could not fix an old row.
     *
     * One question, one save. The two pickers are appended to the same field
     * grid the add form uses, so this reads as the form it already is rather
     * than as a second way of asking.
     */
 ...
 * number** — Open Question 7's proposed bound, which is SB-18 applied to the
...
function outcomeRow(outcome: OutcomeView, context: OutcomeContextView | null = null): HTMLElement {
    ...
    /**
     * WHAT THIS ROW IS CURRENTLY CLAIMING.
     *
     * It used to be the argument, drawn once and never again. The correction
     * posted, the service replaced the record, and the row went on displaying
     * the number that had just been superseded — 62 / 100 under a receipt line
     * saying the correction was recorded. That is `pinsSince`'s lie in a
     * different room: a surface that disagrees with the receipt directly under
     * it about the same number teaches the learner to trust neither.
     *
     * Worse than a stale number, because the correction endpoint SUPERSEDES.
     * It marks the old record deleted, writes a replacement under a new id and
     * answers with it in `outcome` — so the id this row was closed over is a
     * 404 ("no such active outcome") the moment the first correction lands. The
     * control removed itself before the request and never came back, which hid
     * that: a learner could not correct twice without leaving the room and
     * coming back, and had they been able to, the second one would have failed
     * against a record that no longer exists.
     *
     * So the row keeps what it is currently about, repaints from the service's
     * own replacement, and aims the next correction at the new id.
     */
    let current = outcome;
    ...
    open.addEventListener('click', () => {
      ...
      save.addEventListener('click', async () => {
        ...
        release();
        /**
         * The replacement the service wrote, not the strings that were typed at
         * it. `outcomeFromBody` is what decided whether "72" is a score, and it
         * is the only thing that knows the new id. The fallback is for a
         * service too old to return the record it just wrote: the numbers on
         * screen are then still true, and the next correction fails honestly
         * against the superseded id rather than silently correcting nothing.
         */
 ...
 * it, and SB-33 is that comfort is never shown as a number. Nothing here counts
 * what is left, either — a room that opens with "7 things outstanding" is the
 * backlog SB-18 forbids, wearing a different room's clothes.
 */
...
async function renderCourses(
  ...
  pageRoute: MainPageRoute | null | undefined = undefined,
): Promise<void> {
  // Built before the frame, which clears the surface it would be appended to.
  const add = el(`<button class="primary" data-add-open>Add</button>`) as HTMLButtonElement;
  add.setAttribute('aria-expanded', 'false');
  ...
  frame('courses', {
    title: 'My studies', action: add,
    ...(pageRoute === undefined ? {} : { route: pageRoute }),
  });
  roomContent.setAttribute('data-guide-target', 'manage-surface');
  roomContent.setAttribute('data-guide-section', 'manage-state');
  const owner = roomOwnership();
