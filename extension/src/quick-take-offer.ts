...
/**
 * SB-286 — THE ONE MINUTE HERO, WITH SOMETHING TO SAY BACK.
 *
 ...
 * SB-282's exclusion — neither of these can reach the modality tallies, because
 * one writes a preference kind the weight table has no entry for and the other
 * writes no signal at all.
 *
 * Extracted rather than added to `panel.ts` for the reason SB-283 gives: a room
 ...
 * (The affordance-first interface contract).
 *
