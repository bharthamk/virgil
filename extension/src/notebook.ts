...
export interface ForegroundNotebookDocument {
  readonly key: typeof LEARN_NOW_DOC;
  readonly title: 'Virgil: learn now';
  readonly body: string;
  readonly html: string;
}

const notebookHtmlText = (value: string): string => value
  .replace(/&/g, '&amp;')
  .replace(/</g, '&lt;')
  .replace(/>/g, '&gt;')
  .replace(/"/g, '&quot;')
...
const notebookParagraphs = (value: string): string => value.trim()
  .split(/\n\s*\n/)
  .filter(Boolean)
  .map((paragraph) => `<p>${notebookHtmlText(paragraph).replace(/\n/g, '<br>')}</p>`)
  .join('\n');

/**
 * The exact lesson currently visible on the full-page Learn surface, rendered
 * as the one stable Drive source Notebook already knows.
 *
 * Foreground lessons have deliberately not been persisted as nightly sessions.
 * Asking `/notebook/export` for `learn-now` would therefore write some other
 * stored lesson. This small pure builder keeps the hand-off exact without
 * creating a fourth source or sending the question's marking points.
 */
export function foregroundNotebookDocument(
  family: string | null, area: string, body: string, question: string | null,
): ForegroundNotebookDocument {
  const exactFamily = family?.trim() ?? '';
  const exactArea = area.trim();
  const exactQuestion = question?.trim() ?? '';
  const lessonHeading = exactArea || exactFamily || 'Your current lesson';
  const markdownSubject = exactFamily ? `\n**Subject:** ${exactFamily}\n` : '';
  const markdownPractice = exactQuestion ? `\n## Test your knowledge\n\n${exactQuestion}\n` : '';
  const markdown = '# Virgil: learn now\n\n'
    + 'Virgil wrote this. This is the lesson you sent from Learn.\n\n'
    + '## The lesson in front of you\n'
    + markdownSubject
    + `\n### ${lessonHeading}\n\n${body.trim()}\n`
    + markdownPractice;
  const subject = exactFamily ? `<p><strong>Subject:</strong> ${notebookHtmlText(exactFamily)}</p>\n` : '';
  const practice = exactQuestion
    ? `<h2>Test your knowledge</h2>\n<p>${notebookHtmlText(exactQuestion).replace(/\n/g, '<br>')}</p>\n`
    : '';
  return {
    key: LEARN_NOW_DOC,
    title: 'Virgil: learn now',
    body: markdown,
    html: '<!DOCTYPE html>\n'
      + '<html><head><meta charset="utf-8">\n'
      + '<title>Virgil: learn now</title>\n'
      + '</head><body>\n'
      + '<h1>Virgil: learn now</h1>\n'
      + '<p>Virgil wrote this. This is the lesson you sent from Learn.</p>\n'
      + '<h2>The lesson in front of you</h2>\n'
      + subject
      + `<h3>${notebookHtmlText(lessonHeading)}</h3>\n`
      + `${notebookParagraphs(body)}\n`
      + practice
      + '</body></html>\n',
  };
}
