...
async function request(url: string, init: RequestInit = {}, retry = true): Promise<Response> {
  ...
  if (response.status === 401 && retry) {
    ...
    accessToken = '';
    // Chrome keeps Identity API tokens in its own cache. Clearing only this
    // module's copy can hand the exact rejected token back on the retry. The
    // hosted web shim obtains a new Google token on every call and does not
    // expose this Chrome-only method, so keep that runtime optional.
    const identity = chrome.identity as typeof chrome.identity & {
      removeCachedAuthToken?: (details: { token: string }) => Promise<void>;
    };
    if (rejected && typeof identity.removeCachedAuthToken === 'function') {
      await identity.removeCachedAuthToken({ token: rejected }).catch(() => {});
    }
    await token(true);
...
const queryLiteral = (value: string): string => value
  .replace(/\\/g, '\\\\')
  .replace(/'/g, "\\'");

/**
 * Recover the original app-owned source when browser storage has been cleared.
 *
 * Drive's `drive.file` scope lists only files this app created or the learner
 * explicitly opened with it, so an exact title inside Virgil's exact folder is
 * a bounded recovery set. When an older build already made duplicates, the
 * oldest file is the stable identity most likely to be attached to Notebook.
 */
async function oldestSource(
  document: HostedNotebookDocument, folderId: string,
): Promise<string> {
  const q = `mimeType = '${DOC_MIME}' and name = '${queryLiteral(document.title)}' `
    + `and '${queryLiteral(folderId)}' in parents and trashed = false`;
  const listed = await request(`${API}/drive/v3/files?q=${encodeURIComponent(q)}`
    + '&spaces=drive&fields=files(id,createdTime)&pageSize=100');
  if (!listed.ok) {
    throw new Error('I could not recover your existing Notebook sources in Google Drive. Nothing was sent.');
  }
  const files = (await json(listed)).files;
  ...
  return files.flatMap((file) => {
    ...
    return typeof row.id === 'string' && row.id
      ...
      : [];
  }).sort((a, b) => a.createdTime.localeCompare(b.createdTime) || a.id.localeCompare(b.id))[0]?.id ?? '';
}
 ...
 * Each exact name is also resolved inside that folder before a write. Browser
 * storage is a cache, not the identity authority: clearing it must recover the
 * original source rather than silently create a second set Notebook does not
 * read. If an older build already made duplicates, the oldest app-owned file
 * wins and is persisted again.
...
export async function writeHostedNotebookDocuments(
  ...
  try {
    for (const document of documents) {
      ...
      const recovered = await oldestSource(document, folderId);
      const existing = remembered || recovered;
      if (existing) {
        const updated = await request(
          `${API}/upload/drive/v3/files/${encodeURIComponent(existing)}?uploadType=media&fields=id`,
          { method: 'PATCH', headers: { 'content-type': 'text/html; charset=UTF-8' }, body: document.html },
        );
        if (updated.ok) {
          driveWrites += 1;
          files[document.key] = existing;
          await chrome.storage.local.set({
            [storageKey]: { account: email, folderId, files: { ...files } },
          });
          written.push({ account: email, key: document.key, fileId: existing, created: false });
          continue;
        }
