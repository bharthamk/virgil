/**
 * STANDARD — the pin you look at before you make it (`mode-standard`).
 *
 * Flash is one gesture and no decisions. This is the mode for the other case:
 * the learner has a moment, and wants to see what is actually being captured
 * before it goes on the board. Which turned out to matter more than it sounds,
 * because the first real pin anybody made captured an AI notice and a
 * navigation list without anybody being able to tell.
 *
 * Four things it can change and nothing else can:
 *
 *  - **the passage**, because a selection is often nearly right, and trimming
 *    it is faster than re-selecting it;
 *  - **the context**, which is the page's own title and is the strongest hint
 *    the topic model gets. A page about hotels carrying a passage about
 *    cooking steak used to file the steak under hotels, with nothing on
 *    screen saying so or letting anybody correct it;
 *  - **a note**, which the service has accepted on every pin since it was
 *    written and which nothing in the extension has ever sent. The Composer
 *    and the quick take both read it as the learner's own words about their
 *    own material, which is the most valuable sentence on any pin;
 *  - **how much they want on it**, whose bottom option also carries the
 *    struggle signal.
 *
 * ## Why the state lives in the page
 *
 * The box waits for a person, and MV3 kills a service worker that is waiting.
 * So nothing is held in the worker across the wait: the whole envelope is
 * handed to the box, and the box hands it back with the edits on it in one
 * message. A worker that died while the learner was reading wakes up on that
 * message with everything it needs, and the pin lands anyway.
 *
 * ## Why the structure is static
 *
 * Every dynamic value here is text from somebody else's page. None of it is
 * interpolated into markup: the template below contains no `${}` at all and
 * the passage arrives through `.value` and `.textContent`, which cannot be
 * parsed as anything. That is a stronger guarantee than escaping it correctly,
 * and it needs no escaping function to be right.
 */
