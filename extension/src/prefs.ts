...
export interface CachedPrefs {
  ...
  /** , riding the same copy: the per-origin rejection counts. */
...
export function mayObserve(
  ...
  if (isPaused(prefs, now)) return false;          //
  const host = hostOf(url ?? '');
  if (!host) return false;                          // a page we cannot name
  return !isDomainExcluded(host, prefs?.excludedDomains); //
}

/**
 * The same question with  folded in, which is the whole of what the worker
 ...
 *
 * The learner-confirmation contract’s spirit is that manual is the trusted spine, and a deliberate
 ...
 * costs something real — a paper opened from disk is a file url, and  is
