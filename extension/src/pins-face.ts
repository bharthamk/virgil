...
function pinCard(
  ...
  const sourceLength = Array.from(pin.source.text ?? '').length;
  const noteLength = Array.from(pin.note ?? '').length;
  const collapsible = sourceLength > 160 || noteLength > 140;
  if (!pin.source.text) excerpt.remove();
  if (!collapsible) {
    more.remove();
  } else {
  ...
  if (collapsible) {
    globalThis.requestAnimationFrame?.(() => {
      if (!card.isConnected || card.getAttribute('data-expanded') === 'true') return;
      const clipped = [excerpt, own].some((line) => line.isConnected
...
export async function mountPinsFace(host: HTMLElement, shell: PinsFaceShell): Promise<void> {
  ...
  const addGroup = (heading: string, detail: string, pins: PinSummary[]): void => {
    const section = el(`<section class="pins-group"><header class="pins-group-head"><h1></h1><p></p></header><div class="pins-list"></div></section>`);
    (section.querySelector('h1') as HTMLElement).textContent = heading;
