...
/** Validate the one unsaved field a connection check may preview without
 * widening Cloud or CLI into browser-selected network destinations. */
export function modelConnectionCheckEndpoint(
  mode: ModelMode, body: Record<string, unknown>,
): { readonly endpoint?: string; readonly error?: string } {
  if (Object.keys(body).some((key) => key !== 'endpoint')) {
    return { error: 'connection check body may contain only endpoint' };
  }
  if (mode !== 'local' && body.endpoint !== undefined) {
    return { error: 'only a Local connection check accepts an endpoint' };
  }
  if (body.endpoint !== undefined && typeof body.endpoint !== 'string') {
    return { error: 'endpoint must be a string' };
  }
  return mode === 'local' && typeof body.endpoint === 'string' ? { endpoint: body.endpoint } : {};
}
