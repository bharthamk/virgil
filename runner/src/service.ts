...
import {
  NotebookScopeError, exportNotebook, exportForegroundNotebook,
  foregroundNotebookDocument, notebookScope,
...
import {
  ...
  modelConnectionCheckEndpoint, modelEndpoint, type ModelMode, type ModelProviderToggles, type ModelRoutes,
...
import { handleJudgeAccessRoute, handleJudgeLoginRoute, judgeDemoOverrides } from './judge-demo.js';
import { BOARD_WEB_ROOT, serveBoardWeb, type BoardWebOptions } from './board-web.js';
import {
...
/**
 * A request this service will not act on because of what the CLIENT sent.
 *
 * Separated from every other throw for one reason: a 500 means "something on
 * this side broke and somebody should look at the log", and a body with a
 * missing field is neither. Both used to land on the same generic handler, so a
 * typo in the extension and a corrupt store were the same colour from the
 * outside — and the extension, which shows the body to nobody, had no way to
 * tell a fault it should retry from one it never should.
 *
 * The message is plain and names the field. It is read by whoever is wiring the
 * extension, not by the learner.
 */
/**
...
/**
 * The pin a confirmed suggestion becomes (SB-15, step 4: "It becomes a struggle
 * pin").
 *
 ...
 *
 * `rejectedOrigins` is deliberately absent from this list. It is the SB-16
...
const validPrefsPatch = (body: Record<string, unknown>): Record<string, unknown> => {
  ...
  take('excludedDomains',
    ...
    'an array of strings');
  // The event-driven processing contract. Null means never automatic, which is the default and the only
...
/**
 * The site a suggestion came from, for the SB-16 count. Null if unparseable.
 *
 ...
 * them shared one bucket named "null". SB-16 quiets at two rejections, which
...
/** One source, as SB-44's tap needs it: whose page it was, what it was called,
...
interface ResolvedSource {
  ...
  /**
   * SB-10: seconds into the video this pin was made at, or null.
   *
  ...
  /** SB-11: which page of the PDF, when the pin came off one. */
...
function resolveSources(
  ...
  for (const p of pins) {
    ...
    index.set(`${p.id}:origin`, {
      ...
      // The provenance tap used to resolve six selections from one page into
      // six identical links. The URL says where; this says what evidence the
      // learner actually chose there. Prefer the deliberate selection, with
      // surrounding text as the whole-page fallback, and keep the receipt
      // bounded so opening sources cannot become another reading surface.
...
/** SB-31: progress is tracked per section so a resume does not discard the
...
/**
 * The session with one section taken out of it — The learner-controlled lineup contract's X.
 *
...
/**
 * The session's sections in the order the learner put them — The learner-controlled lineup contract's
...
export interface AppOptions {
  ...
  /** Dedicated password-backed preview board; never inherits personal setup. */
  readonly judgeSession?: boolean;
  ...
  /** Server-held grant used only inside an authenticated learner router. */
  readonly hostedNotebookDrive?: ManagedDriveGrant | null;
...
export interface ModelServiceOptions {
  ...
  /** `day` is reserved for a deployment-owned per-identity allowance. */
  readonly operatorWindow?: ModelBudgetWindow;
  ...
  /** A deployment-owned board may expose model state but no setup controls. */
...
async function modelReceipt(store: Deps['store'], options: ModelServiceOptions = {}) {
  ...
  const providers: Record<ModelMode, ModelChoiceReceipt> = {
    ...
    local: {
      ...
      setup: {
        editable: !options.readOnly && (!options.hosted || connectorSupported), managed: false,
  ...
  return {
    schemaVersion: MODEL_CONFIG_SCHEMA_VERSION, providers, routes, limits: CHECK_LIMITS,
...
async function checkModelConnection(
  mode: ModelMode, store: Deps['store'], options: ModelServiceOptions, requestedLocalEndpoint?: string,
  ...
  if (mode === 'local') {
    ...
    try {
      endpoint = modelEndpoint(
        requestedLocalEndpoint ?? prefs.localModelEndpoint,
        options.localEndpoint ?? DEFAULT_LOCAL_MODEL_ENDPOINT,
...
export function createApp(deps: Deps, opts: AppOptions = {}): RequestListener {
  ...
  if (!identity || !forLearner) {
    ...
    if (await handleJudgeLoginRoute(req, res, requestUrl, {
      access: opts.access ?? null, readBody, reply: json,
    })) return;
    ...
    try {
      learner = token ? opts.access?.judgeIdentity(token) ?? await identity.verify(token) : null;
    } catch { learner = null; }
    ...
    if (!opening) {
      ...
      opening = forLearner(learner).then((boardDeps) => routes(boardDeps, {
        ...opts, secret: null, learner, ...judgeDemoOverrides(opts, learner),
      }));
  ...
  return (req, res) => serveBoardWeb(req, res, web, allowedOrigin) ? undefined : dataApp(req, res);
}
...
function routes(deps: Deps, opts: AppOptions = {}): RequestListener {
  ...
  const models: ModelServiceOptions = {
    ...(opts.models ?? {}),
    setupAuthenticated: !opts.judgeSession && (secret !== null || Boolean(opts.identity)),
    readOnly: opts.judgeSession === true,
  };
  const modelSetupAllowed = Boolean(models.setupAuthenticated || models.setupTrustedLocal);
  const managedNotebook = opts.hostedNotebookDrive
    ? new DriveNotebookExport({
      auth: new DriveTokens({
        client: () => opts.hostedNotebookDrive!.client,
        refreshToken: () => opts.hostedNotebookDrive!.refreshToken,
        clock: deps.clock,
      }),
      ids: managedDriveIds(deps.store, opts.hostedNotebookDrive.account, () => deps.clock.now()),
      clock: deps.clock,
    })
    : null;
  ...
  const budget = new ModelBudgetLedger({
    ...
    ...(opts.models?.operatorWindow ? { operatorWindow: opts.models.operatorWindow } : {}),
   ...
   * `issuedNotReturned` — the usage-accounting contract reads that field as "issued and presumed
   ...
   *
   * The hackathon claim this repository has been making — *agents that handle
   ...
   * more under the learner-controlled lineup contract, because this id is now the only real
   ...
   * and buying them a session they did not ask for is the waste the event-driven processing contract
   * exists to stop.
   */
  ...
  async function maybeAutoRun(): Promise<HostedStart | null> {
    ...
    /**
     * **The flag is taken at the check, not at the decision.**
     *
     * `building` used to go true five lines further down, after `getPrefs` and
     * `listPins` had both been awaited. Between the `if (building) return` and
     * the assignment sat two suspension points, and this function is called
     * from the pin path — the one place in the service where two requests
     * arriving inside the same second is the *normal* case, because that is
     * what pinning while browsing looks like. Both calls read `false`, both
     * planned, both set it, and the board ran two concurrent nights: the case
     * `batch-racing.test.ts` was written about, reached through a door that
     * test does not watch.
     *
     * Claiming at the check means claiming before knowing whether there is
     * anything to run, so every path that decides *not* to run has to give the
     * flag back — paused, nothing piled up, a store that threw, or a `runBatch`
     * that failed to start at all. The `finally` does it in one place rather
     * than four, and `handedToRun` is set only once `reportRun` has the promise,
     * because from that instant the flag belongs to its `.finally` instead.
     */
  ...
  /** Whether collection is paused right now (the collection-pause contract). */
   ...
   */
  /**
   * How many pins are actually waiting to be processed — the same fact
   * `planBatch` reads, by the same definition: no topic yet.
   *
   * This used to count every pin on a sessionless board, and dates-since-built
   * on the rest. The first production board caught the lie: a batch that filed
   * all three pins and then lost its compose stage to a provider 503 left a
   * card saying *"Being built — 3 things on your board, waiting for you to
   * process them"* about pins that had been processed an hour ago, beside a
   * `/batch` honestly answering "Nothing new to process". A filed pin is not
   * waiting for a run, whatever the session's date says — and a card that
   * disagrees with the button under it about the same number teaches the
   * learner to trust neither.
   */
  ...
  async function createPin(
    ...
    /**
     * The same pin, arriving twice.
     *
     * A pin is posted with the toast's patience rather than the network's, and
     * a post that runs over 2.5s is abandoned and queued. The drain then
     * retries a request the service may already have completed, and until now
     * that made a second pin: found on a real board as two `data` pins sharing
     * one `capturedAt` to the millisecond, which is one gesture, because that
     * stamp is taken once per gesture.
     *
     * So a repeat is answered with the pin it already made. Checked before the
     * Scout call, which is the expensive half: a retry costs one scan and no
     * model call at all.
     */
    if (body.clientRef) {
      const already = (await deps.store.listPins()).find((p) => p.clientRef === body.clientRef);
      if (already) {
        // Exact, both halves. The label used to be the one thing a retry
        // answered differently, because Scout's was not stored; it is now.
    ...
    if (suppliedLabel) {
      ...
      if (matched) {
        ...
        // SB-03: the label goes in the toast, so Scout's failure must never become
    ...
    const pin: Pin = {
      ...
      // SB-15: the field existed and was hardcoded false at the only place a
  ...
  /**
   * Resolve foreground context from the current board without waiting for a batch.
   *
   * Every foreground route used to read `pin.topicId`, which the Clusterer
   * writes overnight and nothing else writes at all. So a pin made four seconds
   * ago had no topic, no comfort, and `registerFor(undefined)` — which is
   * `from-nothing`. The fastest path in the product taught every learner as a
   * beginner, every time, however much history the board held on the subject,
   * and it did it by construction rather than by accident.
   *
   * One embed call answers it instead: the material and one document per topic,
   * in a single batch, compared by cosine. No model call, no stored vectors, no
   * migration, nothing that can go stale between the pin and the question.
   *
   * `MATCH_MATERIAL` is the same window the quick take reads, so the thing
   * being matched is the thing being taught.
   */
  ...
  /**
   * One foreground Tutor brief, from the same learner truth as Composer.
   *
   * Insights used to govern only the background session. The first action the
   * Insights screen offers after saving one leads to a pin's immediate Tutor,
   * where every route then said it knew nothing about the learner. Reusing the
   * pipeline's correction-first projection keeps one authority law across the
   * quick lesson, guide, stuck explanation, Ask and composed session.
   */
  ...
  /**
   * A topic for a pin the nightly has not clustered yet, so a tap has somewhere
   * to land (SB-61).
   *
   * Deliberately the cluster stage's own rescue shape rather than a new idea:
   * one topic, the pin in it, labelled from the page and summarised from the
   * material, `waiting` because nothing has been demonstrated about it. The
   * clusterer is attach-only, so it reads this as an existing topic on the next
   * run and attaches to it — a slightly redundant topic is recoverable through
   * the merge control, and a signal with nowhere to go is not recoverable at
   * all.
   *
   * **The board is asked first, and that retires the cost this used to carry.**
   * It used to go straight to a new topic, and the comment here recorded what
   * that bought: *"two taps on two related pins before the first nightly leave
   * two topics, because attach-only clustering never merges two topics that
   * already exist. That is the learner's to repair."*
   *
   * It was worse than untidy. The new topic is created at comfort 0.15, so
   * pressing *I am stuck* on a passage about a subject the learner is fluent in
   * made a beginner's topic beside their real one and filed the signal there —
   * the strongest negative evidence this product collects, recorded against a
   * topic that did not exist a second earlier and separated from the history it
   * belongs to. A ledger split in two measures neither half.
   *
   * So a match attaches, and only genuinely new material makes a topic. This is
   * not the Clusterer's decision being taken: this function already wrote
   * `topicId` onto the pin, and attaching to a topic the board already has is
   * strictly less of an intervention than inventing one beside it.
   */
  ...
  const handle: RequestListener = async (req, res) => {
    ...
    /**
     * The exposed-service authentication boundary's door, and it is in front of **every** route including
     * `/health`.
     *
    ...
    try {
      ...
      /**
       * SB-117: the learner's answer after trying the source link.
       *
      ...
      /**
       * SB-111/SB-115: inspect the original receipt and repair only what the
       ...
       * opening the take is not one, and it follows the same rule as SB-44's
       ...
       *
       * There is no pause check here, and that is the manual-capture pause exemption rather than an
      ...
      if (req.method === 'POST' && take) {
        ...
        // The foreground lesson uses the same hierarchy as a stored session:
        // course (when one really owns the topic), then topic, then the exact
        // lesson heading. The quick-take path used to flatten all three into
        // `label`, which left the learner looking at prose with no context.
        ...
        return json(res, 200, {
          ...
          subject,
          topicLabel,
          // What to call it on the screen. The topic's name once it has one,
          // and the page's own heading before that, the same fallback the
          // toast uses, so the take is headed what the pin was called.
          // The topic's name once it has one, what the Scout called this pin
          // before that, and the page's own heading only when there is
          // neither. The heading used to skip the middle one, so a take about
          // derivatives was titled "Prerequisites".
          label: topicLabel,
       ...
       *
       * This replaced a *Keep teaching* button that stepped the register down
       * on every press, so asking twice got you explained to as though you
       * were younger. The rule it was serving is mine rather than the
       * hackathon's, and it was being read as a ban on engagement; SB-30's own
       * walkthrough says "he asks in the panel".
       *
       * What survives of that rule is the part that is about the product: the
       * answer stays on their material, and a question that has become a
       * subject of its own is offered as a pin rather than answered as an
       * essay. The bound is on size, not on willingness: the passage, the
       * register, and the last few turns, so the tenth question costs what the
       * first did.
       *
      ...
      /**
       * SB-61 — *got it* / *still shaky*, the reason the quick take exists.
       *
       ...
       * exists to refuse, and dropping it makes SB-61 inert in exactly the case
       * it was written for.
       *
       ...
       * SB-283's third answer added no third mechanism: what each tap writes
      ...
      if (req.method === 'GET' && url.pathname === '/session') {
        ...
        /**
         * The learner-controlled lineup contract's `(i)`, for a session composed before there was one.
         *
        ...
        const session = safeStored
          ? {
            ...
            sections: actionableSections.map((sec) => ({
              ...
              // The affordance-first interface contract: the deadline this lesson moves forward, shown on
      ...
      if (req.method === 'GET' && url.pathname === '/board') {
        ...
        const pins = await deps.store.listPins();
        /**
         * Each topic says which AREA of the board it is on, and the service is
         * what says it.
         *
         * The panel used to group by `Topic.state`, which is the Registrar's
         * three-value answer for the agents. Two of the board's five areas are
         * not in it: **Paused** is `retiredByUser` (which `applyComfort` maps to
         * `settled`, so a topic the learner put down was filed under a heading
         * claiming they had learnt it) and **Recharging** is the spaced-review
         * rule, which needs the signal history. Neither is derivable from the
         * payload the panel had, and the review rule must not be copied into a
         * second place — so it is answered here, once, by `boardAreaFor`.
         *
         * The cost is one `listSignals` on a read that did not make one. Every
         * other endpoint on this screen already does.
         */
      ...
      /**
       * SB-31's second demand — "if enough time has passed that the earlier
       * material has gone cold, the resume opens with a two-line recap".
       *
       * **This used to be behind a flag, and the flag was about money.** It was
       * the only place in the product where OPENING a screen bought a model
       * call, so the machinery shipped switched off behind `SB_RESUME_RECAP=1`
       * and a real feature sat dark because of what it cost to look at.
       *
       * It costs nothing now. The Composer writes each section's recap line as
       * it writes the section — in a call already being paid for, with more
       * context than the recap call ever had — so coming back is assembly. The
       * flag has no argument left, and a switch kept after its reason has gone
       * is just a way for a feature to be off for nobody's benefit.
       *
       * `enabled` stays in the reply, always true, because the panel reads it
       * and an old panel must not read a missing field as "off".
       *
       * Read-only, like the provenance tap. Reading a reminder is not progress
       * through a session and must not be recorded as any.
       */
      ...
      /**
       * SB-38 — "Check my work", and the Reviewer's way in.
       *
       * `review()` began as written and tested but unreachable. The interactive-agent reachability contract wired
       ...
       * nothing**. SB-39's loop back to the board is one tap further on and is
       ...
       * checked, which is the same rule that keeps SB-44's provenance tap
       * signal-free.
       *
      ...
      if (req.method === 'POST' && url.pathname === '/review') {
        ...
        if (result.rewritesDropped > 0) {
          // The learner-work review boundary is the line this product does not cross, so a model
       ...
       * are parsed in code, verbatim, one row per line (The structured-criteria contract), and no
      ...
      /**
       * SB-30 — a lesson can be questioned without turning Virgil into an
      ...
      /**
       * SB-44 — "why am I seeing this?", which until now was copy and nothing
       * else.
       *
       * The Composer has checked a section's source ids against the ids its
       * brief offered since the story was written, and the panel has rendered
       * the COUNT of them with nothing behind the tap. A provenance count the
       * learner cannot open is precisely the claim this story exists to let
       * them check — the anti-hallucination surface, unopenable.
       *
       * Deliberately a GET, and deliberately the only tap on a section that
       * writes nothing. Every other control here puts a signal in the ledger;
       * checking a source is scepticism rather than performance, and a learner
       * who is scored for being sceptical stops being sceptical.
       *
       * An id that resolves to nothing is COUNTED, never guessed at. A source
       * line the learner cannot follow is the same failure as the dead
       * reference count the Composer's resolver was written to end.
       */
      ...
      /**
       * SB-45 — challenge a claim Virgil taught, not a mark Virgil gave.
       *
      ...
      /**
       * The learner-controlled lineup contract — the order the learner put tonight's lineup in.
       *
      ...
      if (req.method === 'POST' && m) {
        ...
        if (action === 'skip') {
          // SB-29: weaker than a demonstrated answer, and weighted accordingly.
        ...
        /**
         * The learner-controlled lineup contract — the X, which does two things and has to do both.
         *
         ...
         * the mark goes. SB-45's source-rechecking teaching correction is the
        ...
        /**
         * SB-62 — "come back to this", with its one nuance.
         *
         ...
         * a section that has no "answered" state to check (SB-62 keeps it open
        ...
        // SB-27: the strongest comfort signal available to the product.
      ...
      // SB-15/16: a suggestion is a proposal. Accepting promotes it to a pin;
      ...
      if (req.method === 'POST' && url.pathname === '/suggestions') {
        ...
        const created: Suggestion = {
          ...
          // The deliberate-capture precedence, and the thing that separates this from surveillance:
      ...
      if (req.method === 'POST' && sm) {
         ...
         * second rejection against the site, and SB-16 quiets the detector at
        ...
        if (verb === 'accept') {
          // "He clicks Add. It becomes a struggle pin." It used to become a
          // state field. The pin goes through the same path as one made by
          // hand, so it is enriched, clustered and taught exactly like the rest
          // — the only difference is that it says where it came from.
        ...
        // SB-16. The rejection has to reach the detector or it is just a card
      ...
      // --- topic identity repair. Attach-only clustering (D15) means a wrong
      ...
      if (req.method === 'POST' && (url.pathname === '/batch' || url.pathname === '/sessions/build')) {
        ...
        try {
          const prefs = await deps.store.getPrefs();
          if (pausedNow(prefs)) {
            // The collection-pause contract. A pause is a pause, and a button is not a way round it.
            return json(res, 409, { ok: false, error: 'collection is paused' });
          }
      ...
      if (url.pathname === '/notebook/export') {
        const destination = opts.notebook ?? managedNotebook;
        if (!destination) {
          return json(res, 404, { error: 'this service is not keeping documents for a notebook' });
        }
        if (req.method === 'POST') {
          const body = await readBody(req);
          const foreground = foregroundNotebookDocument(body.foreground);
          const receipt = foreground
            ? await exportForegroundNotebook(deps.store, deps.clock, destination, foreground)
            : await exportNotebook(deps.store, deps.clock, destination, notebookScope(body));
      ...
      if (req.method === 'POST' && connection) {
        ...
        const mode = connection[1] as ModelMode;
        const input = modelConnectionCheckEndpoint(mode, await readBody(req));
        if (input.error) throw new BadRequest(input.error);
        return json(res, 200, await checkModelConnection(mode, deps.store, models, input.endpoint));
      }
      ...
      if (url.pathname === '/model-config') {
        ...
        if (req.method === 'PUT') {
          if (models.readOnly) {
            throw new Forbidden('the Demo model configuration is managed by the service operator');
          }
          const body = await readBody(req);
          const prefs = await deps.store.getPrefs();
      ...
      if (req.method === 'POST' && url.pathname === '/commitments') {
        ...
        const commitment: Commitment = {
          ...
          // Checked rather than taken. It used to accept any non-empty string,
          // so a typo made a commitment that belonged to a course that was
          // never there — invisible until something tried to follow the link.
      ...
      if (commitmentRoute) {
        ...
        if (req.method === 'PUT') {
          ...
          const next: Commitment = {
            ...
            /**
             * The course, which this handler used to drop on the floor.
             *
             * `PUT` accepted the field, ignored it, and answered 200 with the
             * commitment unchanged — so a learner could send a perfectly good
             * course id and be told it had worked. Found while checking
             * The learner-controlled lineup contract's subject label against a real board: every
             * commitment made before intake existed carries `courseId: null`,
             * and there was no way to give it one, so the label could never
             * reach an old row.
             *
             * Silently ignoring a field the sibling `POST` accepts is the worst
             * of the three options: an error is a fact, doing the thing is a
             * fact, and this was neither. Same validation as creation now, with
             * `undefined` meaning "leave it" and an explicit `null` meaning
             * "clear it" — the idiom every other optional field on this handler
             * already uses.
             */
      ...
      */
       ...
       * with a course's name on it (SB-33).
       */
      if (req.method === 'GET' && url.pathname === '/courses') {
        /**
         * One read, because the room is drawn together.
         *
         * The study room used to ask for courses and show only courses, so a
         * deadline that belonged to a course lived in the Plan and a course
         * could not say what was coming. The join is additive — `progress` and
         * every existing field are untouched — and it costs one more store
         * read, not one per course: the same rule `GET /plan` was written
         * under, which is that three requests to draw one screen is how the
         * board ended up asking `/board` twice.
         */
        const now = deps.clock.now();
      ...
      // SB-42: editability turns a derived learner profile into a collaboration.
      ...
      // SB-43: deletion must reach derived state. A deleted pin that still shapes
      ...
      if (handleJudgeAccessRoute(req, res, url, {
        access: opts.access ?? null, learner: opts.learner ?? null,
        hostedNeedsRun, hostedRunAvailable: hostedRun !== null, reply: json })) return;
      /**
      ...
      if (req.method === 'POST' && (url.pathname === '/account/restore/preview'
        ...
        if (opts.judgeSession) {
          throw new Forbidden('backup restore is unavailable on the shared Demo board');
        }
        const body = await readBody(req);
      ...
      if (req.method === 'DELETE' && url.pathname === '/everything') {
        if (opts.judgeSession) {
          throw new Forbidden('permanent account deletion is unavailable on the shared Demo board');
        }
      ...
      // A body the client got wrong is not a fault, so it is answered plainly
      // and is not written to the error log. It used to arrive here as a
      // TypeError or a SyntaxError and leave as a 500, which said "this service
      // is broken" about a request that was simply malformed.
...
/**
 * The store this process was pointed at, built from the spec and not from a
 * path.
 *
 * One of the two places in the tree that may construct an adapter, which is why
 * this is duplicated in `cli.ts` rather than shared: `seam-purity.test.ts` holds
 * that `new JsonStore(...)` appears in exactly these two composition roots, and
 * a shared helper would be a third. The guard is not edited to make room for
 * one — the duplication is the shape it is enforcing, and it is eight lines.
 *
 * Firestore is reached by dynamic import and declared as a dependency nowhere,
 * which is the orchestration dependency boundary applied to the store the same way the ADK binding applies
 * it to the framework: the vendor arrives in the commit that means it. Until the
 * Firestore lane merges, `firestore:` is a spec this build understands and
 * cannot serve, and it says exactly that rather than falling back to a disk.
 *
 * `wiring` is settled by the caller, before the listener binds, and is not
 * derived here. The authorisation to leave the emulator has to be refused while
 * a failure to start is still the answer: the adapter's own refusal is correct
 * and arrives on first access, which for a service is the first learner request
 * against an instance the platform has already reported healthy.
 *
 * It is required rather than defaulted, and `null` is how a non-firestore spec
 * says so. A default would let a future caller reach this with the question
 * unasked, which is the exact shape of the defect being closed.
 */
export async function openStore(
  ...
  /**
   * The adapter, named directly. The branch that used to stand here — "this
   * build has no Firestore store", read off a barrel that did not export it — is
   * gone rather than unreached; see `cli.ts` for what it cost.
   *
   * `wiring` is still checked, and it is a different question with a different
   * answer: this function is exported and takes the wiring as a parameter, so a
   * caller can hand it a firestore choice and no wiring. That is a programming
   * error rather than a deployment one, and it fails closed rather than opening
   * a client against the adapter's `virgil-emulator` default.
   */
  if (!wiring) {
...
export async function startService(
  ...
  const web: BoardWebOptions = {
    ...
    googleWebClientId: env.SB_GOOGLE_WEB_CLIENT_ID?.trim() || env.SB_GOOGLE_OAUTH_CLIENT_ID?.trim() || null,
    judgeDemoEnabled: Boolean(env.VIRGIL_JUDGE_ACCESS_CODE_SHA256?.trim()),
  };
  ...
  // Foreground and background work resolve the same configured model route.
  ...
  /**
   * The cloud connection and its free-first key ladder.
   *
  ...
  const app = createApp(deps, {
    ...
    ...(managedDrive ? { hostedNotebookDrive: managedDrive } : {}),
