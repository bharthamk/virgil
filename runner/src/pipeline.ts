...
export interface StageReport {
  ...
  /**
   * Provider failure classification. Null means the stage succeeded or the
   * provider supplied no safe classification. `exhausted` distinguishes work
   * that was not attempted from an honest empty result.
   */
 ...
 * excluded because an edit outranks a derived read ; a rejection is
 * excluded because it is an invisible evidence receipt rather than a sentence.
 *
 *  adds the third. A modality row is not this stage's prose: it is one
...
export async function runBatch(
  ...
  note(await execute('intake', async () => {
    ...
    for (const draft of taken) {
       ...
       * exact failure  forbids for statements, arriving at the drafts.
       *
  ...
  note(await execute('forage', async () => {
    ...
    await mapLimit(pins, opts.concurrency ?? 3, async (pin) => {
      try {
        ...
        // resurrected with an enrichment attached.  says delete means
  ...
  // (clustering-stability constraint). A run over an unchanged board is therefore a no-op:
  ...
  // 5b. The prose the learner reads and edits. Best-effort: losing it
  ...
  note(await execute('statements', async () => {
    ...
    // , after the prose and never before it: a classification that fails
    ...
    //
    // User-edited lines survive:  says an edit outranks derived state
  ...
  note(await execute('compose', async () => {
    ...
    // on a night with nothing new to teach and something worth
    ...
    // "Nothing ready yet" on exactly the night the product contract promises an offer.
    ...
    if (session.outcome === 'nothing-to-teach') {
      return 'nothing to teach and nothing to revise — honest empty state ()';
    }
    // The third state (the honest-empty-state contract). Topics were chosen and the model was
  ...
  // had emptied through both stages (the honest-empty-state contract).
  ...
  note(await execute('verify', async () => {
    ...
    /**
     * The withheld-content contract — the note is rewritten to the session that actually shipped.
     *
     ...
     * model call on this path, which is the contract.
     *
  ...
  if (session && (session as ComposedSession).outcome === 'composed') {
    ...
    if (learnerContextChanged) {
      ...
      await deps.store.putSession({
        ...
        // The batch-key alignment contract. `builtAt` is when this finished; this is which night it was
   ...
   * `notebook-export.ts`, where the contract is written out in full. What is left
