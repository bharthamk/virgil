 ...
 * execution.
 */
import { isModelBudgetLimit } from '@sb/core';
...
export interface HostedRunRequest {
  ...
  /** Optional service-owned restrictions that the launched worker must inherit.
   * Learner input never reaches this object. */
  readonly policy?: HostedRunPolicy;
}

export interface HostedRunPolicy {
  /** A deployment ceiling shared through the board's durable spend ledger. */
  readonly operatorLimit: number;
  /** Demo work is bounded by a UTC day, not the worker process lifetime. */
  readonly operatorWindow: 'day';
  /** A Demo worker must not inherit the Job template's personal Drive grant. */
  readonly notebookExport: 'disabled';
}
...
export class CloudRunJobLauncher implements HostedRunLauncher {
  ...
  async launch(request: HostedRunRequest): Promise<HostedRunLaunch> {
    ...
    if (request.policy && (
      !isModelBudgetLimit(request.policy.operatorLimit)
      || request.policy.operatorWindow !== 'day'
      || request.policy.notebookExport !== 'disabled'
    )) {
      throw new Error('automatic run policy is invalid');
    }
    const args = [
      'runner/dist/cli.js', 'process',
      ...(request.policy ? [
        `--operator-limit=${request.policy.operatorLimit}`,
        `--operator-window=${request.policy.operatorWindow}`,
        '--disable-notebook-export',
      ] : []),
      ...(request.asked ? [] : ['--if-due']),
    ];
    const body = {
      overrides: {
        containerOverrides: [{
