...
try {
  ...
  managedNotebookDrive = managedDriveGrant(process.env.SB_NOTEBOOK_DRIVE_CREDENTIAL);
} catch (err) {
 ...
 * default on 2026-08-20 (the partition-strategy contract). `SB_PARTITION=single` is the way back to
...
const budgetLedger = new ModelBudgetLedger({
  ...
  clock: systemClock,
  defaultMode,
  ...(operatorModelBudgetFrom(process.env.SB_OPERATOR_MODEL_BUDGET_TOKENS) != null
    ? { operatorLimit: operatorModelBudgetFrom(process.env.SB_OPERATOR_MODEL_BUDGET_TOKENS)! } : {}),
...
if (cmd === 'seed') {
   ...
   *
   * **The framework-boundary contract’s commit.** `SB_ORCHESTRATOR` picks the host and `deploy/job.yaml`
  ...
  // reads that (the honest-empty-state contract).
