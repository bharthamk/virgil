...
/**
 * THE GRANT ARRIVED AND THE DISK SAID NO.
 *
 * The consent chain is `void`ed — the HTTP request answered with the URL long
 * ago and nothing is waiting on the rest — and `await credential.setToken(...)`
 * used to sit *outside* its try. `setToken` writes a file: it goes through
 * `writePrivate`, which refuses a symlinked path and re-chmods to `0600`, and
 * it fails for every ordinary reason a write fails. A throw there was therefore
 * not a failed connection but an **unhandled rejection**, and Node's default
 * answer to one of those is to kill the process. The learner's whole service —
 * the board, the pins, the nightly — gone because one file could not be
 * written, on a code path that runs while nobody is looking at it.
 *
 * The debris was the other half. `pendingConsent` was never cleared, so the
 * next Connect cancelled a listener nobody was waiting on, and `connectPhase`
 * stayed `'waiting'`, so the setup screen sat there quoting a browser tab that
 * had closed minutes earlier. Nothing anywhere said a word about a disk.
 *
 * This test is deliberately not about EACCES. It is about *any* throw from
 * storage arriving on a promise nobody awaits, and it asserts the three things
 * that were wrong: the process is still here, the phase is honest, and the lane
 * can be connected afterwards by a learner who fixed their permissions.
 */
