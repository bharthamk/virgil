...
test('the take comes back in the register the ledger already reads', async (t) => {
...
test('a pin the nightly has not seen yet is taught from nothing', async (t) => {
...
test('learner-authored Insights govern every foreground Tutor route the product offers next', async (t) => {
...
test('reading a take writes nothing at all', async (t) => {
  // The same rule as  provenance tap, and for the same reason. The taps
...
test('a take that could not be written says so, and is not a 500', async (t) => {
...
test('a fatal source-check finding returns no lesson and writes no learner signal', async (t) => {
...
test('still shaky lands in the ledger on the pin\'s own topic', async (t) => {
...
test('got it lands as the positive read it is', async (t) => {
...
/**
 *  third answer, and the whole reason it needed no new machinery.
 *
 ...
 */
test('not now lands as the lineup mark it already is, with the take as its source', async (t) => {
...
test('the two readings carry no window, because they hold nothing back', async (t) => {
...
test('changing the answer to not now withdraws the reading it replaces', async (t) => {
  // The correction path  built, over the vocabulary  widened. One
...
test('a tap on a pin with no topic yet gives it one, named from the page', async (t) => {
   ...
   * the one-ledger law exists to refuse, and dropping it would make  inert
...
test('material the board knows nothing about still gets a topic of its own', async (t) => {
  ...
  // minting path is what  needs for genuinely new material.
...
test('the learner-action contract: a pause stops the watching, and never the asking', async (t) => {
  /**
   * The learner-confirmation contract, confirmed as the learner-action contract: pause governs what is *watched*, not
   ...
   * solicit anything. The processing-pause contract’s rationale points the same way — a withheld
...
test('a still-shaky tap reaches zone 3 and this run\'s reason, with no second store', async (t) => {
...
/**
 *  — THE ONE MINUTE HERO'S TWO REFUSALS, THROUGH THE DOORS THEY USE.
 *
 ...
 */
test('not now before the take is read holds the topic exactly as it does after', async (t) => {
...
test('a passed over pick is held out of the read and nothing is written', async (t) => {
  ...
  // Nothing about how somebody learns, either:  tallies are built from
...
test('an unrecognised refusal changes nothing about the answer', async (t) => {
