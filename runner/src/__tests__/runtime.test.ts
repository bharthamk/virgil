...
// --- who may knock on the door (the exposed-service authentication boundary) ----------------------------------
...
test('a service that binds every interface must have one, or it does not start', () => {
  /**
   * The exposed-service authentication boundary, and the reason it is keyed on the **bind** rather than on
   * `K_SERVICE`.
   *
...
test('a night that honestly produced nothing is a processed night, not a failed one', () => {
  // The three-state batch-result and verifier-withholding contracts, and the same judgement the Pub/Sub lane's ack policy
...
test('a quota-degraded night is processed, because retrying a spent daily cap is the storm the quota-degradation contract forbids', () => {
  // GEMINI_TRANSPORT_PROOF_2026-08-20.md §9 the quota-degradation contract: a daily cap degrades
