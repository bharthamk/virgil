...
/**
 * The deploy configuration is production infrastructure, so its facts are checked
 * like code.
 *
...
test('the job runs the command that does not delete the board', () => {
  ...
  // CLOUD_RUN.md R2.
  // `--if-due` rides along, and what it MEANS changed with the manual-processing contract: it used
  ...
  // is the money this contract exists to stop being spent.
...
test('both base images are pinned by digest rather than by a tag that moves', () => {
  ...
  for (const base of bases) {
    assert.match(base, /@sha256:[0-9a-f]{64}$/,
      `${base} is pinned by tag, so deployment may not get the image this was tested against`);
  }
});
...
test('the image build can see every workspace, or `tsc -b` cannot build any of them', () => {
   ...
   * fails, and it fails in the ordinary suite rather than on deployment.
   */
...
test('the board is closed to every client, and the estate is told to say so', () => {
  ...
  assert.match(cloudRun, /firestore\.rules/,
    'CLOUD_RUN.md does not name the rules file, so deployment would never deploy it and the '
  ...
  for (const [name, text] of [['CLOUD_RUN.md', cloudRun], ['firestore.rules', rules]] as const) {
    assert.ok(!deadLine.test(text),
      ...
      + 'does not exist, and following it means an unrecognised-argument error at step 1 of deployment');
  }
...
test('no deploy file carries key material, and none of them can print a secret', () => {
  ...
  // A scan over nothing passes. `deploy/` is the directory deployment is run
...
test('the job names the host that runs the night, and the code can read the name', () => {
  /**
   * The framework-boundary contract’s trigger, as a line somebody can point at.
   *
  ...
  assert.ok(named, 'job.yaml names no SB_ORCHESTRATOR — the deployed night would run framework-free '
    + 'while the documentation claims otherwise, and nothing would say so');
...
test('a fresh project receives the runtime identity, Firestore database and bounded execution roles', () => {
  ...
  assert.match(apply, /for role in roles\/datastore\.user roles\/aiplatform\.user; do[\s\S]*gcloud projects add-iam-policy-binding "\$PROJECT_ID"[\s\S]*--role "\$role"/,
    'the runtime does not receive the exact Firestore and Vertex execution role pair');
...
test('every script that could bill is behind the confirmation gate', () => {
  // Billable operations require an explicit operator opt-in.
