...
test('rejecting the same suggestion twice does not quiet the site twice over', async (t) => {
  //  quiets at two rejections. If one card can be counted repeatedly, one
...
test('a suggestion whose url has no real origin does not share a quieting bucket', async (t) => {
  ...
  // Two rejections on two unrelated pages, and the  threshold is two: a
...
test('a real origin is counted under its origin, and nothing else', async (t) => {
  // The half of the fix that must not have moved:  is keyed on the origin
...
test('a prefs PUT cannot replace a field with a value of the wrong type', async (t) => {
  ...
  // and the  counts. `PUT /prefs` spreads the body over the stored prefs
  ...
  // wrote — an  quieting a learner never asked for is a detector going
