...
/**
 * The secret this exposed single-board fixture must have — the service-protection contract.
 *
...
test('a warm-up is visible without becoming something the learner pressed', async () => {
...
test('a composed session the verifier emptied is not a session', () => {
  ...
  // nothing. The delivery-safety contract’s distinction, read at the exit code.
  assert.deepEqual(
    outcomeOf({ session: { outcome: 'composed', sections: [] } }),
