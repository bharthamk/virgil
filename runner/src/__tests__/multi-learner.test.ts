...
test('Demo keeps bounded Grow while operator-owned and destructive controls stay read-only', async (t) => {
  const password = 'Virgil-demo-only-2026-cobalt';
  const worker = new RunLauncherStub();
  const access = learnerAccessPolicy({
    ownerEmail: 'alice@example.com', allowedEmails: ['alice@example.com'], requestsPerMinute: 120,
    judgePassHash: createHash('sha256').update(password).digest('hex'),
    judgeDailyCloudTokens: 500_000,
  });
  const r = await rig({ access, hosted: true, hostedRun: worker });
  t.after(() => r.close());

  assert.equal((await r.call(null, 'POST', '/judge/login', { pass: 'wrong-password-that-is-long-enough' })).status, 403);
  const login = await r.call(null, 'POST', '/judge/login', { pass: password });
  assert.equal(login.status, 200);
  assert.equal(login.body.uid, 'judge-workspace-v1');
  assert.equal(login.body.personalConnections, false);
  assert.equal(typeof login.body.token, 'string');

  const token = login.body.token as string;
  await r.call(token, 'POST', '/pins', capture('the shared demo pinned this'));
  assert.equal((await r.call(login.body.token, 'GET', '/board')).status, 200);
  const judge = await r.call(token, 'GET', '/judge/access');
  assert.equal(judge.body.dailyCloudTokens, 500_000);
  assert.deepEqual(judge.body.capabilities, {
    grow: { available: true, mode: 'cloud-run-job' },
    personalNotebook: false,
    personalDrive: false,
  });
  assert.deepEqual(judge.body.readOnly, {
    modelConfiguration: true,
    modelBudget: true,
    restore: true,
    permanentDelete: true,
  });
  assert.equal((await r.call(login.body.token, 'GET', '/notebook/drive')).status, 404);

  const modelConfig = await r.call(token, 'GET', '/model-config');
  assert.equal(modelConfig.status, 200);
  assert.equal(modelConfig.body.readOnly, true);
  assert.equal(modelConfig.body.providers.cloud.setup.editable, false);
  assert.equal(modelConfig.body.providers.local.setup.editable, false);
  assert.equal(modelConfig.body.providers.cli.setup.editable, false);
  const budget = await r.call(token, 'GET', '/model-budget');
  assert.equal(budget.status, 200);
  assert.equal(budget.body.readOnly, true);
  assert.equal(budget.body.operatorLimit, 500_000);
  assert.equal(budget.body.operatorWindow, 'day');

  const forbidden: readonly [string, string, unknown?][] = [
    ['PUT', '/model-config', { preset: 'recommended' }],
    ['POST', '/model-connections/cloud/check', {}],
    ['POST', '/model-connections/local/check', {}],
    ['POST', '/model-connections/cli/check', {}],
    ['PUT', '/model-connections/cloud/credential', { apiKey: 'judge-cannot-change-this' }],
    ['DELETE', '/model-connections/cloud/credential'],
    ['POST', '/local-connector/pair'],
    ['DELETE', '/local-connector/pair'],
    ['PUT', '/model-budget', { limit: 10_000, unit: 'tokens' }],
    ['DELETE', '/model-budget'],
    ['POST', '/model-budget/reset'],
    ['POST', '/account/restore/preview', {}],
    ['POST', '/account/restore', {}],
    ['DELETE', '/everything'],
  ];
  for (const [method, path, body] of forbidden) {
    const refusal = await r.call(token, method, path, body);
    assert.equal(refusal.status, 403, `${method} ${path} was not read-only`);
  }

  const grow = await r.call(token, 'POST', '/batch');
  assert.equal(grow.status, 200);
  assert.equal(grow.body.queued, true);
  assert.equal(worker.launches.length, 1);
  assert.deepEqual(worker.launches[0]!.policy, {
    operatorLimit: 500_000,
    operatorWindow: 'day',
    notebookExport: 'disabled',
  });
  assert.equal(worker.launches[0]!.boardId, 'learner-judge-workspace-v1');
  assert.equal((await r.boards.get('judge-workspace-v1')!.listPins()).length, 1);
  assert.equal((await r.boards.get('judge-workspace-v1')!.getPrefs()).modelBudget, undefined);
  assert.equal(r.boards.has('aliceUid'), false, 'Demo mode opened the owner board');
});
...
test('health answers without a token, and says nothing about anybody', async (t) => {
  // It is the one route a platform probe may need, and it is also the route
  // that used to report a pin count — which is a learner's data. It reports
  // that the process is up and nothing else now.
  const r = await rig();
  t.after(() => r.close());
