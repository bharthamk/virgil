...
test('a request that was issued and did not come back is counted, not lost', async (t) => {
   ...
   *
   * That is not a tidiness point. **The quota-accounting contract as amended presumes every issued
   * request billed** — there is no unbilled-503 category on this provider — and
   * the correction exists because a run took 17 × 503, retried 3 more, and hit
...
test('a genuine provider failure is still on the bill', async () => {
  // The property the exclusion must not cost. The correction exists because
...
test('the runs lane records a failing provider too, and it did not', async () => {
   ...
   * `meterLlmAs` grew the `issuedNotReturned` catch when the quota-accounting contract was amended
   ...
   * nobody watching and whose own 503 storm is the incident the correction was
   * written for.
   */
  const meter = new UsageMeter();
  meter.enter('compose');
