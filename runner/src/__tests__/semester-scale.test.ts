...
/**
 * The scripted model, taught the one schema the nightly did not used to ask.
 *
 * The `intake` stage reaches the intake specialist, whose reply is
 * `{objectives, commitments, questions}` — a shape `stageOf` in the shared
 * harness has never had to recognise, because until this stage existed nothing
 * in a run asked for it. Extending the shared stub instead would make every
 * other suite carry a stage they do not run.
 */
...
test('the naming pass is chunked, so a semester of new topics is never one enormous call', async () => {
  // Every document its own topic, which is the worst case for the naming call
  // and the one that used to ask a fast-tier model for thousands of output
  // tokens in a single reply and get a truncated one back.
