...
test('Today opens the learning-link choice when an unlinked deadline has board topics', async () => {
  ...
  try {
    ...
    const today = await h.call('GET', '/today?minutes=3');
    assert.equal(today.body.next.primary.destination, 'plan');
    assert.equal(today.body.next.primary.cta, 'Choose learning links');
    assert.equal(today.body.next.primary.planIntent, 'links');
  } finally { await h.close(); }
});

/**
 * The assessed result, on the move it is about.
 *
 * This used to assert that a bad rubric result flipped the next move from the
 * assignment to the lesson. The session leads whenever there is one, so the flip is no
 * longer something a result can cause, and asserting it would be asserting the
 * old defect. What a result still does, and what actually reaches the learner,
 * is name itself in the reasons under the move, and reach the signal ledger
 * where it changes what the next session is built from.
 */
test('a negative assessed outcome names itself on the next move', async () => {
  ...
  try {
    ...
    // the lesson captions itself with nothing. The affordance-first interface contract took the standing
...
test('SB-253: an assignment result carries its owning course and refuses a contradictory one', async () => {
...
test('SB-253: result correction persists offered relationships and keeps hidden evidence fields', async () => {
