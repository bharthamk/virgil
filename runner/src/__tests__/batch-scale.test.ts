...
test('the cold-run cost is linear in new pins and new topics, not quadratic in either', async () => {
  ...
  /**
   * Naming is linear in NEW TOPICS, and it is meant to be.
   *
   * This assertion used to read "one naming call for thirty new topics", and
   * that was the bug rather than the property: one call asking for thirty labels
   * requests 3,800 output tokens and comes back truncated, which lands as
   * heading-path fallback labels on a board nobody looks at twice. The pass is
   * chunked now, so fifteen new topics cost two calls and thirty cost three.
   *
   * What still has to hold is the shape: the naming cost tracks the number of
   * **new topics**, not the number of pins, so doubling a board of the same
   * topic density doubles it and doubling the pins inside fixed topics does not.
   */
