...
/**
 * A BUDGET STOP ENDS THE RUN — it does not degrade nine stages in a row.
 *
 * The pipeline is failure-tolerant by stage, deliberately and for good reason:
 * one truncated Analyst reply used to abort a whole night, and a session built
 * without observations is worth far more than no session. That argument is
 * about a bad night. A stop is not a bad night — nothing was sent, nothing will
 * be sent, and every stage after the first one would go and rediscover the same
 * refusal. The run that came back said "six stages degraded" and named the
 * cause nowhere.
 *
 * `cli.ts` has printed `batch-outcome budget-stopped` at its top-level result
 * catch since the switch shipped, and could not reach it: the run it awaited
 * always resolved, carrying failed stage reports that look exactly like a
 * provider outage. What this file holds is the propagation that makes that
 * branch live, in both lanes — the plain pipeline, which stops where it stands,
 * and the ADK-hosted one, where the framework owns the sequence and the run has
 * to be failed at the end rather than abandoned in the middle.
 */
