...
/**
 *  — THE GAP THAT COULD NEVER FIRE, FIRING.
 *
 ...
 * `Statement.topicId`, and  found that field was written null on every
...
test('a sentence about one topic carries it, so the comfort-gated gap can fire', async () => {
...
test('a sentence about the whole board still carries no topic, and no gap is doubled', async () => {
