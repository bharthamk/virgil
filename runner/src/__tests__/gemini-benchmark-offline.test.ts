...
describe('stub run with a provider outage injected', () => {
  ...
  test('the ledger separates the attempts made from what was presumed billed', () => {
    // The usage-accounting contract's accounting clause: a 503 with no exhaustion signal never
...
/**
 * The usage-accounting contract's second clause, end to end: a stage that failed did not save.
 *
