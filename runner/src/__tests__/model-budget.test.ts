...
test('the worker operator window accepts only the two ledger windows', () => {
  assert.equal(operatorModelBudgetWindowFrom(undefined), undefined);
  assert.equal(operatorModelBudgetWindowFrom(' day '), 'day');
  assert.equal(operatorModelBudgetWindowFrom('total'), 'total');
  assert.throws(() => operatorModelBudgetWindowFrom('week'), /must be total or day/);
});
...
test('a Demo-mode operator ceiling resets Cloud/API at UTC midnight and preserves free connections', async () => {
  let instant = new Date('2026-08-19T23:59:00.000Z');
  const movingClock = { now: () => new Date(instant) };
  const store = new PrefsStore({
    ...PREFS,
    modelSpend: {
      since: '2026-08-19T08:00:00.000Z',
      connections: {
        cloud: { calls: 2, inputTokens: 450_000, outputTokens: 50_000, issuedNotReturned: 0 },
        local: { calls: 1, inputTokens: 700, outputTokens: 300, issuedNotReturned: 0 },
        cli: { calls: 0, inputTokens: 0, outputTokens: 0, issuedNotReturned: 0 },
      },
    },
  });
  const first = new ModelBudgetLedger({
    store, clock: movingClock, operatorLimit: 500_000, operatorWindow: 'day',
  });
  assert.equal((await first.receipt()).state.status, 'exhausted');
  assert.equal((await first.receipt()).state.window, 'day');

  instant = new Date('2026-08-20T00:00:01.000Z');
  const after = await first.receipt();
  assert.equal(after.state.status, 'ok');
  assert.equal(after.state.used, 0);
  assert.equal(after.spend.connections.local.inputTokens, 700);
  assert.equal(after.spend.connections.local.outputTokens, 300);
  assert.equal(after.spend.since, '2026-08-20T00:00:01.000Z');
});
...
test('a budget refusal is NOT recorded as an issued request', async () => {
  // The distinction the whole ordering exists to protect. The usage-accounting contract reads
...
test('a refusal thrown from INSIDE the call is not recorded as an issued request either', async () => {
  /**
   * The half of the ordering the ordering could not protect. `budgetedLlm`
   * refuses before the `try` and that has always been safe — but the router
   * underneath it throws `LlmCredentialMissing` when the connection has no key
   * saved, and since the free-arm ruling the key ladder throws the deferred
   * `ModelBudgetStop` at the paid switch. Both land in the `catch`, where every
   * throw used to be written into the learner's own window as
   * `issuedNotReturned` — the field the usage-accounting contract reads as "issued and presumed
   * billed". A learner who had simply never pasted a key watched their budget
   * fill up with calls that were never built.
   */
  const store = new PrefsStore({ ...PREFS, modelBudget: budgeted(1_000) });
  const refusing: Llm = {
