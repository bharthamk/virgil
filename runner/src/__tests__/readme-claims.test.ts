...
/**
 * The README and its linked deep dive are judged artefacts, so their facts are
 * checked like code.
 *
 ...
 * owns. Each test reads the concise README plus the technical document it
 * routes to, pulls out what they claim, and compares it to the source of that
 * claim — so the failure message names the drift rather than a magic constant
...
const deepDive = read('docs/DEEP_DIVE.md');
const publicDocs = `${readme}\n${deepDive}`;

/** Fenced shell blocks — where the public docs tell a stranger what to run. */
const shellBlocks = [...publicDocs.matchAll(/```bash\n([\s\S]*?)```/g)].map((m) => m[1]!);
const shell = shellBlocks.join('\n');

test('the concise README routes technical reviewers to the checked deep dive', () => {
  assert.match(readme, /\]\(docs\/DEEP_DIVE\.md\)/,
    'the README no longer links the technical document that owns its moved contracts');
});
...
test('every SB_ environment variable the public docs document is read by the code', () => {
  const documented = new Set([...publicDocs.matchAll(/\bSB_[A-Z_]+/g)].map((m) => m[0]));
  assert.ok(documented.size >= 3, 'the public docs stopped naming environment variables, so this test stopped reading them');

  const sources = ENV_SOURCES.map(read).join('\n');
  for (const name of documented) {
    assert.ok(readsEnv(sources, name),
      `the public docs document ${name} and nothing reads it — either the code dropped it or the docs invented it`);
  }
});

test('every SB_ environment variable the code reads is documented in the public docs', () => {
  ...
  for (const name of read_) {
    assert.ok(publicDocs.includes(name),
      `${name} changes how a run behaves and the public docs do not mention it — a reader cannot find it without the source`);
  }
});
...
test('the platform variables Cloud Run injects are documented too', () => {
  ...
  for (const name of ['PORT', 'K_SERVICE']) {
    ...
    assert.ok(publicDocs.includes(name),
      `${name} decides whether a deployed container serves anything and the public docs do not mention it`);
  }
});

test('the port and store path the public docs quote are the ones the service defaults to', () => {
  ...
  assert.ok(publicDocs.includes(port), `the service defaults to port ${port} and the public docs say something else`);
  ...
  assert.ok(publicDocs.includes(db), `the service defaults to the store at ${db} and the public docs say something else`);
});

test('public-doc seed and process examples leave the included judge board untouched', () => {
  const seed = publicDocs.match(/^(\S*SB_DB=\S+)\s+node runner\/dist\/cli\.js seed$/m)?.[1];
  const process = publicDocs.match(/^(\S*SB_DB=\S+)\s+node runner\/dist\/cli\.js process$/m)?.[1];
...
test('every npm script the public docs tell a stranger to run exists', () => {
  const pkg = JSON.parse(read('package.json')) as { scripts?: Record<string, string> };
  const scripts = Object.keys(pkg.scripts ?? {});
  const named = [...publicDocs.matchAll(/\bnpm run ([a-z][\w:]*)/g)].map((m) => m[1]!);
...
test('every script path the public docs point at exists', () => {
  const paths = new Set([...publicDocs.matchAll(/`(scripts\/[\w-]+\.mjs)`/g)].map((m) => m[1]!));
  for (const path of paths) {
    assert.ok(existsSync(at(path)), `the README points at ${path} and there is no such file`);
  }
});

test('the models the public docs say to pull are the models the adapters ask for', () => {
...
test('the public docs name every agent in the tree, and no agent that is not', () => {
...
test('current public sources cannot fall back to the pre-A+ architecture story', () => {
  ...
  const current = [publicDocs, read('adk/src/stages.ts')];
  ...
  for (const stale of [
    ...
    // 13 was true until Marker and Transcriber landed; the release build caught
    // the regex below still asking for it.
  ...
  // The public tree must carry the fleet count itself so release validation
  // never depends on an operator's surrounding filesystem.
  ...
  assert.match(publicDocs, new RegExp(`\\b${['zero', 'one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine', 'ten', 'eleven'][stageNames.length] ?? String(stageNames.length)}[ -]stage`, 'i'),
  ...
  assert.match(publicDocs, new RegExp(`\\b${countWord} learner-facing documents\\b`, 'i'),
    'the README does not state the executable Notebook document count');
  assert.doesNotMatch(publicDocs, /\bfive learner-facing documents\b|\bsame five documents\b/i,
