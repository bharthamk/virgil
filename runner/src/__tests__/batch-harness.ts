...
export function stageOf(req: LlmRequest & { schema?: unknown }): Stage | null {
  ...
  // SB-282's classification. Inside the statements stage in the pipeline, but
      ...
      case 'compose': {
        ...
        return {
          sections: ids.map((id) => ({
            ...
            sourceIds: [],
            mediumWarning: null,
            // The persisted batch-identity contract's two registers, required by the composer schema
