...
test('SB-198: notebook input cannot re-export a claim Tutor conceded', async (t) => {
...
test('SB-206: notebook input withholds a historical source-boundary assessment', async (t) => {
 ...
 * board as it stands now. Under the learner-controlled lineup contract, a session also carries each
...
test('a foreground export writes the exact visible lesson and refreshes all three stable sources', async (t) => {
  const directory = where('foreground-exact');
  const h = await startService('nb-foreground-exact', {}, {
    notebook: localTo(directory), hostedNotebookUrl: LIVE_NOTEBOOK,
  });
  t.after(() => h.close());

  const foreground = {
    key: 'learn-now', title: 'Virgil: learn now',
    body: '# Virgil: learn now\n\n## The lesson in front of you\n\n### Reconstruction\n\nExact visible lesson.\n',
  };
  const pushed = await h.call('POST', '/notebook/export', { foreground });
  assert.equal(pushed.status, 200);
  assert.equal(pushed.body.ok, true);
  assert.equal(pushed.body.notebookUrl, LIVE_NOTEBOOK);
  assert.deepEqual((pushed.body.docs as { key: string }[]).map((d) => d.key).sort(),
    ['archive', 'learn-now', 'on-the-board']);
  assert.match(readFileSync(join(directory, 'virgil-learn-now.md'), 'utf8'), /Exact visible lesson/);
});
