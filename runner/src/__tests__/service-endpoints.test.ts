...
/**
 * The rest of the service's surface, at the level of "this endpoint is wired to
 * the thing it says it is".
 *
 * Deliberately not a second copy of the agent and store tests: the happy path,
 * the signal each interaction is supposed to leave behind, and one malformed
 * request per endpoint. The malformed cases are the point of the file.
 *
 * They used to land on the generic 500 and this file asserted that, on the
 * grounds that it was the honest current shape and the cause at least reached
 * the log. It was honest and it was wrong: a 500 says "this service is broken,
 * go and read the log", and an unparseable body or a missing field is a fact
 * about one request that nobody needs to diagnose later. Those cases are 400s
 * with a plain message now, and they stay off the error log. The 500 — and its
 * log line — is still asserted here, for faults that genuinely are ours.
 */

test('SB-157: a declared oversized body is refused before it can be buffered', async (t) => {
...
test('SB-157: the widest valid page request fits beneath the aggregate body ceiling', () => {
...
test('Scout being down is not the learner\'s problem, but it is written down', async (t) => {
  // SB-03: the pin still lands, with the fallback label. The failure used to be
  // invisible — a model down for a week looked exactly like one that worked.
...
test('the session endpoint returns null before there is one, and the session after', async (t) => {
  ...
  // also says why, which is the half SB-23 always asked for.
...
test('SB-117: a learner-confirmed link check changes only its receipt and never fetches or rewrites evidence', async (t) => {
  let fetches = 0;
 ...
 *
 * SB-44 is the anti-hallucination surface: every taught claim traces to a user
 ...
 * offered since the story was written, and the panel has rendered the COUNT of
...
test('SB-44: a section says where each of its claims came from, and which are not the learner\'s', async (t) => {
...
test('SB-10: a pinned video resolves to a link that opens at the moment it was pinned', async (t) => {
...
test('SB-10: a moment on a site with no convention for one is stated and not linked', async (t) => {
...
test('SB-11: a pinned PDF resolves to a link that opens at the page it was pinned on', async (t) => {
...
test('SB-44: a source id that resolves to nothing is counted, never invented', async (t) => {
...
test('SB-44: a section with no sources answers with none, not with an error', async (t) => {
...
test('SB-44: sources are asked for by section, and a section that is not there is a 404', async (t) => {
...
test('SB-44: reading provenance writes nothing — it is the one session tap that is not a signal', async (t) => {
...
test('SB-260: backup counts distinguish the current result from preserved result history', async (t) => {
...
test('a skip completes the section, advances the resume point and leaves a weak signal', async (t) => {
  ...
  assert.equal(saved?.currentSectionIndex, 1, 'SB-31: resume lands on the first unfinished section');
...
test('SB-243: a partial keyed answer records a miss and keeps the lesson open', async (t) => {
  const llm = new StubLlm(() => ({
...
test('a depth shift rewrites one section only, at one step', async (t) => {
  ...
  assert.equal(res.status, 200);
  assert.equal(res.body.depth, 'from-nothing', 'SB-26: one step down from building');
...
/**
 * SB-62 — "come back to this", one tap and one nuance.
 *
 ...
 */
test('SB-62: a resurface mark is one tap, one nuance, and a signal in the ledger', async (t) => {
...
test('SB-62: a refresher is the other statement, and the ledger records it as one', async (t) => {
...
test('SB-62: the mark does not complete the section or move the resume point', async (t) => {
...
test('SB-62: a nuance the panel did not offer is a 400 that names the choices', async (t) => {
...
test('a double-tap on the resurface mark writes one signal, not two', async (t) => {
  ...
  // against — the section is not "answered", it stays open by design (SB-62) —
...
test('SB-30: a lesson question is answered from bounded lesson context and writes nothing', async (t) => {
...
test('SB-172: tangent history crosses whole or is refused before the Tutor', async (t) => {
  const llm = new StubLlm((req) => String(req.system).includes('asked something adjacent')
...
test('SB-30: accepting the Tutor offer creates one source-backed interest pin', async (t) => {
...
test('SB-30: a lesson with no surviving source refuses to invent one for a tangent pin', async (t) => {
...
test('SB-30: malformed lesson questions fail before a model or write', async (t) => {
...
test('SB-45: a conceded taught claim is checked against its source, logged, and removes section-derived comfort', async (t) => {
...
test('SB-45: a supported disagreement is logged and changes no learner evidence', async (t) => {
  const llm = new StubLlm(() => ({
...
test('SB-198: a historical body-only concession is safe on session read without a GET write', async (t) => {
...
test('SB-45: an unreadable cited source refuses before model or write', async (t) => {
...
test('SB-45: malformed or empty correction results write no exchange', async (t) => {
...
/**
 * SB-45, and the surprise the story-coverage audit led with: `invalidateSignals`
...
test('a contested marking stops counting against the learner (SB-45)', async (t) => {
...
test('the detector can raise a suggestion, and it lands pending (SB-15)', async (t) => {
  ...
  // The deliberate-capture precedence: never auto-pinned. It waits on the board for a confirmation.
...
test('accepting a suggestion makes a real pin, and the pin says where it came from', async (t) => {
  // "He clicks Add. It becomes a struggle pin." It used to become a state field:
  // accept flipped `state` and `fromSuggestion` was hardcoded false at the only
  // place a pin was ever made, so the agent's own catch could not be told from
  // the learner's.
  ...
  assert.equal(pin.type, 'struggle', 'the story is explicit: it becomes a struggle pin');
  ...
  // The pin goes down the same path as one made by hand, which means it has to
  // be shaped like one — including the field that used to crash the clusterer.
...
test('rejecting a suggestion counts against the site it came from (SB-16)', async (t) => {
...
test('prefs come back with the shipped defaults and a PUT patches them', async (t) => {
  ...
  assert.ok(got.body.excludedDomains.length > 0, 'SB-41: exclusions ship populated, not empty');
...
test('the learner model is listed, edited and deleted', async (t) => {
  ...
  assert.equal(edited?.userEdited, true, 'SB-42: a user edit outranks derived state');
 ...
 * has been in the store since SB-285 repaired `topicId`; it was simply never
 ...
 *
 * SB-282 built the door for the one modality question, and the distinction it
...
test('SB-285: an insight that plainly names one topic is scoped to it', async (t) => {
  /**
...
test('deleting a pin reaches the signals derived from it', async (t) => {
  ...
  assert.deepEqual((await h.store.getTopic('A'))?.pinIds, ['p2'], 'SB-43: and topic membership');
...
/**
 * WHO IS ALLOWED TO READ A REPLY FROM THIS SERVICE.
 *
 * The block below used to assert `*`, under a comment saying the extension is
 * the only client — which was true about who calls and said nothing about who
 * can read. The loopback shape has no shared secret in front of it by design
 * (`runtime.ts`), so `*` meant any page the learner happened to have open could
 * call `DELETE /everything` and be handed the answer. The header is now an
 * allowlist of exactly two origin shapes, and the last test here is the one
 * that must never go green again.
 */
...
test('SB-273: material progress accepts exact whole remainders and refuses loose coercion', async (t) => {
...
test('SB-206: a persisted source-boundary contradiction is withheld from session and backup reads', async (t) => {
 ...
 * built — the submission draft says so in as many words, and refuses to claim
 * it. The interactive-agent reachability contract wires it and leaves the other two unreachable agents alone.
 *
 ...
 * writes NOTHING. SB-39's loop back to the board is one tap further on and is
...
test('SB-38: a draft comes back reviewed against what the learner is shaky on', async (t) => {
...
test('SB-256: the review wire says whether board personalisation was actually available', async (t) => {
...
test('SB-38: the check writes nothing at all', async (t) => {
  ...
  // the same rule that keeps SB-44's provenance tap signal-free.
...
test('SB-38: a draft with nothing in it is a 400 that names the field', async (t) => {
...
test('SB-38: a scrap is answered honestly rather than declared sound', async (t) => {
...
test('SB-38: a review the model could not do says so, and is not a clean bill of health', async (t) => {
...
test('SB-38: a finding about a topic the board does not have keeps the finding', async (t) => {
...
test('SB-245: harmless topic-id drift retains the learner-specific review line on the wire', async (t) => {
...
test('SB-38: a draft over the cap is reviewed and the learner is told it was cut', async (t) => {
...
test('SB-38: the background a learner pastes reaches the review, screened', async (t) => {
...
test('SB-38: a context that is not a string is a 400 that names the field', async (t) => {
...
test('SB-257: the mark wire separates a real quote from absence and drops a supplied answer', async (t) => {
...
test('SB-244: criterion key drift is repaired before the wire reports coverage', async (t) => {
...
// ------------------------------- the pages, typed out (The structured-criteria contract) (new)
...
test('the transcription writes nothing at all, the same as the two checks beside it', async (t) => {
  ...
  // same rule that keeps `/review`, `/mark` and SB-44's provenance tap
  // signal-free.
...
test('SB-31: a session gone cold comes back with two lines, and buys nothing', async (t) => {
...
test('SB-31: a section the Composer gave no recap line falls back to its heading', async (t) => {
...
test('SB-31: a session touched this morning is not cold, and costs nothing to ask about', async (t) => {
  await (async () => {
...
test('SB-31: a cold session with nothing done yet has nothing to recap', async (t) => {
  await (async () => {
...
test('SB-31: the recap is read-only, like every other tap that is not an answer', async (t) => {
  await (async () => {
...
test('SB-31: a recap the model could not write leaves the resume working', async (t) => {
  await (async () => {
...
test('SB-31: a session that does not exist is a 404 rather than an empty recap', async (t) => {
...
/**
 * `/batch` replaced `/schedule/due` — the event-driven processing contract.
 *
 * The old route asked a clock: an arrived hour and an unbuilt day meant build,
 * and it took nothing about whether there was anything TO build. These hold the
 * same guarantees the three tests here used to, against a decision made of
...
test('a pause stops a run somebody pressed for, rather than quietly running it', async () => {
  // The collection-pause contract: a pause genuinely stops collection. A button is not a way round
 ...
 *
 * `maybeAutoRun` is called from `POST /pins` — the event-driven processing contract, the trigger is a pin
...
test('a pin keeps what the Scout called it, so a take is headed the subject', async () => {
  ...
  try {
    ...
    // And the retry answers with the same one, which used to be the single
    // thing an idempotent post got wrong.
...
test('a pin arriving is the trigger, and only once enough have', async () => {
  // The event-driven processing contract: "runs asynchronously in the background" is served by an event,
...
// ============================== The learner-controlled lineup contract: the lineup, and what it writes
...
test('each section carries the subject its topic belongs to, or nothing', async (t) => {
  /**
   * The learner-controlled lineup contract, amended: *"The actual subject should be next to the expected
 ...
 * commitment unchanged. Found while checking The learner-controlled lineup contract's subject label
...
test('a course id that names nothing is refused, on the way in and on the way through', async (t) => {
  ...
  // And creation is held to it too, so a dangling link cannot arrive by the
  // other door. It used to accept any non-empty string.
  const made = await h.call('POST', '/commitments', {
...
test('SB-204: a quick-take verdict retries exactly and a later opposite choice replaces it', async (t) => {
...
test('a topic the learner retired is not coming up', async (t) => {
  // SB-37: the learner's own decision is honoured here as everywhere.
...
test('a lesson names the dated work it moves forward, and only where one is open', async (t) => {
  /**
   * The affordance-first interface contract moved this from a sentence to a thing. The hero said *"It moves
