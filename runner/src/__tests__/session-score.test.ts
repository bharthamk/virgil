...
test('the scorecard reads the pipeline the product contract describes: three registers, closed provenance', async () => {
  ...
  assert.ok(session.estimatedMinutes <= session.targetMinutes,
    ': the run under-claims rather than over-claims');
});
...
test('the scripted model writes far under budget, and that is a proxy, not a failure', async () => {
  ...
  assert.equal(check(card, 'word-budget').status, 'pass',
    'the budget check is one-sided: overrunning is the  failure, undershooting errs safe');

  /**
   * The material-aware budget's comparability effect, asserted rather than described.
   *
   ...
   * wrote differently.
   *
   * So: `budget-fill` from before this contract is comparable with `budget-fill`
...
test('each hard check fails on the thing it is checking, and only on that', async () => {
  ...
  const cases: readonly [string, ScoreableSession][] = [
    ...
    // The withheld-content contract. The note is true about what the Composer wrote and false
...
test('a withheld section the note does not name is not a defect — the check is about the claim', async () => {
  ...
  // the withheld-content contract forbids is the note going on to claim the withheld material.
