...
/**
 * The forage stage line, and what the next night is owed.
 *
 * The line used to read `21 pins, 19 from capture envelope only, 0 failed` on a
 * run where 19 of 21 model calls had failed. Both halves of that were true and
 * neither said what happened: Forager degrades per pin and swallows the error,
 * so `failed` counted only the pins that threw all the way out of the agent, and
 * a swallowed failure was reported as a pin that needed nothing.
 */
