...
test('SB-251: recommended settings cannot replace a working map before Cloud is configured', async (t) => {
  const p = await probe();
...
test('a Local check can probe the typed endpoint without saving it first', async (t) => {
  const p = await probe();
  const h = await startService('model-config-check-draft-endpoint', {}, {
    secret: SERVICE_SECRET,
    models: { localEndpoint: 'http://127.0.0.1:9' },
  });
  t.after(async () => { await h.close(); await p.close(); });
  const auth = { 'x-virgil-secret': SERVICE_SECRET };
  const before = await h.store.getPrefs();

  const checked = await h.call('POST', '/model-connections/local/check', {
    endpoint: p.endpoint,
  }, auth);
  ...
  assert.deepEqual(await h.store.getPrefs(), before,
    'checking the field persisted an endpoint the learner had not saved');
  assert.equal((await h.call('POST', '/model-connections/cloud/check', {
    endpoint: p.endpoint,
  }, auth)).status, 400, 'a Cloud check accepted a browser-selected destination');
  assert.equal((await h.call('POST', '/model-connections/local/check', {
    endpoint: p.endpoint, extra: true,
  }, auth)).status, 400, 'the check body widened beyond its one preview field');
});
