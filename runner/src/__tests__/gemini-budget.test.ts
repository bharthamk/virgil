...
describe('deep-tier call count is read off the routing, not assumed', () => {
  ...
  test('a board under the Analyst floor loses that call', () => {
    // `analyse` returns before the model on fewer than four pins.
    const n = budget.nightlyCalls({ ...warm, pins: 3 });
    assert.equal(n.byStage.analyse.deep, 0);
    assert.equal(n.deep.min, 3);
  });

  test('a board with material to observe carries the Analyst conditional', () => {
...
// ------------------------------------------- the quota-accounting contract: failure is not a saving
...
describe('the elastic stage absorbs savings, never shortfall caused by failure', () => {
   ...
   *
   * Ruled 32: the elastic stage may only absorb budget freed by a stage that
...
// ------------------------------------------------------- the material-aware budget contract’s two bases

describe('the budget basis is labelled, and the label is not a second opinion', () => {
  /**
   * The material-aware budget contract gave `scoreSession` two budget denominators — `issued`, which
  ...
  test('a board with pins is issued; without them it is register-only', async () => {
    ...
    // which is knowable material of zero words and lands on the material-aware budget contract’s 150-word
...
// ------------------------------------------- the quota-accounting contract: the circuit breaker
...
describe('the circuit breaker stops a run the provider is not answering', () => {
  ...
  test('one retry, and one only — a wait that fails does not buy another wait', async () => {
    // Before the quota-accounting contract the retry branch recursed into a fresh attempt whose
...
// -------------------------------- the quota-accounting contract: attempts against presumed billing

describe('the day ledger reports attempts and presumed billing separately', () => {
  /**
   * The quota-accounting contract, in the words that were ruled: *transport-level 503s that arrive
  ...
  test('a 503 without an exhaustion signal is an attempt but not a billed request', async () => {
    ...
    assert.equal(ledger.deepBilled, 0, 'neither reached the model — the quota-accounting contract');
