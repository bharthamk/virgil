...
test('a course keeps two counts and never a percentage', async () => {
  ...
  try {
    ...
    // SB-33: no combined number. A course percentage is a comfort number with a
    // course's name on it.
...
test('SB-241: a learner can add the missing safe link without replacing material truth', async () => {
...
test('SB-271: material correction changes only learner-editable details', async () => {
...
test('SB-271: one material can be removed only from an active course', async () => {
...
test('completed course work keeps its linked topic inside that course without rewriting it', async () => {
  /**
   * SB-229. The compiled journey linked Web Accessibility to an assignment in
