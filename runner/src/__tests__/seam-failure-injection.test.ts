 ...
 * 429, a 5xx, a socket dying, JSON truncated by a token budget (D10), a reply
 ...
 *
 * The rule being checked throughout is D10's: one agent's bad night must not
 ...
 * shape as D6's fix — a subject nothing else on the board can absorb.
 */
...
for (const fault of LAX_REPLIES) {
  test(`the verifier withholds rather than ships when a lax adapter returns ${fault.name}`, async () => {
    ...
    // looks. This is the exact shape of D6/D9/D11 — the plumbing was wrong and
...
test('no fault at any stage stops the nightly run', async () => {
  // D10: the Analyst returned truncated JSON, the parse threw, and nine minutes
