...
test('a drop interrupted between the pin and the plan finishes the job when it is sent again', async () => {
  /**
   * THE HOLE A RETRY USED TO CONFIRM RATHER THAN CLOSE.
   *
   * A planning item is two writes with nothing making them one: the pin, then
   * the draft. A process that dies between them — or a link that drops at
   * exactly the wrong second — leaves a syllabus on the board as *material with
   * no proposal attached to it*, which is the one outcome this endpoint exists
   * to avoid, and the retry used to seal it in: it found the pin, answered
   * `repeated: true, draftId: null` and moved on. Every number in the receipt
   * said the drop had worked. The deadlines were never proposed and nothing
   * anywhere would ever have said so.
   *
   * **The interruption is staged rather than simulated**, because no store here
   * can delete a draft and a test that reached inside one would be describing
   * the file format instead of the endpoint. The first request sends the
   * syllabus under a kind that does not plan: that writes the pin, writes no
   * draft, and leaves the board in *exactly* the state a death between the two
   * writes leaves — the kind lives on the item, never on the pin, so there is
   * nothing on disk to tell the two apart. The retry then sends the item it
   * always meant to send, and has to converge on the complete state rather than
   * agree with the broken one.
   */
...
test('identical planning files across request chunks keep distinct drafts after a torn write', async () => {
  ...
  try {
    ...
    // Chunk A tears after material. Chunk B completes the same content under a
    // different item identity. Content-only matching used to let A claim B's
    // proposal forever when each chunk rebuilt its own match map.
...
test('two drops whose ids would run into each other cannot be sent at all', async () => {
  ...
  try {
    ...
    // The two ids that used to collide, both legal now that neither can carry
    // the separator, and each seeing only its own pins.
