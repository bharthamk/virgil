...
/**
 * SB-282, THROUGH THE DOORS THE LEARNER ACTUALLY USES.
 *
...
test('the confirm door refuses what it has nothing to say about', async (t) => {
  ...
  t.after(() => h.close());
  // The door serves every machine read since 2026-08-29, not only this
  // question, so the two refusals are about the two rows an endorsement would
  // say nothing about: a sentence the learner wrote, and one that is not there.
