...
test('an empty board runs every stage and produces an honest empty session', async () => {
  ...
  assert.equal(session?.insufficient, true, 'SB-23: the empty card, not a manufactured lesson');
...
test('the garden line says NOT ENOUGH on a night the run then teaches — the line is about the pool', async () => {
  ...
  // teaches a single-topic pool anyway (SB-23, and the Gardener's own note on
...
test('a night whose every section named a topic nobody offered persists nothing', async () => {
  // The three-state batch-result contract. The board is real, the topics are real, the model answered —
  // and every section it wrote was attached to an id that was never offered, so
  // the Composer dropped all of them. What used to happen next: `insufficient`
  // was false, so the run verified nothing, persisted a zero-section session,
  // and the learner's morning was a session-shaped object with no session in
  // it. A stored empty session is worse than no session, because the panel
  // cannot tell it from a real one and the night's material is now marked as
  // taught.
