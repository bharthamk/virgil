...
test('the two-stage package keeps one extension id and ships only browser runtime files', (t) => {
  ...
  const base = ['--service', 'https://learner.example.test/path', '--out', out, '--allow-dirty'];
  ...
  execFileSync(process.execPath,
    [script, '--service', 'https://learner.example.test', '--out', otherOut, '--allow-dirty'],
  ...
  assert.equal(finishedMeta['schema'], 'virgil-extension-package-v2');
  assert.match(finishedMeta['sourceCommit'], /^[a-f0-9]{40}$/);
  assert.match(finishedMeta['sourceTree'], /^[a-f0-9]{40}$/);
  assert.equal(typeof finishedMeta['sourceDirty'], 'boolean');
  assert.ok(finishedMeta['files'].some((entry: { path: string }) => entry.path === 'manifest.json'));
  assert.ok(finishedMeta['runtimeFiles'].some((entry: { path: string }) => entry.path === 'dist/service.js'));

  const rebuilt = execFileSync(process.execPath, [script, ...base], { cwd: repo, encoding: 'utf8' });
  const rebuiltManifest = readJson(join(out, 'manifest.json'));
  assert.match(rebuilt, /extension ready/i);
  assert.equal(rebuiltManifest.key, prepared.key);
  assert.equal(rebuiltManifest.oauth2.client_id, CLIENT,
    'rebuilding a finished watched package silently removed its OAuth client');
...
test('packaging refuses unsafe services, dangerous roots and unrelated existing output', (t) => {
  ...
  for (const args of [
    ['--service', 'http://learner.example.test', '--out', join(tmpdir(), 'virgil-bad-package'), '--allow-dirty'],
    ['--service', 'https://learner.example.test', '--out', join(repo, 'extension'), '--allow-dirty'],
    ['--service', 'https://learner.example.test', '--out', '/', '--allow-dirty'],
    ['--service', 'https://learner.example.test', '--out', unrelated, '--allow-dirty'],
  ]) {
...
test('extension release packaging refuses a dirty source tree by default', (t) => {
  if (!existsSync(join(repo, '.git'))) return;
  const marker = join(repo, 'judge-extension-dirty-marker.tmp');
  writeFileSync(marker, 'test-only');
  t.after(() => rmSync(marker, { force: true }));
  const out = join(mkdtempSync(join(tmpdir(), 'virgil-extension-dirty-')), 'extension');
  assert.throws(() => execFileSync(process.execPath,
    [script, '--service', 'https://learner.example.test', '--out', out],
    { cwd: repo, encoding: 'utf8', stdio: 'pipe' }), /dirty tree/);
});

test('a portable v2 receipt refuses a package whose runtime bytes changed', (t) => {
  const temp = mkdtempSync(join(tmpdir(), 'virgil-extension-tamper-'));
  t.after(() => { rmSync(temp, { recursive: true, force: true }); });
  const out = join(temp, 'extension');
  const args = [
    '--service', 'https://learner.example.test', '--out', out, '--allow-dirty',
  ];
  execFileSync(process.execPath, [script, ...args], { cwd: repo, stdio: 'pipe' });
  writeFileSync(join(out, 'dist/service.js'), '// changed after packaging\n');
  assert.throws(() => execFileSync(process.execPath, [script, ...args], {
    cwd: repo, stdio: 'pipe',
  }), /not a valid Virgil extension package/);
});
