...
test('a bounded Demo launch carries its daily ceiling and disables notebook export', async () => {
  const bodies: unknown[] = [];
  const fake: typeof fetch = async (input, init) => {
    if (String(input).includes('metadata')) {
      return new Response(JSON.stringify({ access_token: 'token-token-token-token', expires_in: 3600 }));
  ...
  const launcher = new CloudRunJobLauncher({ target: TARGET, fetch: fake });
  await launcher.launch({
    boardId: 'learner-judge-workspace-v1', batchKey: '2026-08-19', asked: true,
    receiptId: RECEIPT,
    policy: { operatorLimit: 500_000, operatorWindow: 'day', notebookExport: 'disabled' },
  });
  assert.deepEqual((bodies[0] as any).overrides.containerOverrides[0].args, [
    'runner/dist/cli.js', 'process',
    '--operator-limit=500000',
    '--operator-window=day',
