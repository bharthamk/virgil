...
export interface LlmUsageRow {
   ...
   *
   * They are counted because **the quota-accounting contract as amended says every issued request
   * is presumed billed** — there is no unbilled-503 category on this provider,
   * and that correction exists because a run that took 17 × 503 and then 3 more
  ...
  report(runAt: string): UsageReport {
    ...
    return {
      ...
      notes: [
        ...
        'issuedNotReturned counts requests that were issued and did not come back. The quota-accounting contract as '
 ...
 *
 * `issuedNotReturned` means "issued and presumed billed" — the quota-accounting contract as
 ...
 * `issuedNotReturned` catch when the quota-accounting contract was amended and this one did not,
 ...
 * own 503 storm is the incident the correction was written for. Two decorators
