...
import type {
  ...
  ModelBudgetWindow, ModelSpend, Store,
} from '@sb/core';
import {
  LlmRefused, MAX_BUDGET_TOKENS, MODEL_BUDGET_DAILY_WINDOW, MODEL_BUDGET_UNIT,
  MODEL_BUDGET_WINDOW,
 ...
 * `issuedNotReturned` for anything that throws through it, and the usage-accounting contract reads
 ...
 *
 * They count the same calls and cut them differently, and since the usage-accounting contract
...
interface BudgetScope {
  ...
  /**
   * Armed by a deferred gate; fired by the key ladder's paid arm.
   *
   * **It belongs to one model call and not to the request.** It used to live on
   * the request's record, which is one record for however many calls a handler
   * makes, and that put two faults in it. A gate armed while the budget was
   * exhausted was never taken down again when the budget recovered — nothing
   * cleared it, so a learner who reset their window mid-request had the next
   * call in that request stopped by an armed gate carrying the OLD numbers.
   * And the forage and verify stages run three calls at once in one scope, so
   * a call that gated cleanly could reach the paid arm and fire a gate armed by
   * a different call entirely, with a different connection and a different
   * state in the 402.
   *
   * So each call through `budgetedLlm` gets a child record of its own (see
   * `withCallScope`) and the gate arms there. An armed gate now belongs to the
   * call that armed it, and dies with it.
   */
  paidGate: (() => void) | null;
...
export interface ModelBudgetReceipt {
  ...
  /** `day` only for a deployment-owned allowance such as private judge access. */
  readonly operatorWindow: ModelBudgetWindow | null;
...
export interface ModelBudgetLedgerOptions {
  ...
  readonly operatorLimit?: number | null;
  /** A daily operator ceiling resets its guarded Cloud/API count at 00:00 UTC. */
...
export function operatorModelBudgetWindowFrom(value: string | undefined): ModelBudgetWindow | undefined {
  const window = value?.trim();
  if (!window) return undefined;
  if (window !== MODEL_BUDGET_WINDOW && window !== MODEL_BUDGET_DAILY_WINDOW) {
    throw new RangeError('operator model budget window must be total or day');
  }
  return window;
}
...
export class ModelBudgetLedger {
  ...
  /** Avoid a Firestore transaction on every read while still rechecking after
   *  UTC midnight. The durable `since` value makes concurrent instances agree. */
  private checkedOperatorDay: string | null = null;
  private operatorDayFlight: Promise<void> | null = null;
  ...
  constructor(private readonly opts: ModelBudgetLedgerOptions) {
    ...
    if (opts.operatorWindow !== undefined
      && opts.operatorWindow !== MODEL_BUDGET_WINDOW
      && opts.operatorWindow !== MODEL_BUDGET_DAILY_WINDOW) {
      throw new RangeError('operatorWindow must be total or day');
    }
  }
  ...
  private utcDay(): string { return this.now().slice(0, 10); }

  private async ensureOperatorDay(): Promise<void> {
    if (this.opts.operatorLimit == null || this.opts.operatorWindow !== MODEL_BUDGET_DAILY_WINDOW) return;
    const day = this.utcDay();
    if (this.checkedOperatorDay === day) return;
    if (!this.operatorDayFlight) {
      this.operatorDayFlight = mutateLearnerPrefs(this.opts.store, (prefs) => {
        const spend = readModelSpend(prefs.modelSpend);
        const sinceDay = spend.since?.slice(0, 10) ?? null;
        return sinceDay === day
          ? prefs
          : { ...prefs, modelSpend: resetModelSpend(this.now(), spend) };
      }).then(() => { this.checkedOperatorDay = day; }).finally(() => {
        this.operatorDayFlight = null;
      });
    }
    await this.operatorDayFlight;
  }

  private async current(): Promise<{ budget: ModelBudget | null; spend: ModelSpend }> {
    await this.ensureOperatorDay();
  ...
  private effectiveBudget(budget: ModelBudget | null, spend: ModelSpend): ModelBudget | null {
    ...
    if (limit === null) return budget;
    const operatorWindow = this.opts.operatorWindow ?? MODEL_BUDGET_WINDOW;
    if (operatorWindow === MODEL_BUDGET_WINDOW && budget && budget.limit <= limit) return budget;
    return {
      limit: budget ? Math.min(budget.limit, limit) : limit,
      unit: MODEL_BUDGET_UNIT,
  ...
  private receiptOf(budget: ModelBudget | null, spend: ModelSpend): ModelBudgetReceipt {
    ...
    return {
      ...
      operatorWindow: this.opts.operatorLimit == null
        ? null : this.opts.operatorWindow ?? MODEL_BUDGET_WINDOW,
      budget: effective,
  ...
  async gate(
    ...
    /**
     * Not stopping is a decision, and it has to be written down.
     *
     * This branch used to do nothing at all, which meant `gate` could only ever
     * ARM a scope and never disarm one. On a per-call record that is almost
     * academic; it was not academic on the request-wide record this used to
     * run against, where a gate armed by an earlier call in the same request
     * outlived the budget that justified it. A learner who reset their window
     * mid-request met a 402 quoting the spend from before the reset. `gate` now
     * leaves the scope saying exactly what this call decided, either way.
     */
  ...
  /** One issued call that did not come back. The usage-accounting contract: it is not free. */
 ...
 * line, and since the free-arm ruling the deferred `ModelBudgetStop` is thrown
 ...
 * `issuedNotReturned` for calls that were never built, and the usage-accounting contract reads that
