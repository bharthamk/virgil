...
/**
 * The header the service requires and the extension sends — the service-protection contract.
 *
...
export type BatchOutcome =
  ...
  /** The run completed and honestly produced nothing. */
...
/**
 * Derives the process outcome from persisted results. A composed session with
 * every section withheld is not delivered, and quota exhaustion is distinct
 * from an honest empty board. Quota is checked first because retry guidance
 * differs from the guidance for a run that found nothing to teach.
 */
 ...
 * reported honestly, and left a store that reflects them (the delivery-safety contract and withheld-content contract).
 ...
 * wrong. Transport-proof the quota-retry policy rules that a spent *daily* cap degrades
 ...
 * degrades again immediately. That is graceful-degradation constraint arriving through a door the correction
 * does not watch.
 *
 ...
 * the difference matters when the question on deployment is whether the service
 * is healthy.
 *
...
// ---------------------------------------------------------------------------
// Who is asking (the learner-identity contract)
// ---------------------------------------------------------------------------
 ...
 *
 * The emulator form is legitimate and is how the learner-identity contract says to build; running
