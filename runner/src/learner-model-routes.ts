...
export async function handleLearnerModelRoute(
    ...
    ctx.reply(res, 200, {
      statements: statements.filter((statement) => !statement.rejected).map((statement) => {
        ...
        return {
          ...
          /** SB-285 repaired this join and nothing sent it. The room that draws
          ...
          /**
           * SB-282. Absent on every ordinary row, and the panel branches on it.
           *
  ...
  if (req.method === 'POST' && url.pathname === '/model') {
    ...
    /**
     * SB-285: the learner's own sentence, scoped by the same join as a machine
     * one.
     *
  ...
  /**
   * SB-282: yes, that is how it goes for me. And, since 2026-08-29, yes about
   * any other sentence this product has written about somebody.
   *
   * A door of its own rather than a flag on `PUT`, because it is not an edit.
   * `PUT` replaces the sentence with the learner's words and marks the row as
   * theirs; this leaves the sentence exactly as it was written and records that
   * a person agreed with it. What the Insights screen then shows is Virgil's
   * read, confirmed, which is the only form PRODUCT_SHAPE.md allows a modality
   * claim to take and the honest form for every other read as well.
   *
   * One door for both because it is one act. The modality branch keeps its own
   * field, because that mark carries the question's whole lifecycle and the
   * one-at-a-time cap counts it; an ordinary read has nothing to carry but the
   * answer, so the answer is all that is stored.
   *
   * There is no door back, and there does not need to be one. Somebody who
   * changes their mind corrects the read or rejects it, exactly as before.
   */
