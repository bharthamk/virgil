import type {
  Clock, NotebookDoc, NotebookDocKey, NotebookExport, NotebookInput, Store, WriteReceipt,
} from '@sb/core';
import {
...
const FOREGROUND_BODY_MAX_CHARS = 100_000;

/** Validate the one foreground document a hosted learner may substitute. */
export function foregroundNotebookDocument(value: unknown): NotebookDoc | null {
  if (value === undefined || value === null) return null;
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw new NotebookScopeError('foreground must be the current learn-now document');
  }
  const row = value as Record<string, unknown>;
  const title = notebookDocTitle('learn-now');
  if (row.key !== 'learn-now' || row.title !== title || typeof row.body !== 'string') {
    throw new NotebookScopeError('foreground must be Virgil\'s learn-now document');
  }
  const body = row.body.trim();
  if (!body || body.length > FOREGROUND_BODY_MAX_CHARS) {
    throw new NotebookScopeError('foreground learn-now body is empty or too large');
  }
  return { key: 'learn-now', title, body: `${body}\n` };
}
...
/** Write the exact visible foreground lesson alongside the two ordinary projections. */
export async function exportForegroundNotebook(
  store: Store, clock: Clock, to: NotebookExport, foreground: NotebookDoc,
): Promise<WriteReceipt> {
  const input = await readNotebookInput(store, clock);
  return to.writeDocs(NOTEBOOK_DOC_KEYS.map((key) =>
    key === 'learn-now' ? foreground : notebookDoc(key, input)));
}
 ...
 * two warnings; the ruling is unchanged and is restated where it now lives.
 *
