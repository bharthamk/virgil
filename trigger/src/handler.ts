 ...
 * and must not be delivered again*. the delivery-safety contract and withheld-content contract establish that a night can
 ...
 *
 * The third case is the one that has to be got right or the correction it
 * protects is undone. the transport contract the quota-retry policy, as
 * implemented in `adk/src/errors.ts`, says a spent daily quota **degrades and
 ...
 * again — the retry storm the correction exists to prevent, arriving through a
 * door the correction does not watch. A degraded night **acks**.
 */
...
export type BatchOutcome =
   ...
   * under the quota-retry policy. Not a failure, and explicitly not a retry.
   */
...
export function nightlyTriggerHandler(deps: TriggerHandlerDeps): MessageHandler {
  ...
  return async (delivered: DeliveredMessage): Promise<AckDecision> => {
    ...
    if (decision.verdict === 'already-built') {
      // The night exists under the batch-idempotency contract’s key. The trigger has been honoured —
