 ...
 * held to. The guard reads the per-night idempotency contract's key through it, so the delivery side is
...
/**
 * A handler over a night that writes a session under the per-night idempotency contract's key.
 *
...
function rig(opts: {
  ...
  const handler = nightlyTriggerHandler({
    ...
    run: async ({ batchKey }) => {
      ...
      // The persisted batch-identity contract: the night the run was told to build travels with the row it
...
export function runHandlerContract(harness: TransportHarness): void {
  ...
  test(name('the night key comes off the message, so a redelivery names the same night'), async () => {
    ...
    try {
      ...
      assert.deepEqual(await sessionIds(r.store), [sessionIdForBatch(KEY_FOR_SCHEDULED)],
        'one night, one session row — the per-night idempotency contract held from the delivery side');
    } finally { await t.close(); }
  });
  ...
  test(name('a night already in the store is recognised whatever the store named its row'), async () => {
     ...
     * id it invented.
     *
     * The Firestore adapter (`port/firestore`, the per-night idempotency contract) makes the *document
  ...
  test(name('a night that names itself is recognised by that name, not by the clock it finished on'), async () => {
    /**
     * **The persisted batch-identity contract, from the delivery side.**
     *
  ...
  test(name('a session that names ANOTHER night does not answer for this one'), async () => {
    /**
     * The other half, and the one that costs a learner rather than a budget.
     *
     * A mislabelled row used to make the *following* night look built: the
     * guard read its `builtAt`, found it inside this run, answered yes, and a
     * night that had never run was skipped and acked as an ordinary duplicate.
     * Nothing anywhere reported a failure.
     *
     * With the field, a row that says it is the 19th cannot answer for the
     * 20th no matter what its clock reads.
     */
    const r = rig();
    await r.store.putSession({
  ...
  test(name('a night that honestly produced nothing is ACKED and not redelivered'), async () => {
    /**
     * The three-state batch-result and verifier-withholding contracts. `nothing-to-teach` and `model-failed` are runs that
  ...
  test(name('a quota-degraded night is ACKED and not undone by a retry'), async () => {
    /**
     * `GEMINI_TRANSPORT_PROOF_2026-08-20.md` the quota-degradation contract, as implemented in
     ...
     * redeliver within seconds — the retry storm this protection exists to
     * prevent, arriving through an unguarded path.
     */
