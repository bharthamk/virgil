...
/**
 * **The delivery and write idempotency boundaries, running together.**
 *
 * The trigger boundary and the Firestore write boundary are each tested in
 * isolation. This file proves their shared delivery contract end to end.
 *
 ...
 * `batchKey` it was handed, because the persisted batch-identity contract made that the field the row is
...
/**
 * **The one thing the two lanes had not agreed, pinned here.**
 *
 * The lanes cut a night on different boundaries, and neither is wrong on its own
 * terms:
 *
 * | lane | the night of an instant |
 * | :--- | :--- |
 * | `trigger` | local date of `(instant − boundaryHours)`, `boundaryHours` **defaults to 6** |
 * | `adapters` (Firestore) | `batchKeyOf(builtAt)` — the **plain UTC date**, no offset |
 *
 * A six-hour offset is right in isolation: a 03:00 run belongs to the previous
 * day and a schedule drifting across midnight must not rename the night. A plain
 * UTC date is right in isolation too: the document name *is* the idempotency key,
 * and the simplest key that a retried Job reproduces is the date it ran on.
 *
 * Composed, they are two partitions of the same timeline with their cuts six
 * hours apart, and that has two consequences — one cosmetic, one not:
 *
 *  1. **The labels disagree.** At the deploy commit's `0 3 * * *` schedule the
 *     trigger calls it `2026-08-20` and Firestore names the document
 *     `2026-08-21`. Harmless on its own, because the guard reads `builtAt`
 *     through its own rule rather than reading the document name — that is
 *     §6a's fix and it holds.
 *  2. **Two distinct trigger nights can land on one Firestore document.** This
 *     one is not cosmetic: it is silent data loss, and
 *     `the two partitions disagree under the default, and one night is lost`
 *     below demonstrates it end to end rather than arguing about it.
 *
 * **The rule this branch ships:** with the Firestore adapter behind the trigger,
 * the night boundary is **`VIRGIL_TRIGGER_NIGHT_BOUNDARY_H=0`** with
 * `VIRGIL_TRIGGER_NIGHT_TZ=UTC`. At zero the trigger's partition and
 * `batchKeyOf`'s are not merely compatible for the current schedule — they are the
 * **same function**, which is a property no schedule change can erode. That is
 * the setting required to keep both partitions identical; the finding is that it is the right
 * setting on day one, because the hazard is not the schedule moving, it is the
 * schedule being three hours from a boundary that the two lanes do not share.
 *
 * The default is left at 6. Changing `DEFAULT_NIGHT_KEY_RULE` would be
 * redesigning the trigger lane's half from the integration branch, and the
 * default is correct for the store the trigger lane was written against. This is
 * a **deployment fact**, it belongs in the deploy commit's environment beside the
 * `gcloud` lines, and it is pinned by the tests below so that a deployment which
 * forgets it fails here rather than at 3am.
 */
const ALIGNED: NightKeyRule = { timeZone: 'UTC', boundaryHours: 0 };

/** The configured production schedule hour in UTC. */
 ...
 * and the night the run was told to build (the persisted batch-identity contract).
 *
...
test('THE defect class: a redelivered message finds the night and runs it ZERO extra times', { skip }, async () => {
  /**
   * The reason this lane exists, and the regression pin for §6a.
   *
   * The night completes and writes its session, and *then* the run throws — so
   * the handler nacks a message whose work is already done and the broker
   * genuinely redelivers it. (That is the only way to be handed the same message
   * id twice by a real broker: at-least-once permits redelivering an acked
   * message but no broker can be *asked* to.) The second delivery must find the
   * night that the first one built.
   *
   * **The row it has to find is named nothing either lane would guess.** Its
   * document is `2026-08-21` and its `id` field is a UUID, so
   * `getSession(sessionIdForBatch('2026-08-21'))` — the obvious implementation,
   * and the one this branch's ancestor shipped — returns `null`. That is
   * asserted directly below, before the redelivery assertion, so that the test
   * proves the fast path really is dead and the night was recognised by the
   * domain query alone.
   *
   * Revert `guard.ts`'s `builtBatchKey` to the id lookup and this test reports two
   * runs and — because a second run writes a second UUID under the same
   * document name — one row, which is precisely the shape that made the defect
   * invisible: the *store* stayed correct while the *fleet* spent a second
   * night's worth of model calls every delivery.
   */
  const r = await rig({
    afterWrite: ({ attempt }) => {
...
test('two different nights are two documents, each named after its own night', { skip }, async () => {
   ...
   *
   * It is also half of the per-night idempotency promise. The other half — "the key collapses
...
test('the two partitions still disagree under the DEFAULT boundary, and the persisted batch-identity contract is what stops it costing a night', { skip }, async () => {
  /**
   * **The evidence for the alignment rule, produced rather than argued — and
   * re-read after the persisted batch-identity contract.**
   *
   ...
   * first**, with the per-night idempotency contract's write side working exactly as designed while it
   * happened: the document name *was* the key, and both rows honestly claimed
   * the same name.
   *
   * The persisted batch-identity contract removes the mechanism rather than the mismatch. The document name
  ...
  try {
    ...
    // The label disagreement itself, at the exact instant that used to lose a
    // row: the trigger calls this night 2026-08-20 and `batchKeyOf` calls the same
    // session 2026-08-21. Both rows would have claimed that one name.
...
test('a retry that crosses midnight UTC does not split the night — when the first attempt persisted', { skip }, async () => {
  /**
   * **(a) from the Cloud Run lane, first of two shapes.**
   *
   * The hazard as reported: the Firestore session key is the UTC date of
   * `builtAt`, so a Job task that starts before midnight and whose retry
   * finishes after it writes `2026-08-21` and then `2026-08-22` — two rows for
   * one night, the per-night idempotency contract undone by the clock rather than by a bug.
   *
   * This is the shape where the composed system **holds**, and it is worth being
   * precise about *what* holds it, because it is not the store.
   *
   *  - The trigger's night key is derived from the **message**, so both the
   *    original delivery and the retry name night `2026-08-21`. The clock moving
   *    across midnight cannot move the key. That is `batch-key.ts`'s whole
   *    reason to be pure, and here it pays for itself in a case its own lane
   *    never tested.
   *  - The guard then asks the store *"is there a session in night
   *    2026-08-21?"*, finds the row the first attempt persisted at `23:58Z`,
   *    and refuses to run the night again. **The retry never reaches
   *    `putSession`**, so the second document name is never computed and the
   *    split cannot happen.
   *
   * So the anchor is real, and here it is the trigger's key plus the guard's
   * pre-check. Under the persisted batch-identity contract the document name is a second anchor rather than
   * the weak link it used to be, and the next test is the shape that shows it —
   * the crash where the pre-check has nothing to find.
   */
  const r = await rig({
...
test('a retry that crosses midnight names the row for the night it was FOR, not the night it finished in', { skip }, async () => {
   ...
   * pin for the persisted batch-identity contract.**
   *
   ...
   * row, so the per-night idempotency contract's "one night, one row" survived the letter of the hazard,
   ...
   * name.** The persisted batch-identity contract is that it does: `Session.batchKey` carries it and
...
test('a retry that crossed midnight does NOT consume the following night', { skip }, async () => {
  /**
   * The consequence of the test above, followed through to where it lands on a
   * learner. This was the worst of it, and it is the assertion the persisted batch-identity contract was
   * written to make true.
   *
   * A retry that crossed midnight built night `2026-08-21` and stamped it
   * `builtAt: 2026-08-22T00:03Z`, because that is honestly when the run
   * finished. The guard's only question used to be *"is there a session whose
   * `builtAt` falls inside this night?"* — so when the **22nd's** trigger
   * arrived, the guard found that row, answered yes, and skipped.
   *
   * **The 22nd never ran.** Not overwritten, not corrupted — never built. One
   * crossed-midnight retry silently consumed the following night, and the
   * learner opened the panel to a session that was a day old with nothing
   * anywhere reporting a failure: every component behaving exactly as designed.
   *
   * ## The root cause, and what closed it
   *
   * `Session` had **no field for which night it is for.** `builtAt` was the
   * proxy — for the guard, which read it through `batchKeyFor`, and for the
   * adapter, which named the document `batchKeyOf(builtAt)`. The proxy is exact
   * whenever a run's clock and its night agree, and a retry across a date
   * boundary is precisely the case where they do not. The trigger *knew* the
   * right answer and threw it away, because `putSession` takes a `Session` and a
   * `Session` could not carry it.
   *
   * The persisted batch-identity contract is the field. The guard asks it first and does not fall through to
   * `builtAt` for a row that has one, so a row belonging to the 21st cannot
   * answer for the 22nd however its clock reads. Two nights, two rows, two
   * documents — asserted below rather than argued.
   */
  const r = await rig({
    builtAt: ({ key, attempt }) => {
...
test('the schedule’s margin is no longer what holds the midnight hazard shut', { skip }, async () => {
  /**
   * The mitigation that used to be the whole answer, kept as the record of how
   * thin it was.
   *
   * At `0 3 * * *` the run finishes around 03:08Z and a retry would have to be
   * delayed by more than twenty hours to reach the next boundary — far outside
   * any Pub/Sub backoff (600s maximum) or Cloud Run Job retry window. That is
   * why the hazard was **latent rather than live**, and the thing keeping it
   * latent was the schedule: a `gcloud` line rather than a guarantee, with about
   * 0.2h between "fine" and "a night named for the wrong day".
   *
   * Both facts are asserted together on purpose. The margin is still real and
   * still worth reading when somebody edits the cron; it is simply no longer
   * load-bearing, because the two tests above hold with **no** margin at all.
   * The persisted batch-identity contract's whole claim is that this number stopped being a safety
   * argument — and a number that is no longer a safety argument should say so
   * where it is measured rather than be deleted as though it never was one.
   */
  const r = await rig();
  try {
    const at = scheduleOn('2026-08-21');
...
test('a night that honestly produced nothing is ACKED, writes nothing, and does not come back', { skip }, async () => {
  /**
   * The three-state batch-result and verifier-withholding contracts, over the real store — and the composed behaviour is read
   * from `pipeline.ts` rather than assumed. On `nothing-to-teach` it returns
   * *"nothing to teach and nothing to revise — honest empty state (SB-23)"* and
...
test('a quota-degraded night ACKS against the real store — the quota-degradation contract is not undone', { skip }, async () => {
  /**
   ...
   * `GEMINI_TRANSPORT_PROOF_2026-08-20.md` defines the quota-degradation contract: a spent daily
   ...
   * quota protection exists to prevent, arriving through an unguarded path.
   */
