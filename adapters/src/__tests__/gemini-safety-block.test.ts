 ...
 *
 * `GEMINI_TRANSPORT_PROOF_2026-08-20.md` §3z left this open on purpose: a
    ...
    await assert.rejects(
      ...
      (e: unknown) => {
        if (!(e instanceof GeminiBlockedError)) {
          return assert.fail('the provider-refusal contract: a safety block is a model failure, and the seam says so in a type '
...
test('a structured call that is blocked rejects rather than being retried three times', async () => {
  ...
  // requests to be refused twice more, which is D10's lesson pointed at quota.
  let attempts = 0;
  const real = globalThis.fetch;
...
test('a block that arrives after some text has streamed is still a block', async () => {
  ...
  // section presented as a whole one is D10 wearing a different hat: the learner
...
test('MAX_TOKENS is not a block, because more headroom is the documented fix for it', async () => {
  ...
  // truncation into a hard failure would delete the D10 repair the contract
  // asserts on.
