 ...
 * you do. Clustering is attach-only (DEAD_ENDS.md D15): a pin that already has a
...
// -------------------------------------- the merge D15 left standing, repaired

/**
 * The demonstration.
 *
 * D15's resolution ends on this: "reproducible is not correct. Both embedders
...
test('a second nightly run after the repair is identical to the first', async () => {
  // The property D15 bought and this work must not spend: a run over an
  ...
  assert.equal(shape(second), shape(first), 'asserted char-for-char, as D15 asked for');
});
