 ...
 *  — two hundred pins.
 *
 * "Untested above ~21 pins" was the product contract's own statement of the risk, and the
...
test('every one of two hundred pins lands in exactly one topic', async () => {
...
test('the same two hundred pins partition the same way twice', async () => {
...
test('the order two hundred pins arrive in does not change where they land', async () => {
  const pins = board(200);
...
test('a settled board of two hundred is a no-op on the next night', async () => {
  const pins = board(200);
  const first = await cluster(deps(namer().llm), { pins, existingTopics: [] });
...
test('twenty new pins on a board of two hundred move nothing that was already placed', async () => {
  // The scale risk the product contract names is the nightly; the risk a learner would
...
test('two hundred pins is one embedding call and one naming call', async () => {
  // The measured ceiling in the product contract is a per-call one — the Clusterer sends
