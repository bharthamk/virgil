 ...
 * `reasoning` is the 12x latency lever from model-budget constraint, `maxOutputTokens` is the
 * headroom whose absence caused graceful-degradation constraint.
 */
  ...
  named('reasoning is on unless the caller says otherwise, and off when it does',
    [text('x')], async (s) => {
      ...
      assert.equal(call(1, s.calls).thinking, false,
        'reasoning off is the 12x latency lever from model-budget constraint — an adapter that ignores it breaks the toast budget');
    });
  ...
  named('the output budget reaches the provider, and a request without one still gets a budget',
    [text('x')], async (s) => {
      ...
      assert.equal(call(0, s.calls).maxOutputTokens, 6000,
        'graceful-degradation constraint was a headroom bug — an adapter that drops the budget reintroduces it');
  ...
  named('every call carries a deadline',
    [text('x')], async (s) => {
      ...
      assert.equal(call(0, s.calls).hasAbortSignal, true,
        'streaming-response constraint: a call with no deadline of its own inherits whatever the runtime feels like, '
  ...
  named('truncated JSON is retried with more headroom and no repair lecture',
    [text('{"label": "pull subs", "minu'), text(JSON.stringify(CONFORMING))], async (s) => {
      ...
      assert.ok(call(1, s.calls).maxOutputTokens > call(0, s.calls).maxOutputTokens,
        'graceful-degradation constraint was a budget problem — retrying it with the same budget retries the failure');
 ...
 * honest-degradation rule applied to the seam itself.
 */
