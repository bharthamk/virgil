...
// cap and this ceiling are one decision (the Firestore write-size contract) and have to be checked as
 ...
 * of where it stores things. The transport proof's §9.1 named the gap this file
 ...
 * database, so the emulator is not a convenience here, it is the whole of the
 * safety argument. `GCP_SETUP_2026-08-20.md` is the standing rule it serves:
 ...
 * The production operations guide records the boundaries this file cannot
 * prove.
 */
...
// -------------------------------------------------- the per-night idempotency contract, the whole point

test('a retried night writes one session row, not two', { skip }, async () => {
  // The per-night idempotency contract, and the reason this port implements it rather than inheriting
...
test('the same session written twice is one row, however many times it is written', { skip }, async () => {
  // Idempotence in the narrow sense the ruling asks for: the write is safe to
...
test('a retry whose clock crossed midnight lands on the night it was FOR', { skip }, async () => {
  /**
   * The persisted batch-identity contract, against the real backend.
   *
...
test('two different nights are two rows — the key collapses a retry, not a history', { skip }, async () => {
  // The other half of the ruling, and the one a too-clever key would break.
...
test('two DIFFERENT sessions built on one night collapse, and that is the ruling', { skip }, async () => {
  ...
  //
  // That is what the per-night idempotency contract asks for and it is not free: the product model is
  // one session per night, and if a night ever needs two, the ruling changes
  ...
  try {
    ...
    const all = await (await store(id)).listSessions();
    assert.equal(all.length, 1, 'one night is one row under the per-night idempotency contract');
...
test('law 2, on the ledger: appends read back in the order they were called', { skip }, async () => {
  // SB-22 needs the history in order, and Firestore has no insertion order to
...
test('a one-mebibyte image collides with the document ceiling — the historical defect', { skip }, async () => {
  // HISTORICAL. This case is kept for the collision it records, not for a
  // pressure it still applies, and the name says so because for a while it did
  // not: it was called "an image pin at the extension's own cap", and after
  // the Firestore write-size contract that description was false while the assertion below went on
  // passing. A green test whose name lies is worse than no test.
  //
  // What it found. `extension/src/image.ts` used to set MAX_IMAGE_BYTES to
  // 1_048_576 — exactly one mebibyte, and exactly Firestore's ceiling for one
  // whole document. The image-size capture constraint fixed image pins at capture by fetching and
  // encoding to a data URI, and base64 costs four bytes for every three: an
  // image the extension considered perfectly legal became roughly 1.4 MiB on
  // the wire and could not be written at all. Two caps set in different months
  // by people solving different problems.
  //
  // What it is now. The Firestore write-size contract (ee9447c) moved the cap to 737_280, derived from
  // this ceiling rather than chosen, so a mebibyte of picture is no longer
  // reachable *from capture* — `fetchPinnedImage` refuses it on both the
  // counted and the claimed path, which `extension/src/__tests__/
  // image-capture.test.ts` proves offline. The number below therefore no
  // longer stands for anything the product can produce.
  //
  // Why it stays. The ceiling is Firestore's, not ours, and the taxonomy that
  // classifies the refusal is this adapter's. A pin can still arrive over the
  // ceiling by other routes — the uncapped `selection` field, an import, a
  // hand-built record — and when it does, the store must still refuse it
  // loudly and as a fact about the pin. That is the property this case guards.
  // The companion below is the one that tracks the shipped cap.
  const id = boardId();
  const s = await store(id);
  try {
...
test('an image pin at the hosted Pins cap fits in a Firestore document', { skip }, async () => {
  // The alignment the Firestore write-size contract promises, checked against the store that sets the
...
test('the installed SDK is the pinned version this adapter was measured against', { skip }, async () => {
  // A version that moves under a proof
