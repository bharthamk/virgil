...
test('a failed flush is reported to the caller rather than swallowed', async () => {
  ...
  // run degrade the stage instead — which is what D10 already expects of it.
  const fs = new MemFs();
  const store = new JsonStore(PATH, fs);
  await store.putPin(aPin('p1'));
...
test('a second handle cannot rename the first handle\'s half-written file into place', async () => {
  // The temp file used to be `${path}.${pid}.tmp` — per process, not per handle.
  // Two handles in one process is not exotic: the service holds one and a test,
  // a migration or a second run opens another over the same file. With a shared
  // temp path, one handle's rename can pick up another handle's partial write,
  // and the store file becomes the thing the rename exists to prevent.
  const fs = new MemFs();
...
test('a store file that will not parse stops the store rather than reading as empty', async () => {
  // `load` used to catch everything, so a corrupt file and a first run were the
  // same event — and the next flush wrote the empty board back over whatever was
  // there. The product call this lane makes is fail-closed: a board we cannot
  // read is not a board with nothing on it, and a crash naming the file beats a
  // silent wipe of every pin the learner ever saved.
  const fs = new MemFs();
