...
/**
 * The Gemma transport proof — the architecture, "Scout onto Gemma". Real calls.
 *
 ...
 * to quote as ballpark and are never a pass condition — closes by
...
/**
 * Grounds streaming-response constraint, on this family specifically.
 *
...
test('a Gemma response really arrives as a stream, CRLF-framed, in more than one chunk', { skip }, async () => {
  ...
  assert.ok(chunks > 1, 'the whole response arrived in one chunk — the silent connection streaming-response constraint died on');
