 ...
 * transport real."*
 *
 * The error strings asserted below are **recorded, not invented** — every one
 * of them was produced by a real emulator and is quoted in
 * The transport contract is exercised here and by `firestore-live.test.ts`.
 * proof's other standing lesson: a fixture you made up tests your beliefs.
 */
...
// ----------------------------------------------------- the batch-idempotency contract’s night key
...
test('a retried run and the run it retried land on the same night key', () => {
  // The batch-idempotency contract, stated as the one property that makes the write idempotent: two
...
// ------------------------------------------- the batch-key alignment contract’s night, as a field
...
test('a session that names its night is stored under that name, and no clock is asked', () => {
  /**
   * The batch-key alignment contract, and the whole of it in two lines.
   *
...
test('a session written before the field existed still names a night', () => {
  // Absent is the honest answer for every row persisted before the batch-key alignment contract, and
...
test('the three refusals that are facts about the data are told apart', () => {
  ...
  // only in the message — the transport contract's third defect in a new costume.
...
test('a transport failure is marked retryable and a refusal is not', () => {
  // The field the runner's degradation policy actually reads (graceful-degradation constraint, and the
  // transport contract's sixth correction). A stage that retries a permission
...
test('the store refuses to open a client with no emulator host set', async () => {
  // Local execution must never reach a billable database by default.
...
test('the refusal names the way past it, so it is a decision and not an obstacle', async () => {
  ...
  try {
    ...
    assert.match(err.message, /production opt-in/, 'the required operator decision is explicit');
  } finally {
    if (host !== undefined) process.env['FIRESTORE_EMULATOR_HOST'] = host;
  }
});
...
test('the vendor module is named as a string and pinned to an exact version', () => {
  // The transport contract's second correction: an alias instead of an exact
...
test('a session written under the old field name still lands on one day', async () => {
  // `batchKey` was `nightKey` until the manual-processing contract. The field is optional and was
  ...
  // second key, which is exactly the twice-built day the batch-key alignment contract closed.
