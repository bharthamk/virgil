...
test('the kill-switch fires BEFORE the paid arm is touched, and its refusal stops the reach', async () => {
  // The contract's second half: "then switches to paid with the internal
...
test('a non-Gemini failure does not fail over — provider-retry constraint\'s no-retry law is not amended here', async () => {
