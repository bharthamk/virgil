 ...
 *
 * This is the seam capture-envelope constraint happened at, and capture-envelope constraint is the reason it is worth writing
...
/**
 * The same page as the vendor served it to the nightly run in capture-envelope constraint — a different
...
export function runResearchContract(subject: ResearchSubject): void {
  ...
  named('the page that was asked for is the page that is requested',
    [page(ENGLISH)], async (s) => {
      ...
      assert.equal(call(0, s.calls).url, URL_,
        ...
        + 'other than the page the learner pinned, which is capture-envelope constraint with a different cause');
    });

  named('every request carries the language control capture-envelope constraint was fixed with',
    [page(ENGLISH)], async (s) => {
      await s.research.fetchPage(URL_);
      const lang = call(0, s.calls).headers['accept-language'];
      assert.ok(lang && lang.trim().length > 0,
        'capture-envelope constraint: with no accept-language, vendor documentation sites serve whatever localisation '
  ...
  named('every request identifies the product rather than arriving anonymous',
    [page(ENGLISH)], async (s) => {
      ...
      assert.ok(ua && ua.trim().length > 0,
        ...
        + 'and both arrive as  fallbacks nobody can explain');
    });
  ...
  named('every request carries a deadline of its own',
    [page(ENGLISH)], async (s) => {
      ...
      assert.equal(call(0, s.calls).hasAbortSignal, true,
        'streaming-response constraint: the nightly run has no one watching it, and a page that never finishes '
        + 'answering holds a forage slot for as long as the runtime allows');
    });

  // ------------------------------------------------------ capture-envelope constraint, made mechanical

  named('a page that ignored the language control comes back verbatim, so the check can catch it',
    [page(SPANISH)], async (s) => {
      // The whole of capture-envelope constraint in one assertion. The adapter did everything right and
    ...
    named(`a ${status} is a failure the caller can see, not an empty page`,
      [{ kind: 'http', status, body: 'nope' }], async (s) => {
        assert.equal(await s.research.fetchPage(URL_), null,
          ': gated and dead pages are normal and the caller falls back to the capture '
  ...
  named('a transport failure is null rather than a throw or an empty page',
    [{ kind: 'network', message: 'ECONNRESET' }], async (s) => {
      assert.equal(await s.research.fetchPage(URL_), null,
        'one unreachable host must degrade one pin, not abort the forage stage — graceful-degradation constraint');
    });
    ...
    [page(ENGLISH)], async (s) => {
      ...
      for (const q of ['the ack deadline', '', 'ñ 日本語 🧪', 'x'.repeat(50_000)]) {
        assert.deepEqual([...await s.research.findReferences(q, 3)], [],
          ': a reference the product cannot attribute is worse than no reference. '
      ...
      for (const limit of [0, 1, 3]) {
        ...
        for (const r of refs) {
          ...
          assert.equal(r.origin, 'agent-sourced',
            ': an agent-sourced reference must never be marked as the learner\'s own pin');
