 ...
 * `artifacts/GEMINI_TRANSPORT_PROOF_2026-08-20.md` rather than faked here.
 *
 * ## Gated, deliberately
 *
 ...
 * free tier, and DEAD_ENDS.md closes by saying so. Timings are printed for the
 ...
 * error to leave the structured ladder immediately (D18), and an adapter that
 * quietly retried a 429 would make the nightly run's cost and latency
 * unpredictable in exactly the way D18 warns about. Retry policy belongs to the
...
test('a missing key is refused before any request is made', { skip }, async () => {
  // The refusal used to name GEMINI_API_KEY; since the Settings → Models
  // credential work it is an LlmCredentialMissing whose message tells the
  // learner where a key gets saved. The ground is unchanged — a config error,
  // caught before the wire — and this gated test was the last place still
  // expecting the old copy.
  ...
  await liveOrQuotaSkip(t, 'tier map', async () => {
    ...
    for (const id of [fast.modelId, deep.modelId]) {
      ...
      assert.ok(Number(version) >= 3.5,
        `${id} is below the Gemini 3.5 floor the submission has to clear`);
    }
  });
});
 ...
 * arrived, all 60,015 tokens of them — and D6/D11 say to suspect the test before
...
/**
 * Grounds D19 directly rather than any contract assertion.
 *
 * D19: undici applies its own body timeout that an AbortController does not
...
test('the live response really arrives as a stream, in more than one chunk', { skip }, async () => {
  ...
  assert.ok(chunks > 1,
    'the whole response arrived in one chunk — this is the silent connection D19 died on');
