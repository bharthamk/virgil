 ...
 * attach-only (clustering-stability constraint).
 *
 ...
 * makes regression detectable , and the union happens when signals are
...
test('the ledger on disk is not rewritten by a merge', async () => {
  ...
  // guarantee that  regression detection rests on would be gone.
  const p = path('ledger');
...
test('decay state survives a merge in the direction that keeps safe-empty-state constraint honest', async () => {
  const s = store('decay');
  await s.putPin(pin('p1', 'A'));
  await s.putPin(pin('p2', 'B'));
