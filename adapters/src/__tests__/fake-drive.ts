...
export class FakeDrive {
  ...
  private put(file: Omit<FakeFile, 'id' | 'trashed' | 'createdTime'>): FakeFile {
    const id = `file-${this.nextId++}`;
    const made: FakeFile = {
      ...file, id, trashed: false,
      createdTime: new Date(Date.UTC(2026, 7, 24, 3, 0, this.nextId)).toISOString(),
    };
