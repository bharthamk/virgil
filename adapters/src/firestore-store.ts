...
/**
 * Firestore adapter — the storage half of the mandatory infra requirement.
 *
 * Written the way the Gemini adapter was written *after* its transport proof
 * rather than before it: every claim about how Firestore behaves below was
 * measured against a running emulator before a line of the mapping was fixed.
 * `GEMINI_TRANSPORT_PROOF_2026-08-20.md` §9.1 named this file's risk before it
 * existed, and the answer is the same one the LLM port arrived at: a contract
 * over a mocked backend bounds shape
 * risk and nothing else, so probe first, then wire.
 *
 * Probing first paid for itself once, and the finding is the reason for the
 * write queue below rather than a footnote about it. **Two concurrent `set()`
 * calls on one document do not resolve in call order.** Measured five times on
 * the emulator with the same two writes: `second, second, first, second,
 * first`. The store's own serialisation law — *"the same id written twice
 * concurrently keeps the later call, not the later flush"* — is a product
 * promise that both existing implementations keep for free, and a Firestore
 * store that issues writes as they arrive breaks it **non-deterministically**,
 * which is the worst way for a promise to break: it passes in CI and loses a
 * learner's edit on a Tuesday. `serial()` is the fix, and it is the same fix
 * `JsonStore` already carries for the same reason one layer down.
 *
 * And no production database. `open()` refuses to construct a client unless
 * `FIRESTORE_EMULATOR_HOST` is set, because until credits land there is no
 * database to point at and `GCP_SETUP_2026-08-20.md` forbids creating one:
 * *"Nothing gets deployed to Cloud Run / Firestore / Pub/Sub before credits
 * arrive."* A flag exists to lift the refusal, and the day it is passed is a
 * decision somebody makes on purpose, in the commit that makes it.
 *
 * ## The mapping, and why it is shaped this way
 *
 * ```
 * boards/{boardId}                     — prefs, aliases  (the board singleton)
 * boards/{boardId}/pins/{docId}        — one document per pin
 * boards/{boardId}/topics/{docId}      — one document per topic
 * boards/{boardId}/signals/{docId}     — one document per signal, + `seq`
 * boards/{boardId}/statements/{docId}  — one document per statement
 * boards/{boardId}/suggestions/{docId} — one document per suggestion
 * boards/{boardId}/edges/{ordinal}     — one document per edge, + `ord`
 * boards/{boardId}/sessions/{batchKey} — one document per NIGHT (the per-night idempotency and persisted batch-identity contracts)
 * ```
 *
 * **`boards/{boardId}` is the store file path.** A `JsonStore` over a path and
 * a `FirestoreStore` over a board id name the same thing: one learner's board.
 * A shared database needs the level a per-learner file gave for free, and
 * putting it at the root also makes `deleteEverything` one `recursiveDelete`
 * of one document rather than a sweep of seven collections.
 *
 * **`prefs` and `aliases` are fields on the root document, not collections.**
 * Both are board-level singletons that are read on nearly every operation and
 * replaced whole, never partially. One document read serves both, which is why
 * every method here starts with `root()` in the same place `JsonStore` starts
 * with `load()`. The bound is stated rather than assumed: an alias map is one
 * entry per merge for ever and chains are deliberately not compressed
 * (`aliases.ts`), so at roughly eighty bytes an entry the 1,048,576-byte
 * document ceiling is about thirteen thousand merges. That is not a limit a
 * learner reaches; it is a limit that should be *known*.
 *
 * **Everything else is one document per row.** The alternative — one document
 * holding an array, mirroring the JSON file — was rejected on measurement, not
 * taste: the emulator refuses a single property over 1,048,487 bytes and a
 * whole document over 1,048,576, so an array-per-collection store would have a
 * hard ceiling of a few thousand pins and would fail by *losing the write*
 * rather than by slowing down. One document per row moves the ceiling to the
 * per-pin level, which is where the product's own caps already are.
 *
 * **Sessions are keyed by the night, and that is the per-night idempotency contract.** The document id
 * is the night the session says it is for — `sessionBatchKey`, the persisted batch-identity contract, falling
 * back to the UTC date of `builtAt` for rows written before the field — so a
 * Cloud Run Job the platform retries writes the same path twice and leaves one
 * row, including the retry whose clock has crossed midnight. `batch-idempotence`
 * currently documents the opposite as a known property of the local store
 * (*"the nightly run is idempotent for topics, pins, edges, signals and
 * statements — and NOT for sessions"*); at this port it stops being true, by
 * construction rather than by a uniqueness check somebody has to remember to
 * run. The `SessionId` moves to a field, and `getSession` finds it there.
 *
 * ## Alias resolution, and the query it rules out
 *
 * The read-side alias table in `json-store.ts` applies here unchanged, and one
 * consequence of it is worth stating because it looks like a missed
 * optimisation. `listSignals(topicId)` cannot be a `where('topicId', '==', x)`
 * query. A merged history lives in the ledger under the *retired* id — the
 * ledger is append-only and a merge rewrites nothing — so the rows that must
 * come back are exactly the rows a server-side equality filter would not
 * return. The alias closure could be pushed into an `in` query, but `in` is
 * capped at thirty values (measured: a 40-value `in` is `INVALID_ARGUMENT`),
 * and a topic merged thirty-one times would silently start returning less
 * history than it has. So the filter is in memory, over a collection read, and
 * it is a correctness decision rather than a lazy one.
 *
 * The same reasoning removed `orderBy` from the signal read. A Firestore
 * `orderBy` **omits documents that lack the field** (measured), so ordering the
 * ledger server-side by `seq` would make any row written without a `seq` — a
 * migration, a hand-repair, a future writer — vanish from the learner's history
 * rather than sort oddly. Sorting in memory cannot lose a row.
 */
 ...
 * alias, so the dependency cannot move underneath the transport proof. */
 ...
 * caller's degradation policy (D10) guess. The three that matter to this
...
export async function openFirestoreClient(opts: FirestoreClientOptions): Promise<FsFirestore> {
  ...
  if (!process.env.FIRESTORE_EMULATOR_HOST && opts.allowProduction !== true) {
    throw new FirestoreStoreError('production-not-authorised',
      ...
      + 'GCP_SETUP_2026-08-20.md: nothing is deployed to Firestore before credits arrive. '
      + 'Point at the local emulator, or pass allowProduction: true and mean it.');
  }
 ...
 * message**, which is exactly the situation the transport proof's third defect
...
/**
 * The night a session belongs to — the per-night idempotency contract's key, as one field.
 *
...
/**
 * The night a session is stored under — the persisted batch-identity contract, and the only caller of
...
export function sessionBatchKey(session: Session): string {
  // `batchKey` was called `nightKey` until the event-driven processing contract renamed the whole idea.
...
export interface FirestoreStoreOptions {
  ...
  readonly projectId?: string;
  /**
   * Lifts the emulator-only refusal. There is no database to point at until
   * credits land; passing this is a decision, and it belongs in the commit that
   * makes it rather than in an environment variable on somebody's laptop.
   */
  readonly allowProduction?: boolean;
...
export class FirestoreStore implements Store {
  /**
   * Single-flight client construction — the D17 shape, avoided on purpose.
   *
  ...
  /**
   * The pin cascade, SB-43, transcribed from `json-store.ts` clause by clause.
   *
  ...
  async appendSignal(signal: Signal): Promise<void> {
    return this.serial(async () => {
      ...
      // Append-only: history is what makes SB-22 possible. Keyed by the signal's
      // own id, which also makes a retried Job's re-append a replacement rather
      // than a duplicate — the same idempotence the per-night idempotency contract buys for sessions,
  ...
  async replaceCommitments(puts: readonly Commitment[], removeIds: readonly string[]): Promise<void> {
    return this.serial(async () => {
      // SB-124 caps a series at twenty and therefore this operation below forty
  ...
  /**
   * The per-night idempotency contract, and the one place this store deliberately does not match the
   * local one.
   *
   * The document name is the night the session says it is for (the persisted batch-identity contract), so a
   ...
   * and the reason matters: SB-41's exclusion list is a *product* rule, not a
