 ...
 * the ones you already had. See `artifacts/the transport contract
 ...
 *
 * Gemma exposed the same lesson in the other direction:
 *
 ...
 *      now per family and the table below is the authority.
 *
 ...
 * a "fast vs deep" decision  and the fast model is the multimodal one.
 */
 ...
 *     a request rather than a guarantee, and model-budget constraint’s 12x figure is a local-model
...
export class GeminiError extends Error {
   ...
   * morning instead of degrading the stage and moving on (graceful-degradation constraint).
   */
 ...
 * headroom and retries — and promoting it to a hard failure would delete graceful-degradation constraint’s
 ...
 * the content-refusal contract asks for: a reason nobody has seen before, arriving with no text,
...
export class GeminiLlm implements Llm {
   ...
   * cover, and this project has a file named for a reason. The
  ...
  private async call(req: LlmRequest, json: boolean): Promise<LlmResult<string>> {
    ...
    if (!apiKey && !accessToken) {
       ...
       * set — provider-configuration constraint’s rule applied to a config error.
       *
    ...
    // Streamed, for streaming-response constraint’s reason and not for the user's: a silent connection is
    ...
    try {
      ...
      // The content-refusal contract, and the last thing that happens before a result exists: a
      // blocked reply never becomes an `LlmResult`, whether it carried partial
      // text or none. Half a section that reads as a whole one is graceful-degradation constraint in a
