...
/** ships with sensible exclusions, not an empty list. */
export const DEFAULT_PREFS: LearnerPrefs = {
  targetMinutes: 15,
  // PROJECT.md The time-window contract: Virgil's session model is the flash-sized choice
  ...
  interview: {},
  //. Empty is the honest default: nothing has been rejected yet, and the
 ...
 * concern and not left to callers:  requires a deleted pin to stop
 ...
 * detectable , so the union of two merged histories happens **on read**:
...
export class JsonStore implements Store {
  ...
  async deletePin(id: PinId, opts: { keepEmptyTopic?: boolean } = {}) {
    ...
    // built on, and  says a deletion reaches derived state. History under a
  ...
  async putTopic(topic: Topic) {
    ...
    // The nightly stages are independently failure-tolerant (graceful-degradation constraint), so a throw
  ...
  async splitTopic(topicId: TopicId, pinIds: readonly PinId[], newLabel: string): Promise<Topic> {
    ...
    // one of them. The new topic starts with no evidence, which safe-empty-state constraint made a safe
  ...
  async appendSignal(signal: Signal) {
    const db = await this.load();
    db.signals.push(signal); // append-only: history is what makes  possible
    await this.save();
  }
  /**
