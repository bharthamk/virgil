 ...
 * pretending — which is the honest-degradation rule from SB-23 applied to
...
export class LocalResearch implements Research {
  ...
  async fetchPage(url: string): Promise<FetchedPage | null> {
    ...
    //
    // The clear used to sit between the fetch and the `!r.ok` check, so every
    // way a fetch can fail — a dead host, a malformed URL out of a capture
    // envelope, the deadline firing — skipped it and left a 15s timer holding
    // the event loop open. On the nightly run that is one leaked timer per
    // unreachable pin and up to fifteen seconds of a Cloud Run Job billed after
    // its work is done; in the test suite it was fifteen seconds of the process
    // refusing to exit, which is how it was found.
    //
    // It also puts the body read inside the deadline for the first time. A page
    // that answers its headers promptly and then dribbles its body had no
    // deadline at all before, which is D19's failure exactly one layer down.
    try {
      let target = url;
      ...
      if (!r) return null;
      if (!r.ok) return null; // SB-46: gated or dead — caller falls back
...
function pinnedFetch(value: string, init: RequestInit, address: string): Promise<Response> {
  return new Promise((resolve, reject) => {
    const url = new URL(value);
    const headers: Record<string, string> = {};
    new Headers(init.headers).forEach((content, name) => { headers[name] = content; });
    const lookupPinned = (
      _hostname: string, _options: unknown,
      callback: (error: NodeJS.ErrnoException | null, address: string, family: number) => void,
    ): void => callback(null, address, isIP(address));
