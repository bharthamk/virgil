 ...
 * diagram, the writeup's agent count, the hosted tree — is downstream of the
 * registry being true.
 */
...
test('the fleet is fifteen agents, and the split between them is stated', () => {
  /**
   * The number the writeup uses. Eleven stages run in the nightly; six agents
   ...
   * scales, one agent — and counting it twice would put a sixteenth agent in a
   * writeup that has fifteen.
   *
...
test('the night scout is a hosted stage like any other, and degrades like one', () => {
  /**
   * SB-280. The hosted tree has to carry it, and has to carry it as an ordinary
