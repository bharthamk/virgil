...
export interface AdkConfig {
  ...
  /**
   * Whether the host may reach a network at all.
   *
   * Default false, and that direction is the whole point. A layer whose default
   * is "offline" cannot spend money because somebody's laptop happened to have a
   * key exported; a layer whose default is "online" spends it the first time it
   * is imported by a test. The composition root turns this on, deliberately, in
   * the commit that means it.
   */
