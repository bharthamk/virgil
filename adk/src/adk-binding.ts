/**
 * The nightly, hosted in Google's Agent Development Kit.
 *
 * This is the only file in the repository that names `@google/adk`, and it is
 * the only one that ever should be. Everything else in this workspace — the
 * stage registry, the degradation policy, the config, the contract — is
 * framework-neutral and is what the contract test runs against `localHost` as a
 * control.
 *
 * ## What is expressed in the framework's primitives, and what deliberately is not
 *
 * The nightly becomes an ADK `SequentialAgent` whose sub-agents are one
 * `BaseAgent` subclass per stage, in `runBatch`'s order. This expresses the
 * agents as the framework's own primitives, and it is checkable: `describe()` below reads
 * the real constructor names off the built tree rather than reciting them.
 *
 * What is NOT expressed in ADK's primitives is the model call. Each stage agent
 * is a *deterministic* ADK agent: it runs a `StageWork` whose body is a `core/`
 * agent reached through the injected `Deps`, and it makes no model call of its
 * own. The alternative — an `LlmAgent` per stage, with ADK owning the prompt and
 * the call — was rejected, and the reasons are worth keeping next to the code:
 *
 *  1. **It would move the model boundary.** `LlmAgent` calls the provider
 *     itself. The seam that `core/src/ports/llm.ts` defines and
 *     `seam-purity.test.ts` guards would stop being the only door to the model,
 *     and the adapter seam's "two adapters, not a rewrite" would stop being true the
 *     day it shipped.
 *  2. **It would put thirteen frontier-tuned agents through a second templating
 *     layer.** ADK composes its own instruction, contents and tool declarations.
 *     Every prompt in this fleet was iterated against the reference session; a
 *     layer that rewrites what reaches the model invalidates that work and buys
 *     a full re-evaluation nobody has budget for.
 *  3. **There is nothing to gain yet.** ADK's `LlmAgent` earns its keep on tool
 *     use, delegation and multi-turn state. The nightly is a deterministic
 *     nine-stage batch with no tool calls and no conversation.
 *
 * The door is not nailed shut: `VirgilSeamLlm` at the bottom of this file is the
 * documented route by which a stage could later become a real `LlmAgent` and
 * still call *through* the seam rather than around it. It is written, not wired.
 *
 * ## The dependency, declared
 *
 * `@google/adk` brings a material runtime dependency tree, and whether
 * the product took that on was a decision reserved for the build owner in the
 * commit that declared the dependency:
 * `deploy/job.yaml` sets `SB_ORCHESTRATOR=adk`, so the deployed nightly hosts
 * itself here and `adk/package.json` declares the package.
 *
 * The import stays dynamic anyway, for a reason beyond the original choice: a
 * process that chose the framework-free host should not pay its module
 * loading to find that out. What the declaration buys is that the package is in
 * the image at all — `npm ci` installs from the lockfile, and nothing named only
 * inside `await import(...)` reaches either.
 *
 * Nothing here calls Google. Every stage agent is deterministic, so the entire
 * hosted nightly runs offline against a fake `Llm` — which is how it was proven
 * on 2026-08-20 with the model budget untouched.
 */
...
function makeStageAgentClass(Base: AdkAgentCtor, createEvent: AdkModule['createEvent']) {
  return class StageAgent extends Base {
     ...
     * because `analyse` had a bad night is precisely the failure D10 exists to
