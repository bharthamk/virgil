...
/**
 *  — the fallback offer, which the Gardener computed and nothing composed.
 *
 ...
 * the product contract calls the offer mandatory and calls it something that must be
 * *genuinely useful, not a consolation prize* — so it is a real session, over
 * material the learner has already met, written to the product contract's five minutes.
 */
...
test('the revision offer is composed, where before it was computed and dropped', async () => {
...
test('the refresh is written to five minutes, not to the session budget', async () => {
...
test('absorbed material is admitted on this path and on no other', async () => {
...
test('the offer is capped at two things, however much is on the shelf', async () => {
  const { deps } = stub();
...
test('an empty board offers no refresh, and the honest empty state survives', async () => {
  const { deps } = stub();
