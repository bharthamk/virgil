 ...
 *
 * The absence of a metrics surface is the product contract's real demand and is the harder
...
test('a finished session carries the three clauses the panel closes on', async () => {
  const { deps } = stub(CLOSING);
  const out = await compose(deps, input(['iam', 'pubsub']));
  assert.equal(out.closingNote, CLOSING);
});

test('the model is told what closure is, and what it must not become', async () => {
...
test('a session with no closing note says none rather than undefined', async () => {
...
test('a night with nothing to teach closes on nothing at all', async () => {
  const { deps } = stub(CLOSING);
...
test('there is nowhere on a session for a score to live', async () => {
