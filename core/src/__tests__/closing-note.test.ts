 ...
 *
 * The ruling is a MECHANICAL strip. No model call on this path, ever: a
...
test('a clause is the unit, so a kept topic named in the same clause goes with it', () => {
  // The stilted note the ruling accepted. "Firestore and IAM both moved" is one
...
test('the strip is pure: same input, same answer, no dependencies to hand it', () => {
  ...
  // call on a withhold night is what the verifier-withholding contract declined.
