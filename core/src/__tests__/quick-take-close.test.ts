...
/**
 * SB-283 — WHAT THE QUICK TAKE'S CLOSE WRITES, AND EVERYWHERE IT MAY AND MAY
 * NOT REACH.
 *
 ...
 * the consumers that were always allowed to read them, and that SB-282's
...
test('SB-283: the three answers write three kinds the ledger already carried', () => {
...
test('SB-283: the two readings are evidence and the deferral is taste, by type', () => {
  // The learner-controlled lineup contract's separation, enforced structurally. `SIGNAL_WEIGHT` is keyed
...
// ------------------------------------------------- SB-282's exclusion survives

test('SB-283: no closing tap reaches the modality tallies, whatever it says', () => {
  /**
   ...
   * report back to themselves. SB-282 excluded the two taps by type; the third
...
test('SB-283: a not now from a take is what the avoided-topic gap is looking for', () => {
  /**
   * SB-280's fourth gap kind reads marks the learner MADE: offered, and
...
test('SB-283: a not now from a take holds the topic for the window it promised', () => {
