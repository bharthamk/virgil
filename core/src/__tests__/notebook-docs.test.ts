...
test('no document carries a comfort number, a percentage, or a score of mine', () => {
  ...
  for (const d of docs) {
    //. The defence is structural first: `NotebookComfort` is not given
...
test('nothing counts anything: no points, no stars, no badges, no streaks', () => {
  // , and `commitments.ts`: points never write a signal and are not
...
test('the three ruled-out phrases never reach a document', () => {
  // The vocabulary contract. "Your saved pages" and "your sources" are the human
...
test('an unanswered modality question does not travel either', () => {
  // a question is a question. This document goes to another tool under
...
test('learn now is the lesson the store says you are on, with where it sits', () => {
  ...
  // the panel's own word for this register, so the two documents a
 ...
 * one for a document about a night that has already happened. Since the learner-lineup contract
...
test('a session says when it was built and from how many things you saved', () => {
  // the background work has to be legible or it may as well not have
...
test('a signal the learner withdrew is not described as evidence', () => {
  ...
  assert.equal(body.includes('a step you told me you were stuck on'), false,
    ': a conceded error withdraws what was read into it, here as well as in the ledger');
});
...
test('a subject the learner removed is in neither document, because it is not on the board', () => {
  // Removal is a delete  and a deleted topic is not in the store's rows
