...
/**
 * Material-aware budgeting caps each section by the evidence available for its
 * topic. Thin topics still receive a useful explanation, but never enough room
 * to encourage padding or unsupported detail. The material cap can only reduce
 * the register-derived budget; it can never make a session longer.
 */
...
test('a five-minute slot takes the learner-requested topic before an inferred regression', async () => {
...
test('a topic with no pins at all is still taught, at the floor', async () => {
  // Degrades gracefully, which the material-aware budget contract asks for in as many words. A topic the
