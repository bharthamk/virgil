 ...
 *
 * `markAnswer`, `answerTangent` (SB-30), and `handleCorrection` (SB-45) are all
 ...
 *
 * The bound is the point of SB-30. The tangent takes one lesson and at most two
...
test('SB-26: depth moves one step and stops at the ends', () => {
...
test('SB-27: the mark is a signal, and it is derived from one field', async () => {
...
test('SB-243: a required point the marker says is missing cannot receive full credit', async () => {
...
test('SB-243: an invented missing requirement cannot keep a complete keyed answer open', async () => {
...
test('SB-170: exact Unicode answer boundaries reach both markers whole', async () => {
...
test('SB-30: a tangent is answered and then routed back to the pin mechanic', async () => {
  const { llm } = spyLlm({
...
test('SB-30: a question that needed no follow-up offers nothing, rather than pinning anyway', async () => {
...
test('SB-30: the tangent is told where the learner is and nothing else about them', async () => {
...
test('SB-169: the Unicode question boundary reaches the Tutor whole', async () => {
  const { llm, calls } = spyLlm({ answer: 'a', offerAsPin: null });
...
test('SB-30: follow-ups receive the lesson and only a fixed rolling window', async () => {
  const { llm, calls } = spyLlm({ answer: 'a', offerAsPin: null });
...
test('SB-172: the fixed rolling window carries each visible Unicode exchange whole', async () => {
  const { llm, calls } = spyLlm({ answer: 'a', offerAsPin: null });
...
test('SB-171: the side-panel Unicode question boundary reaches the Tutor whole', async () => {
...
test('SB-45: it is allowed to lose the argument, and says which way it went', async () => {
...
test('SB-45: holding the line means showing the source, and is still a reply', async () => {
...
test('SB-45: a verdict that is not a boolean is not a concession', async () => {
...
test('SB-45: the claim, the source it rested on and the challenge all reach the check', async () => {
...
test('SB-169: the Unicode correction boundary reaches the source check whole', async () => {
  const { llm, calls } = spyLlm({ conceded: true, reply: 'r' });
...
test('SB-198: a concession retires every authored shell around the bad claim', () => {
  const topic: Topic = {
...
test('SB-206: historical source-boundary contradictions are withheld on read without rewriting them', () => {
