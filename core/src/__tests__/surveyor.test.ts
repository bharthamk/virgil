...
/**
 * SB-21 — foundations before advanced material.
 *
 * The story's walkthrough is the whole test: an advanced ordering-key pattern
 * pinned first, basic subscription types struggled with separately, and the
 * session teaches the basics first. Until now the prerequisite graph had no test
 * file at all — its evidence was one measured run — so what is asserted here is
 * the part that has to hold every night: which edges the Surveyor is willing to
 * keep, and that the ordering degrades to something sane rather than hanging
...
test('SB-37: a topic the learner retired is not in the graph, and cannot make the pair', async () => {
...
test('SB-21: an edge carries the one line the ordering could be explained with', async () => {
  const { llm } = spyLlm({
...
test('SB-21: the advanced pin he made first is taught second', () => {
  // The story, exactly: an ordering-key pattern pinned as interest, subscription
