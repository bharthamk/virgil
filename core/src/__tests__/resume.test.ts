...
/**
 *  second demand: "If enough time has passed that the earlier material
...
test('the product contract\'s own case is the threshold: two days later is cold', () => {
...
test('the recap is two lines, however many sections were finished', () => {
...
test('it is the ground next to where they stopped, not where they started', () => {
...
test('nothing finished is nothing to recap', () => {
  assert.deepEqual(recapSoFar([]), []);
});

test('a section composed before recap lines existed falls back to its heading', () => {
...
test('a section with neither is dropped rather than rendered blank', () => {
  assert.deepEqual(recapSoFar([finished('  ', ''), finished('IAM', 'What binds.')]), ['What binds.']);
});

test('the recap reaches for no model, and the guard says so in as many words', async () => {
