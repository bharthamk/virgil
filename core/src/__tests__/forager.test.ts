...
test('the pinned image reaches the model as an image, in the form the adapters unwrap', async () => {
  //  other half, and the one that was never true. The capture path now
 ...
 * true`, so a Spanish pin re-fetched in English passed without a murmur.
 * whole demand is that nothing in the pipeline assumes English, and capture-envelope constraint is the
...
/** A whole-page pin () captured in `lang`, re-fetched as `text`. */
...
test('a re-fetch in the language the pin was captured in is used', async () => {
...
test('the mismatch is caught in BOTH directions, not only for English pins', async () => {
  // The drift, stated as a test. The first assertion passed before this change;
  // the second is the one  exists for and it did not.
  ...
  assert.equal(englishPinServedSpanish.refetchedText, null,
    'capture-envelope constraint: the capture envelope is the version the learner actually read');
  assert.equal(spanishPinServedEnglish.confidence, 'reduced',
    'a Spanish pin is owed the same guard, which is the whole of ');
  assert.equal(spanishPinServedEnglish.refetchedText, null);
});

test('a regional tag is the same language as its primary subtag', async () => {
...
test('a pin with no captured language cannot be checked, and is not penalised for it', async () => {
...
test('a page whose language cannot be read is not treated as a mismatch', async () => {
...
test('und and zxx are ways of saying "unknown", and are read as absent', async () => {
...
test('a language the check does not model is never reported as a mismatch', async () => {
...
/**
 * The bug this exists for was real and is named in the product contract: the first
...
test('a page that no longer says what the learner read is not taught from', async () => {
...
test('an anchored re-fetch is still the better source, and is read around the selection', async () => {
...
test('a whole-page pin has no anchor to lose, so the top of the page is the right answer', async () => {
  // Anchor failure is a property of a selection pin. A page pin  never
...
test('the pin\'s own page is cited either way, "as it read when you pinned it"', async () => {
