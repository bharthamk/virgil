...
const AGENTS: readonly AgentLint[] = [
  {
    ...
    module: 'scout',
    ceilingChars: 26_000,
    // The passage is capped at 900 characters; the learner's own note, the page
    // title and the whole list of existing topic labels are not. On a 40-topic
    // board the labels are most of this number — in the one agent that has to
    // answer inside a 1.5-second toast. Labels are fenced even though their
    // aggregate size is still worth measuring. Audit finding S1.
  ...
  },
  {
    ...
    // The gist is capped at 200 characters per pin. The page title, the heading
    // path and the learner's note beside it are not, so the prompt is set by
    // the longest title on the board rather than by the cap. Audit finding C1.
    run: (llm) => cluster(
  ...
  },
  {
    ...
    // Labels and summaries, uncapped, for every active topic — which is what
    // AGENT_REQUIREMENTS §7 asks for, and is why this is the largest prompt in
    // the fleet on a big board. Audit finding SV2.
  ...
  },
  {
    ...
    run: async (llm) => {
      ...
      // SB-282's classification, on the same oversized board. Labels only: it
  ...
  },
  {
    ...
    module: 'composer',
    ceilingChars: 64_000,
    // Topic labels and Analyst observations are model prose over pinned text,
    // so both belong inside the data fence. Product-owned ids, register,
    // reason and budget remain outside as instructions. Audit finding CO1.
  ...
  },
  {
    ...
    module: 'tutor',
    ceilingChars: 32_000,
    // `rewriteAtDepth` still interpolates the whole section body with no cap —
    // the only prompt in the fleet built from an uncapped string. Audit finding
    // T2 remains a budget concern, but all Tutor-supplied text must be fenced.
    run: async (llm) => {
      ...
      // SB-31's recap used to be a call here, and is not one any more: the
      // Composer writes each section's recap line as it writes the section, so
      // coming back to a cold session is assembly. A prompt that does not exist
      // cannot be linted, and cannot be injected either.
      // SB-59's quick take. Everything it reads comes off a page, so all of it
      // is fenced. Register, length and the correction-first learner context
      // stay outside that PAGE fence; learner-authored context owns its own
      // delimiter and standing rule.
  ...
  },
  {
    name: 'reviewer',
    module: 'reviewer',
    // Was 104,000, of which the draft — the thing being reviewed — was six per
    // cent: the weak-topic list carried every shaky topic's summary in full,
    // for as many topics as the board had. Audit finding R2, closed with the
    // agent: twelve topics, a label capped at 60 and a summary at 200.
    // Was 13,000 when the draft and the weak-topic list were all it read. The
    // learner's own background is a third input capped at 4,000, and it arrives
    // under a heading of its own: 11,899 without it, 16,006 with it, and 17,000
    // is that with room for a sentence.
    // Unchanged by the as-is route of 2026-08-24, and measured rather than
    // assumed: pages ride in `req.media`, which is not prompt text, and the one
    // sentence the prompt gains to say what they are is 130 characters. The run
    // below carries pages AND a draft, which is the worst case of the two.
    ceilingChars: 17_000,
 ...
 * Tutor's verifier-shaped call answers the code that parses it. SB-282's
...
test('every agent prompt carries the untrusted-content rule', () => {
  ...
  // and is what happened to the Reviewer when SB-38 gave it a caller.
  assert.deepEqual(
...
test('the Registrar asks for exactly the demand kinds the code will accept', () => {
  // SB-282. The vocabulary is closed in `domain/modality.ts` and enforced by
...
test('the Composer states a per-section length, and it is the computed one', () => {
  // SB-05 rests on this. The model is given words — a thing it can control —
...
/**
 * The Composer's section schema requires `question` and requires nothing inside
 * it. `validateSchema` checks the keys a `required` names and no others, so
 * `{"question": {"prompt": "..."}}` conforms — and the Composer used to hand
 * that straight on as a `SessionQuestion`, a type whose `expectedPoints` is not
 * optional. `markAnswer` reads that field in the foreground while the learner
 * waits, and a missing key threw there rather than here.
 *
 * The schema is left alone on purpose: it is part of a prompt, and prompt
 * changes in this product are settled by an evaluation run. The reading is what
 * moved.
 */
