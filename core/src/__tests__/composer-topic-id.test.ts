 ...
 *
 * When every section is dropped, `compose` used to return a session with no
 * sections and `insufficient: false` — and `runSession` read `insufficient` to
 * decide whether this run was the honest empty state, so it persisted a
 * zero-section session and showed it as a real one.
 *
 * `insufficient: true` would have been the wrong fix rather than a smaller one:
 * it means "there was nothing to teach", and this is a board with material on
 * it whose sections the model misaddressed. Those are the two states the
 * Forager's `nothing-found` / `model-failed` split exists to keep apart, so the
 * fix is the same split under the same names: `outcome` is `nothing-to-teach`
 * when the board had nothing to offer and `model-failed` when it did and the
 * reply could not be attached to any of it.
 * See `artifacts-local/ADVERSARIAL_RUN_2026-08-20.md`.
 */
...
test('a night that teaches something says so, and a board with nothing to teach says that', async () => {
  ...
  assert.equal(empty.insufficient, true, 'SB-23 keeps its name for the state it was written about');
});

test('insufficient is the SB-23 name for exactly one outcome, and never for the other two', async () => {
  // The field is kept for SB-23's contract and readers that predate `outcome`.
...
/**
 * The learner-controlled lineup contract — THE `why now:` LINE, KEPT.
 *
