...
test('a fix says what is missing and never writes the replacement', async () => {
  /*
   * The learner-work review boundary, held in code rather than requested in a prompt. This is the
...
test('SB-257: a missing row cannot supply an answer or fabricate a learner quotation', async () => {
  const deps = llmReturning([
    {
...
test('a miss can name the topic the learner is already shaky on', async () => {
  ...
  // A reference the learner cannot follow is the thing SB-44 exists to end, so
...
test('SB-245: harmless topic-id case and wrapping keep the learner-specific mark attribution', async () => {
  const deps = llmReturning([
...
test('SB-245: an ambiguous case-insensitive topic id is not guessed', async () => {
  const deps = llmReturning([
    { criterionId: 'c1', verdict: 'does-not-meet', evidence: 'No metric.', relatedTopicId: 'topic t1' },
  ]);
  const r = await markAssignment(deps, WORK, RUBRIC,
...
test('SB-244: harmless criterion-key wording drift still joins to the authored rubric row', async () => {
  const deps = llmReturning([
...
test('SB-244: the first uniquely matched answer wins and reply order cannot overwrite it', async () => {
  const deps = llmReturning([
