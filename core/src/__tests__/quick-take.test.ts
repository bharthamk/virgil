...
/**
 * SB-59/60 — the quick take, at the level of what the call is allowed to be.
 *
...
test('SB-203: foreground generation is followed by an independent reasoning-on source check', async () => {
...
test('SB-210: a thin pin cannot be expanded with uncited domain consequences or analogies', async () => {
...
test('SB-210: the exact live hydration overreach is narrowed before the verifier can clear it', async () => {
...
test('SB-248: an adapted take cannot invent a worked example or analogy beyond its source', async () => {
...
test('SB-210: a concise source-bound paraphrase still earns the independent check', async () => {
...
test('SB-248: a source-heavy comparison remains eligible for independent checking', async () => {
...
test('SB-247: the next immediate lesson carries learner words through generation and checking', async () => {
...
test('SB-60: a call that did not land is said to be one, never an empty take', async () => {
...
test('SB-203: a fatal source-check finding withholds the take and its confidence controls', async () => {
  const r = recorder((req) => {
    const required = (req.schema as { required?: readonly string[] })?.required ?? [];
