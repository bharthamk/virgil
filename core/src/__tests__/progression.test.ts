 ...
 * do — which is SB-65's own example, arrived at rather than asserted.
 */
...
test('SB-65: a milestone is a register transition the ledger actually made', () => {
...
test('SB-65: the wording is evidence off the ledger, not a template of praise', () => {
...
test('SB-224: one demonstrated answer cannot mint a fluent milestone', () => {
  const firstSitting = [
...
test('SB-65: a slip is not a milestone in the other direction', () => {
...
test('SB-66: a chain counts demonstrated recalls on one topic, at day 2, 5 and 12', () => {
...
test('SB-66: answering the same topic twice in one day cannot extend a chain', () => {
  const sameDay = [
    right('tritone', 2),
...
test('SB-66: a fortnight away pauses the chain and leaves the number where it was', () => {
...
test('SB-66: a miss holds the chain, never resets it, and pulls the next recall in', () => {
  ...
  // SB-66's hardest demand, checked as arithmetic rather than as copy: a wrong
...
test('SB-66: nothing anywhere adds two chains together', () => {
  // The global chain is banned by the story, and the cheapest way to keep it
...
test('a topic with no demonstrations has no chain at all, rather than a chain of zero', () => {
  ...
  assert.deepEqual(out.chains, [], 'a displayed zero is exactly what SB-66 refuses');
});

test('SB-66: a quick-take tap cannot extend a chain, however often it is tapped', () => {
  /**
   ...
   * the learner controls entirely, which is the SB-66 failure arriving through
   * a door SB-61 had to open to exist at all.
   *
...
test('SB-66: out-of-order signals chain the same as the sorted ones would', () => {
...
test('SB-66: a chain never crashes and never extends on a farm of hostile signals', () => {
...
test('SB-66: a miss on a farmed ledger still never costs more than the absence of a right one', () => {
  const clean = [right('tritone', 2), right('tritone', 5), right('tritone', 12)];
...
test('SB-67: the badge set is the four, and adding a fifth is a decision not an accident', () => {
  assert.deepEqual([...BADGE_KINDS],
    ['closure', 'regression-conquered', 'comeback', 'medium-follow-through']);
});

test('SB-67: closure is the comfort threshold — a topic retired as learned', () => {
...
test('SB-67: comfort that settled on attendance alone closes nothing', () => {
  /**
...
test('SB-67: pure re-read noise settles comfort on its own and still closes nothing', () => {
...
test('SB-67: a topic the learner retired by hand earns no closure', () => {
...
test('SB-67: the comeback fires when a topic the learner flagged is carried past where it was', () => {
  const flagged = [
...
test('SB-67: asking to go DEEPER is not an admission of weakness and mints no comeback', () => {
...
test('SB-67: a slip re-earned is regression conquered', () => {
...
test('SB-67: a slip still open is not celebrated, and the refusal is recorded', () => {
...
test('SB-67: medium follow-through needs the doing, not the reading', () => {
...
test('SB-67: volume earns nothing — the whole of it', () => {
  /**
...
test('the two words this product may never say do not appear in the source', async () => {
  /**
   * SB-66: **no day-streaks anywhere in the product** — attendance is not
...
test('the still-shaky tap is a shaky mark, and got-it is not', () => {
  ...
  assert.ok(live.includes('quick-take-still-shaky'),
    'the tap SB-67 opens on has to be one of the flags the comeback is measured from');
  assert.ok(!live.includes('quick-take-got-it'),
    'a learner who said they had it has not admitted anything to come back from');
});

test('SB-67: the comeback is earned from a still-shaky tap at first contact', () => {
  // The story's own walkthrough, end to end: the tap goes in on day zero, the
...
test('SB-67: got it at first contact mints nothing, however well it goes afterwards', () => {
  const confident = [
    sig('iam', 'quick-take-got-it', 'positive', 1),
    sig('iam', 'answer-correct', 'positive', 8),
    sig('iam', 'answer-correct', 'positive', 15),
...
test('SB-231: the compact strip keeps only the newest repeat of one exact milestone move', () => {
...
test('a withdrawn signal is not evidence of anything, here as everywhere', () => {
  ...
  assert.deepEqual(out.events, [], 'SB-45: a conceded marking stops counting, including towards a badge');
