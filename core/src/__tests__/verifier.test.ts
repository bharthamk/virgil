...
test('the verifier receives every authored field the learner sees and the key used to mark them', async () => {
  let prompt = '';
  const llm: Llm = {
    complete: async () => ({ value: '', modelId: 'stub', inputTokens: 0, outputTokens: 0 }),
...
test('Virgil cannot mark the learner on specificity it explicitly says the source lacks', async () => {
  let calls = 0;
  const llm: Llm = {
    complete: async () => ({ value: '', modelId: 'stub', inputTokens: 0, outputTokens: 0 }),
...
test('unknown defect kinds make the verdict unchecked instead of clean', async () => {
...
test('unknown severity makes the verdict unchecked instead of weak', async () => {
  await assert.rejects(() => verify(deps({
...
test('the structured contract constrains both safety classifications', async () => {
...
test('a checkable question or marking key escalates even when the body is plain', () => {
  assert.equal(tierFor({
