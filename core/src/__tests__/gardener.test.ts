...
test('SB-23: too little to teach reports insufficient rather than padding', () => {
...
// -------------------------------------------- SB-23: the fallback is an OFFER
 ...
 * night when the story promises "a 5-minute refresh on two things from last
...
test('SB-23: a night with nothing new to teach offers a refresh, not nothing', () => {
...
test('SB-23: the refresh is two things, and what is due leads what is absorbed', () => {
  ...
  assert.equal(pool.fallback, 'revision');
  assert.equal(pool.revise.length, 2, 'the story says two things, not the whole shelf');
  assert.equal(pool.revise[0]?.disposition, 'review', 'due for a check outranks absorbed');
});

test('SB-23: a retired topic is never revision material either', () => {
  // SB-37 is honoured on this path as on every other. A refresh built from
...
test('SB-23: one genuinely new topic is still taught, not replaced by a refresh', () => {
...
test('SB-23: nothing to teach and nothing to refresh stays an honest empty state', () => {
  const pool = duePool(tend({
 ...
 *
 * The story asks for two things the Gardener owns: the mark feeds the existing
 ...
 */
test('SB-62: a topic the learner asked to come back to is resurfaced, not merely taught', () => {
  const [d] = tend({
...
test('SB-62: the learner\'s ask outranks a regression and every ordinary teach', () => {
  const decisions = tend({
    topics: [
...
test('SB-205: an explicit ask owns the reason even when the same topic has regressed', () => {
  const [decision] = tend({
...
test('SB-62: a mark from before the topic was last taught has already been answered', () => {
  ...
  // the nag SB-62 is careful not to be.
  const [d] = tend({
...
test('SB-62: a mark the learner took back does not resurface anything', () => {
 ...
 * shape as SB-62's mark: the learner has told the product, in one tap, that
 ...
 */
test('SB-61: a still-shaky tap brings the topic back, and the session says whose idea it was', () => {
  const [d] = tend({
...
test('SB-61: got it is not a request to come back — it is the opposite', () => {
...
test('SB-62: the learner retiring a topic still wins over their own older mark', () => {
  const [d] = tend({
...
// ================================ The learner-controlled lineup contract: the lineup feeds the ranking
...
test('a removal outranks a regression, because one of them is a person speaking', () => {
  // SB-36 puts a regression above everything derived, and it stays there. This
...
test('the removal is not retirement: the topic is never dropped, only delayed', () => {
  // SB-22's law. The board must never quietly lose something somebody chose to
...
test('the lineup marks never touch the comfort model, only the order', () => {
   ...
   * this* would be the product inferring ability from taste, which is SB-29's
