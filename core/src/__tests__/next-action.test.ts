...
test('SB-267: a whole commitment identifies the work without repeating a storage date', () => {
  const c = course();
...
// ---------------------------------- SB-283: the one-minute pick, and its reason
...
test('SB-283: the take goes to the weakest ground, and the reason names it', () => {
...
test('SB-283: with everything asked about, the least settled ground leads', () => {
...
test('SB-283: a board with nothing to choose between says so rather than sounding chosen', () => {
...
test('SB-283: a caller with no comfort reads still ranks on the board it has', () => {
  // Every pre-SB-283 caller. The stored comfort is the only standing available
...
test('SB-283: every sentence the quick take can say is free of the two dashes', () => {
  // The learner-controlled lineup contract's copy law, asserted on the copy this slice added. A domain
...
/**
 * SB-286 — the pick a learner has refused, and the count that decides whether
 ...
 */
test('SB-286: a refused pick is replaced by the next one down, not repeated', () => {
  const { topics, pins } = board(3);
  const comforts = [
...
test('SB-286: refusing a pick writes nothing and changes nothing else', () => {
...
test('an unlinked commitment with board topics opens the choice that makes it useful', () => {
  ...
  assert.equal(decision.primary.destination, 'plan');
  assert.equal(decision.primary.cta, 'Choose learning links');
...
test('SB-241: linkless course material asks for its link instead of pretending to start', () => {
  const c = course();
  const decision = chooseNextAction(input({
    courses: [{
      ...c,
...
test('SB-273: a whole final material remainder keeps its exact duration', () => {
  const c = course();
  const decision = chooseNextAction(input({
    availableMinutes: 5,
    courses: [{
...
test('SB-266: a one-minute continuation remains singular', () => {
  const c = course();
  const decision = chooseNextAction(input({
    availableMinutes: 1,
    courses: [{
...
test('SB-239: work explicitly planned for today outranks generic course material', () => {
  const decision = chooseNextAction(input({
...
test('SB-239: a future plan does not pull work forward early', () => {
  const decision = chooseNextAction(input({
    commitments: [commitment({
      topicIds: [], dueAt: '2026-09-05T23:59:00.000Z',
...
test('SB-239: the learner timezone decides when the planned day becomes today', () => {
  const decision = chooseNextAction(input({
...
test('SB-239: a prepared lesson still leads work planned for today', () => {
  const decision = chooseNextAction(input({
...
test('SB-255: completed course history is caught up in My studies, not reset to first run', () => {
  const decision = chooseNextAction(input({
    courses: [course({
      material: [{
        id: 'm1', title: 'Agent lecture', url: 'https://example.test/lecture', kind: 'class',
...
test('SB-255: completed Plan-only history is caught up in Plan, not reset to first run', () => {
  const decision = chooseNextAction(input({
...
test('SB-268: caught-up history and current attention are separate facts', () => {
...
test('The flash-time duration policy exposes only one, three and five minute windows', () => {
