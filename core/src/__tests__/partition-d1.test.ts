 ...
 * (clustering-stability constraint) and the partition assertion (partition-invariant constraint). The rest are D1's own,
...
// -------------------------------------------------------------------- partition-invariant constraint
...
test('an unrecognised strategy name selects the default rule, never a guess', () => {
  ...
  assert.equal(partitionStrategyFrom('model-budget constraint'), DEFAULT_PARTITION_STRATEGY, 'a strategy nobody implemented is not a strategy');
