...
test('§5: the why-line is the Gardener\'s own sentence, for the topic that drove the night', () => {
  ...
  // A colon, not a dash. The learner-controlled lineup contract's copy law bans the em-dash and the
...
test('§5: material waiting is stated as waiting, and no run is promised for it', () => {
  ...
  // The event-driven processing contract: and no run is promised for them either, because nothing picks
  // them up on its own.
...
test('SB-61: the still-shaky tap joins the list it was always going to join', () => {
  // §5 zone 3 is "still-shaky taps (SB-61), resurface marks (SB-62), regression
...
test('SB-61: got it is not a flag — the list is what the learner asked for back', () => {
