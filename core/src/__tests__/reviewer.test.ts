...
/**
 * SB-38/39 — the QC cameo.
 *
 * The agent is written and reachable from nothing, so these tests do not claim
 * the story is delivered. What they cover is the part that would be wrong if it
 * ever were wired: the framing rule that separates this from a proofreader, and
 * the two ways a model can wander out of it.
 *
 * The learner-work review boundary is the whole design of this agent — it reviews the learner's own
...
test('SB-38: the review is pointed at what this learner is shaky on, and at nothing else', async () => {
  // The differentiator over a generic checker is the learner model. If the weak
  // spots stop arriving, this is a proofreader and the story is not delivered.
...
test('SB-38: a topic with no evidence behind it is not called a weakness', async () => {
  // Low comfort with no evidence is "I have barely seen you do this" (SB-52),
...
test('SB-38: a learner with a blank board still gets a review, honestly framed', async () => {
...
test('SB-256: the review receipt counts only evidence-backed weak areas actually admitted', async () => {
...
test('SB-39: the loop back to the board survives, because it is what earns the cameo', async () => {
...
test('SB-39: a finding that suggests no pin says so rather than being dropped', async () => {
...
// -------------------------------------------------------------- The learner-work review boundary

test('SB-38: there is nowhere in the contract for a rewrite to arrive', async () => {
...
test('SB-38: a rewrite offered anyway does not survive the return trip', async () => {
  const { llm } = spyLlm({
...
test('a call that did not happen is not a clean bill of health', async () => {
  // The Forager's lesson (EnrichmentOutcome), applied here: an empty list from
  // a failed call and an empty list from a sound draft used to be the same
  // value, and the panel would have said "this reads sound" about a review that
  // never ran. That is the worst sentence this agent could say.
  const { llm } = spyLlm({});
  const broken: Llm = {
...
test('SB-245: harmless topic-id wrapping keeps the learner-specific review attribution', async () => {
...
test('the prompt names the one thing a draft can ask for that it will not get', async () => {
  ...
  // The learner-work review boundary forbids: "could you give me a better version of this".
  const { llm, calls } = spyLlm({ findings: [] });
  await review(deps(llm), DRAFT, [], []);
...
test('a finding that is a rewrite wearing a diagnosis is dropped, and counted', async () => {
  // The learner-work review boundary was enforced by the prompt and by nothing else. `problem` is
...
/**
 * The Marker's parameter, on the sibling agent, for the same boundary: what comes
