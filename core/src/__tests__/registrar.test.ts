...
test('SB-29: self-declared comfort counts for less than a demonstrated answer', () => {
...
// ------------------------------------------- SB-61: the quick take's two taps
 ...
 */
test('SB-61: a quick-take tap outranks a bare skip and is outranked by a marked answer', () => {
...
test('SB-61: still shaky biases the register down, and got it does not', () => {
  ...
  // the whole point of the story's "no new signal type semantics" — so it is
...
test('SB-61: one tap at first contact does not carry a topic on its own', () => {
...
test('SB-224: one marked answer starts learning but cannot establish fluency', () => {
  const c = computeComfort('T', [
...
test('SB-36: regression is detected when solid history is undercut recently', () => {
  const c = computeComfort('T', [
...
test('invalidated signals stop counting (SB-45 correction path)', () => {
...
// ------------------------------------- SB-55: the statement must reach the evidence
 ...
 * is the half of SB-55 the story calls the load-bearing one: *rejecting an
...
test('SB-224: automatic settling requires repeated demonstrated evidence', () => {
...
test('SB-55: comfort names the signals it counted, so a statement has something to point at', () => {
...
test('SB-55: an invalidated signal is not evidence for anything (SB-45 holds here too)', () => {
...
test('SB-55: a statement carries the ids of the signals behind the topic it names', async () => {
...
test('SB-55: a statement that names two topics carries both, once each', async () => {
...
test('SB-55: a topic with no signals yields a statement with an empty list, and that is honest', async () => {
...
test('SB-55: an observation about the whole board names no topic and claims no evidence', async () => {
  ...
  assert.deepEqual([...(statement?.evidenceSignalIds ?? [])], [],
    'the sentence in the story is exactly this shape, and it must not borrow a topic\'s evidence');
});

/**
 * SB-285: THE TOPIC A SENTENCE IS ABOUT, WRITTEN DOWN.
 *
...
test('SB-285: a statement about one topic carries that topic', async () => {
  const signals = [forTopic('t1', 'sig-1')];
  const topics = [topic('t1', 'Firestore indexes'), topic('t2', 'IAM conditions')];
  const comforts = topics.map((t) => computeComfort(t.id, signals, NOW));
...
test('SB-285: a statement about two topics, or about none, keeps null', async () => {
...
// ============================ The learner-controlled lineup contract: taste is not evidence about anybody
 ...
 * model — which is SB-29's failure with the sign flipped. A learner who marked
...
test('a preference cannot be cited as the evidence behind a statement', () => {
  // SB-55's contestability. A learner who rejects a sentence about themselves
...
test('a preference cannot trigger a regression, which is the loudest thing the model says', () => {
  // SB-36 fires on solid history plus a heavy recent negative. A thumbs-down on
