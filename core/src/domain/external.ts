...
/**
 * WHAT LEFT VIRGIL, AND WHAT CAME BACK.
 *
 * The lesson rail offers four ways to take a lesson to Gemini, and until now
 * every one of them was the end of the thread. The learner pressed a button, a
 * tab opened, and whatever happened out there happened to somebody Virgil could
 * not see. A product whose whole claim is that every surface feeds one learner
 * model had four doors out of it and none back.
 *
 * An entry is the record of one handoff and one place for the learner to say
 * how it went. It is bookkeeping, not evidence: the row itself claims nothing
 * about what anybody understands, and the only thing that reaches the ledger is
 * a mark the learner made on purpose.
 *
 * ## The three separations this module holds
 *
 *  1. **No new signal kinds.** Every mark below resolves to a mark this product
 *     already writes, through `QUICK_TAKE_MARKS`, which is the vocabulary the
 *     quick take's close already uses and every consumer already reads. A fifth
 *     word for "that went well" would be a fifth thing the comfort model, the
 *     night scout and the slipping read each had to learn.
 *
 *  2. **A mark needs something on the board to be about.** A signal is keyed on
 *     a topic. An entry the learner typed in themselves, about a page nothing on
 *     their board has ever heard of, has no topic and therefore no honest place
 *     to put a mark: it records the mark on its own row and writes nothing, and
 *     the surface says so rather than implying a board change that did not
 *     happen.
 *
 *  3. **Removal writes nothing at all.** Ruled: *"remove it from the external
 *     tab with nothing recorded"*. Deleting a row is the learner saying the row
 *     should not exist, which is not a statement about a topic, so it does not
 *     become one.
 */
 ...
 * IT LOOKS.
 *
 * `easy` and `hard` are SB-61's two readings, unchanged: the learner read
 ...
 *
 * SB-282's honest path for a learner-authored claim about themselves is a
