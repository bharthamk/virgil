...
/**
 * The topics this board cannot call settled, from the two sources the story
...
/**
 * WHAT KEEPS SLIPPING, AS A LIST.
 *
 * Four legs, all of which must hold, and each of which can drop a candidate on
 * its own. Ordered by `idleDays x standing weight` and broken on the key, so
 * two reads of an unchanged board produce the same three rows in the same
 * order on any machine.
 *
 * The window leg is satisfied by construction rather than by a separate test:
 * the only thing this list is ever used to offer is the one-minute block, and
 * `offerable` is what checks there is something real to open inside it.
 */
