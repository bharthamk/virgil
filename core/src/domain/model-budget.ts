 ...
 */
export type ModelBudgetWindow = 'total' | 'day';

export const MODEL_BUDGET_UNIT: ModelBudgetUnit = 'tokens';
export const MODEL_BUDGET_WINDOW = 'total' as const satisfies ModelBudgetWindow;
/** Deployment-owned judge allowance. It resets lazily at the first read or
 *  model admission after 00:00 UTC, so it does not require a sleeping timer. */
export const MODEL_BUDGET_DAILY_WINDOW = 'day' as const satisfies ModelBudgetWindow;
...
export interface ConnectionSpend {
   ...
   *
   * The usage-accounting contract presumes every issued request billed, so these are
