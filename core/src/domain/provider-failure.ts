 ...
 * needed it most. `runtime.ts` said so in as many words — the classifier was
 * *"wired into nothing — `runBatch` cannot yet report that a daily cap was
 * met"* — while `BatchOutcome` carried a `quota-degraded` case nothing could
 * produce. A night that never got to try looked exactly like a night with
 * nothing to teach.
 *
...
export type Directive =
   ...
   * exactly the way D18 warns about.
   */
 ...
 *
 * The order of the branches is the ruling, so it is worth reading as one:
 *
 ...
 *     arrives over an ordinary 200 (the provider-refusal contract) and has no status to sort on.
...
export function classify(err: unknown): Directive {
  ...
  if (isTrue(e.exhaustedForPeriod)) {
    ...
    return {
      kind: 'degrade',
      reason: 'exhausted',
      note: `${quota} is spent for the period — degraded rather than retried (D10); `
