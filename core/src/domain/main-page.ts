 ...
 * withheld for failing verification. That is the safe-empty-state constraint shape and §3a's last row —
...
export function sessionCard(input: SessionCardInput): SessionCard {
  ...
  if (pinsWaiting > 0) {
    return {
      ...
      // The manual-processing contract: nothing picks them up on its own. Telling somebody a run is
...
/**
 * The honest empty state, with the reason.
 *
...
function nothingReason(topics: readonly Topic[], decisions: readonly TopicReason[]): string {
  ...
  // The manual-processing contract: there is no next run to try again. The honest sentence names
 ...
 *
 * The still-shaky tap from the quick take  is the first source §5 names,
