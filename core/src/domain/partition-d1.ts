 ...
 *
 * Both cautions were on the record when the flip was ruled (the partition-strategy contract,
 ...
 * (clustering-stability constraint) and half of it is literally the same code: every iteration
 ...
 *
 * Flipped to `d1` on 2026-08-20 (the partition-strategy contract). What earned the flip is on the
...
export function partitionD1(input: D1PartitionInput): readonly PartitionGroup[] {
  ...
  // partition-invariant constraint, for the same reason `partition` asserts it: a partition that is true
