 ...
 *
 * The learner-lineup contract. Everything else in `SignalType` is something that happened —
 ...
 * comfort model, which is  failure with the sign flipped.
 *
 ...
 *
 *  is the reason this table exists: if "I know this" counted as much as a
...
export const SIGNAL_WEIGHT: Readonly<Record<EvidenceSignalType, number>> = {
  ...
  'depth-simpler': 0.5,
  'depth-deeper': 0.5,
  // the same statement as a depth shift, made deliberately and about a
  ...
  // "stronger than declared-only evidence, weaker than repeated
  ...
  'user-model-edit': 1.0, // wins until contradicted by new evidence
};

/** Seed weight decays as real behavioural evidence accrues. */
export const SEED_HALF_LIFE_SIGNALS = 6;

/** Comfort decays with disuse —  resurfacing depends on this. */
export const COMFORT_HALF_LIFE_DAYS = 45;

// ------------------------------------------- The learner-lineup contract: taste, as arithmetic
 ...
 * one-tap "not now" into a silent deletion, which is the failure  names:
 ...
 *  two readings and keep the weight and the direction they were given.
 ...
 * somebody learns that it would then be asked to confirm , and
