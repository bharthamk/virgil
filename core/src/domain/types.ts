...
/** SB-13/14: the two pin types get genuinely different downstream treatment. */
export type PinType = 'interest' | 'struggle';

/** SB-14/17: a struggle pin is often several parts with distinct roles. */
...
/**
 * Why a pin that was made *about* an image is carrying no image (SB-09).
 *
 ...
 * this is not one: the visible frame the story asks for cannot be taken from a
...
/**
 * SB-46: re-fetch will fail on gated pages, so capture-time context must be
 * rich enough to teach from on its own. This envelope is deliberately fat.
 */
export interface CaptureEnvelope {
  readonly selection: string | null; // null for a whole-page pin (SB-07)
  ...
  /**
   * SB-10: the moment in a video this pin was made at, when there was one.
   *
  ...
  /**
   * SB-11: which page of the PDF this came off, where the viewer said.
   *
...
export interface Pin {
  ...
  /** SB-12: a scrap of intent, high signal, cheap to give. */
  ...
  /** SB-15: true when this began as an agent suggestion the user confirmed. */
...
/** SB-44: every taught claim traces to a pin or to agent-sourced material. */
 ...
 * line counted neither. That is the D14 shape: nothing failed, and the product
 * was wrong.
 *
...
export interface Enrichment {
  ...
  /**
   * SB-46: when re-fetch failed we ran on capture-time context alone.
...
export interface Topic {
  ...
  /** 0..1 derived, never shown as a number (SB-33). */
  readonly comfort: number;
  readonly lastExposedAt: string | null;
  /** SB-37: retired by the user. Distinct from deleted (SB-43). */
...
/** SB-21: ordering must be explainable in one line. */
...
export type SignalType =
   ...
   * demonstrated and are weighted as such (SB-29). Both put the topic on the
   * flagged list, because both are the learner asking for it back.
   */
  | 'resurface-refresher'
  | 'resurface-deeper'
  /**
   * SB-61 — the two taps that close a quick take (UX_SPEC §3).
   *
   ...
   *
   * SB-61 places them exactly: *"stronger than declared-only evidence, weaker
  ...
  /**
   * The learner-controlled lineup contract — the three marks the learner makes on the LINEUP, before any
   * of it has been read.
   *
   ...
   * SB-29's weight table exists to prevent, arriving through a new door.
   *
   ...
   * weighted as demonstrated for exactly that reason (SB-29).
   */
  | 'guide-stuck';

export type SignalDirection = 'positive' | 'negative' | 'neutral';

/** SB-29: demonstrated comfort must outweigh declared comfort. */
export interface Signal {
  ...
  /** SB-45: a conceded error invalidates signals derived from that section. */
...
/**
 * SB-282: the four kinds of demand a piece of material can make.
 *
...
/** SB-42: the model must render as editable prose, which constrains storage. */
export interface Statement {
  ...
  readonly confirmedAt?: string | null;
  /**
   * SB-282, and absent on every ordinary statement.
   *
...
/** SB-25: three registers in one session is the differentiator. */
...
/** SB-45: the durable exchange when a learner challenges Virgil's teaching.
...
export interface SessionSection {
  ...
  /** SB-44: every claim carries provenance, structurally not in prose. */
  readonly sourceIds: readonly SourceId[];
   ...
   *
   * The learner-controlled lineup contract's `(i)`. The Gardener already writes one reason per topic and
  ...
  /**
   * One sentence naming what this section covered, for the resume's recap.
   *
   * Written by the Composer, at composition time, while it has the whole
   * session in hand — which is both the cheapest moment and the best-informed
   * one. It used to be generated on demand instead: coming back to a cold
   * session made a model call whose entire job was to restate sections this
   * product had already written, without adding new information.
   *
   * Optional because every session composed before this has none, and the
   * heading is the honest fallback for those — it is a real description of the
   * section, written by the same pass.
   */
   ...
   * signals derived from that mark were withdrawn. SB-45's teaching challenge
...
export interface Session {
  ...
  /**
   * Which night this session is for — the persisted batch-identity contract.
   *
  ...
  /** SB-31: resume is section-granular, and stale resumes need a recap. */
  readonly currentSectionIndex: number;
  readonly closingNote: string | null;
  /**
   * SB-23: this is the revision offer — "not enough new to build a proper
...
/** SB-15: never auto-promoted to a pin. Always confirmed by the user. */
export interface Suggestion {
   ...
   *
   * Accepting a suggestion makes a real pin (SB-15 step 4), and a pin with no
 ...
 * accepts it. Optional for every local and pre-SB-154 board.
 */
...
export interface LearnerPrefs {
  ...
  /** SB-40: pause must genuinely stop collection, not just hide the UI. */
  readonly pausedUntil: string | null;
  /** SB-41: ships with sensible defaults, not empty. */
  readonly excludedDomains: readonly string[];
  readonly interview: Readonly<Record<string, string>>;
  /**
   * SB-16: how many suggestions the learner has turned down, per site.
   *
   * The story asks that repeated rejections *quiet the detector, not just filter
   ...
   * failure SB-29's weight table exists to prevent arriving through a new door.
   *
  ...
  /**
   * SB-282: the learner's answer of no to a modality question, and when.
   *
