 ...
 *    same shape `Commitment.topicIds` was in before the topic-relationship contract — a link that
 ...
 *    draft, and the topic ids became writable under the topic-relationship contract, from the dated
...
/**
 * The dated work a topic is on the hook for, when the learner linked it.
 *
 * The affordance-first interface contract moved this fact from a sentence to a thing. The hero used to say
 * *"It moves 'Stats problem set 3' forward, and that is still open past its
 * date."* over a list. It is a fact about one lesson, announced over all of them, in a
 * paragraph nobody reads twice. Shown on the row it is actually about, as the
 * same quiet chip the subject already uses, it needs no sentence at all.
 *
 * Open work only, and the nearest date first. A commitment that is already
 * handed in is not something tonight moves forward, and where two are open the
 * one due soonest is the one worth naming on a row that has space for one.
 *
 * Pure, deterministic, no model, no I/O. Same join and same refusals as
 * `subjectForTopic`: the link is `Commitment.topicIds`, which is real, and a
 * topic nobody linked to anything gets nothing rather than a guess.
 */
