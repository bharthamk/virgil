 ...
 * (`Topic.comfort`, 0..1) and  is that it is never shown as one. The
 ...
 *
 * Three phrases are banned outright by the vocabulary contract and none of them
...
export interface NotebookComfort {
  ...
  /** it had settled, and something recent has undercut it. */
...
/**
 * the same register vocabulary the panel shows, in a sentence.
 *
 ...
 * score taken out, and  and  both land on it: comfort is never a
 ...
 * so that a learner can disagree with it. That is  contestability
...
function readBoard(input: NotebookInput): Board {
  ...
  // An invalidated signal is one the learner said was wrong. It is
...
function evidenceLine(board: Board, topicId: TopicId): string | null {
  ...
  for (const signal of board.liveSignals) {
    ...
    // The learner-lineup contract: the lineup's three marks say what the learner WANTS taught.
 ...
 * already happened. Since the learner-lineup contract a session carries each section's `why`,
...
function sessionFacts(session: Session): readonly string[] {
  const out: string[] = [
    //  own copy: the background work has to be legible or it may as well
    // not have happened.
...
function boardBody(board: Board): string {
   ...
   * and an unanswered modality question is a question. Neither is
