...
export function estimateCalls(input: CallEstimateInput): number {
  ...
  // run does not pay. See the hold recorded in `pipeline.ts` and DEAD_ENDS.md.
  const forage = owed;
...
/**
 * A night that read the board and came back with nothing to say about it.
 *
 * The conjunction is the whole definition and every term is load-bearing. A
 * night that wrote no statements is an ordinary night if the Analyst had
 * something to say, because the sentences are only one of the things
 * observations feed. A night that raised no proposal is ordinary too: the
 * scout is allowed to find nothing. It is the three together that mean the
 * deeper pass produced nothing for anybody downstream, which is the state that
 * used to be reported as silence and is now said out loud.
 *
 * Deliberately not a judgement about failure. A degraded stage already has its
 * own honest line, and the surface that renders this one decides whether to say
 * both; this function counts what the run produced and nothing else.
 */
