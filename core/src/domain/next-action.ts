...
/**
 * The flash-time duration policy: Learn is a flash-time product. Longer chips centred the wrong
...
/** The deadline has no learning link yet; the press opens that exact editor. */
export const CTA_CHOOSE_WHAT_IT_NEEDS = 'Choose learning links';
...
export interface ActionOption {
  ...
  /**
   * SB-286: how many other topics stand behind a quick take's pick, ready to be
...
export interface NextActionInput {
  ...
  /**
   * SB-283: the Registrar's current comfort reads, by topic.
   *
   ...
   * is the only standing available, which is what every pre-SB-283 caller had.
   */
  readonly comforts?: readonly TopicComfortRead[];
  /**
   * SB-286: the quick-take pins the learner has refused on this visit to Learn.
   *
 ...
 *
 * SB-283, from the walkthrough. At a one-minute window with nothing due, the
 ...
 * and this number is that ruling made true. It sits above every commitment
 ...
 *
 * Lifted out of `candidates` when SB-286 gave it a second output. It was always
...
const candidates = (input: NextActionInput): ActionOption[] => {
  ...
  if (input.session && remaining.length) {
    ...
    if (whole || firstFit) {
      /**
       * The nearest thing due that this lesson actually teaches for.
       *
       * A deadline whose topics tonight's session already covers is not a rival
       * to it, it is the reason for it. It used to be both: the commitment
       * ranked separately, won on urgency, and sent the learner to a list while
       * the lesson that prepared the very topic sat unopened. So its urgency
       * arrives here, in words, naming the work it serves.
       */
      const served = open
      ...
      out.push({
        ...
        // on screen is the narration the affordance-first interface contract bans.
        detail: whole
  ...
  if (!out.length) {
    ...
    out.push(hasBoardHistory ? {
      ...
      // SB-279: the arrival screen is the front door, and a single verb sold
