 ...
 *
 * Runs on every pin and must render inside the toast , so it takes
...
export async function scout(deps: PureDeps, input: ScoutInput): Promise<ScoutOutput> {
  ...
  // whole-page pins are a first-class case, so fall back to the page's
...
/**
 *  says never block the toast on Scout. When the model is unavailable or
