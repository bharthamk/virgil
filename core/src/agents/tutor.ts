 ...
 *
 * Bounded deliberately. SB-30: tangents get a brief answer and route back to
...
export async function markAnswer(
  ...
  return {
    ...
    missed,
    // SB-27's whole point: this is the strongest comfort signal available.
...
/** SB-26: depth changes per section, never globally. A global slider would be a
...
/** SB-45: without a working correction path, one wrong lesson poisons the whole
 ...
 *
 * SB-30 bounds this agent's context deliberately: an ever-growing Tutor
...
/**
 * Three outcomes, and the middle one is why this is not `QuickTakeResult`.
 *
 *  - `ready`        — there are steps.
 *  - `no-subject`   — there is nothing in the material to be walked through
 *                     doing: a navigation list, a cookie notice, boilerplate.
 *                     Narrow on purpose. It used to be `not-a-task` and it
 *                     covered any passage that did not issue instructions,
 *                     which refused the entire feature — see the note above.
 *                     A passage that describes something is still guidable;
 *                     material that names no subject at all is not, and
 *                     inventing one would send the learner off performing a
 *                     task nobody set.
 *  - `model-failed` — no call landed, or what came back was unreadable.
 */
 ...
 * than answered as an essay. That is the pin mechanic SB-30 asks for.
 *
...
export interface AskResult {
   ...
   *
   * SB-30's route back to the pin mechanic, and the half of that story worth
