...
/**
 * Builds one source-grounded learning session at the depth and duration the
 * learner requested. The Composer may shorten or refuse a section when the
 * available material cannot support it; it never invents content to fill time.
 */
...
export interface ComposerInput {
  ...
  /** Compose a short revision when the Gardener selects previously seen material. */
  readonly fallback?: 'revision' | null;
}

/** Keep revision sessions short enough to complete in one sitting. */
...
/**
 * Duration is computed from the generated word count and reading-depth rate.
 * The model receives a word budget, which it can control directly, while the
 * application derives the displayed duration from the returned content.
 */
 ...
 *     session total cannot be a flat sum; it is solved for.  makes the
 ...
 * material that does not exist, which is the fault the material-aware budget contract exists to fix
 ...
 *
 * The material-aware budget contract is explicit that a thin topic gets a short section and NOT a
 ...
 *
 * A material-aware cap prevents a single-pin topic from receiving the
 ...
 * A shorter session over thin material is the honest outcome  already
 ...
 * contract makes and would change output on boards where nothing is wrong.
 */
 ...
 *
 *  says every claim carries provenance structurally, and the panel renders
 ...
 * the prompt says to list the ids there;  whole point is that provenance
 ...
 *                       refreshing.  honest empty state. The model is
...
export interface ComposedSession extends Omit<Session, 'id' | 'sections'> {
  ...
  /**
   * true when there was honestly not enough to build a session.
   *
   * Kept, and kept meaning exactly what it always meant — but it is now the
   *  name for ONE outcome rather than the answer to "is this a session".
  ...
  /**
   * true when this is the revision offer rather than a full session —
   ...
   * because the learner is owed the sentence the product contract actually promises
...
export async function compose(deps: PureDeps, input: ComposerInput): Promise<ComposedSession> {
  ...
  // a refresh of what they already have, not a thin lesson. Absorbed
  ...
  // Fit topics to the budget before writing anything. : compose to the
  ...
  if (!chosen.length) {
    // honest empty state. Never manufacture a lesson to look busy, and
  ...
  */
  ...
  // Cap the register budget at what the topic's source material can support. The register decides
  ...
  /**
   * The learner-lineup contract — the `why now:` line from the brief, kept.
   *
  ...
  const composedSections: ComposedSection[] = (Array.isArray(res.value?.sections) ? res.value.sections : [])
    ...
    // Source ids have been checked against what was offered since ; the
