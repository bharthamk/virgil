...
/**
 * REVIEWER — the QC cameo (SB-38).
 *
 * HARD CONSTRAINT, and it is not negotiable (The learner-work review boundary):
 * this reviews the learner's OWN work and never produces submittable content.
 * No rewriting, no drafting, no "here's a better version". If that line ever
 * blurs, the product becomes something that writes your work for you, and the
 * whole thing stops being defensible as an employment artefact.
 *
 * What makes it worth having at all is the learner model. Any generic checker
 * can find weak writing; this one reviews against what *this person* is
 * demonstrably shaky on, and routes findings back into the board (SB-39).
 *
 * The constraint is now held in three places, because a prompt alone was
 * holding it and a prompt is an honour system:
 *
 *  1. the schema, which has nowhere for a replacement sentence to arrive;
 *  2. the prompt, which says so, and now also names the specific request a
 *     draft can make — "give me a better version of this" — that the standing
 *     untrusted rule does not cover;
 *  3. `looksLikeARewrite`, a deterministic tripwire in the shape of
 *     `suspectedInjection`: it acts in code rather than asking the model to be
 *     careful, and it counts what it drops so a model that has started drafting
...
export interface ReviewFinding {
  ...
  /** SB-39: the loop back to the board is what earns this agent its place. */
 ...
 * the `suspectedInjection` shape, acting in code, where the learner-work review boundary can actually
...
export async function review(
   ...
   *
   * The Marker's parameter, on the sibling agent, for the same boundary: what
  ...
  const shaky = topics
    .filter((t) => {
      ...
      // seen you do this" (SB-52), and reviewing somebody against a guess is
  ...
  try {
    const res = await deps.llm.structured<{ findings: ReviewFinding[] }>({
      ...
      system: SYSTEM,
      prompt: [
        // Fenced, every one of them. The draft is the learner's own writing, which
        // is exactly why nobody thought of it as untrusted — and the weak-topic
        // list is model prose written over text off a web page, which is the
        // same material one step removed. The `"""` this used to use is not the
        // fleet delimiter and cannot be escaped: a draft containing `"""` closed
        // it and put the rest of itself back at instruction level.
        shaky.length
    ...
    raw = res.value;
    // A reply of the wrong shape is a call that did not happen, not a draft
    // with nothing wrong with it. Same rule as the Forager's, for the same
    // reason: the two used to be one value.
  ...
  for (const f of raw.findings) {
    ...
    const finding: ReviewFinding = {
      ...
      // reference the learner cannot follow is the thing SB-44 exists to end.
