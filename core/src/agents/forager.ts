 ...
 *
 * SB-20: the learner pinned a scrap. Forager goes and gets what the scrap was
...
async function prepare(deps: Deps, pin: Pin): Promise<Prepared> {
  ...
  // SB-20: read around the selection with fresh eyes.
  ...
  // they never read (DEAD_ENDS.md D9). Trust the capture envelope over a
  ...
  // SB-46: gated, dead or JS-only pages are normal, not exceptional. Fall back
...
function readAnswer(value: unknown): Answer {
  ...
  // A reply of the wrong shape is a failed call, not a passage that needed
  // nothing. `?? []` used to stand here and the two outcomes collapsed into
  // one: `nothing-found` also tells the store this pin has been answered and
  // is owed nothing further, so a lax adapter returning prose would retire the
  // pin from enrichment for ever on the strength of a reply nobody read.
...
async function finish(deps: Deps, prepared: Prepared, answer: Answer | null): Promise<Enrichment> {
  ...
  // SB-34/SB-44: agent-sourced references are fetched only where the product can
 ...
 * is awake for. The event-driven processing contract removed the nightly. Somebody presses Process and
 * waits for it.
 *
...
/**
 * Does this re-fetched page read as the language the pin was captured in?
 *
 * SB-48's whole demand is that nothing in the pipeline assumes English, and the
 * guard this replaces did exactly that: it returned true for every non-English
 * tag, so the D9 defect — a vendor serving a different localisation to the
 * nightly re-fetch than to the learner — was caught for English pins and for
 * nobody else. A Spanish pin re-fetched in English was, until now, taught
 * without a murmur.
 *
 * Three answers, and the third is the one that matters:
 *
 *  1. The page reads as the captured language  -> matches.
 *  2. The page clearly reads as a DIFFERENT profiled language -> mismatch, and
 *     the caller falls back to the capture envelope at reduced confidence.
 *  3. Either side is absent, unknown, or unprofiled -> **cannot verify**, which
 *     is not evidence of a mismatch. The confidence is left alone rather than
 *     marked down: penalising a pin for a missing `lang` attribute would make
 *     every page that declares nothing look like a localisation swap.
 *
 * Deliberately a cheap heuristic over the page TEXT rather than a declared tag.
 * D9's fix was to send `accept-language`, and its lesson was that the header
 * cannot also be the check — a page that lies about, or simply omits, what it
 * served is exactly the case this exists for.
 */
