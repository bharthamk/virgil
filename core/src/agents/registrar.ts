...
/**
 * REGISTRAR — the learner model.
 *
 * Comfort is arithmetic, not judgement. That is deliberate: it makes the model
 * predictable, cheap, replayable, and explainable to the user when they ask why
 * (SB-42). The only part that needs a model is turning the numbers into the
 * plain sentences the user reads and edits.
 *
 * SB-29 is enforced here by the weight table: demonstrated comfort outweighs
 * declared comfort, so "I know this" cannot be used to lie to yourself.
 */

export interface ComfortResult {
  readonly topicId: TopicId;
  readonly comfort: number;
  /** True when comfort was solid and recent evidence has undercut it (SB-36). */
  ...
  /**
   * SB-55: the signals this number was actually computed from.
   *
 ...
 * inaccurately is corrected by what they actually do (SB-02).
 */
...
export function computeComfort(
  ...
  /**
   * The learner-controlled lineup contract: the lineup's three marks are filtered out here, and this is
  ...
  // Regression (SB-36): the learner had this, and recent evidence says they no
...
export function applyComfort(topics: readonly Topic[], signals: readonly Signal[], now: Date): Topic[] {
  return topics.map((t) => {
    ...
    return {
      ...t,
      comfort: c.comfort,
      // SB-22: absorbed material settles itself, but only on real evidence.
...
/**
 * SB-285: which one topic a sentence is about, or nothing.
 *
...
/**
 * SB-55: the signals a statement summarises — the live signals of the topics it
...
/**
 * SB-282's one model call, and the smallest job in the fleet.
 *
