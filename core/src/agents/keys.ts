...
/**
 * Which offered key a returned one means, or null when that is not certain.
 *
 * The read-back used to be an exact string match on model output, which is a
 * drift the Clusterer records having suffered: a reply of "group g1" where
 * "g1" was asked for matched nothing, and every new topic in that run silently
 * took the fallback label.
 *
 * Tolerant in one direction only. Every step either identifies exactly one
 * offered key or gives up: there is no nearest match and no positional guess,
 * because an answer attached to the wrong item is worse than no answer — a
 * mislabelled topic on the learner's board, or one pin's prerequisites written
 * onto another's. Ambiguity keeps the fallback.
 */
