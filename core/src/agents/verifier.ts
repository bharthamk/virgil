...
export async function verify(deps: PureDeps, input: VerifyInput): Promise<readonly Defect[]> {
  ...
  // A reply with no defects list in it is not a verdict of "no defects".
  //
  // `?? []` used to stand here, and it is the fail-open direction: a reply that
  // was prose, or an object missing the one field the schema required, became
  // an empty defect list, which is exactly what a clean section looks like. The
  // adapter is supposed to make that impossible — but "the adapter enforces the
  // schema" is a promise about code that does not exist yet on the provider the
  // product ships on, and this agent is the one place in the fleet where
  // trusting an unkept promise means shipping unchecked teaching.
  //
  // Throwing hands the section to `verifySections`, which withholds it as
  // `unverified` and says so in the run. Loud and unshipped beats quiet and
  // wrong, for the same reason `reasoning` is not configurable here.
