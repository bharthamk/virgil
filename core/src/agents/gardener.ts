 ...
 *
 * a learning board that rots is not neutral, it is guilt. This agent is
...
/** pinned but never engaged with. Offered for retirement, never silently
...
export type Disposition =
  ...
  | 'resurface'      // regressed, needs bringing back ()
  | 'settled'        // absorbed; retire quietly
  | 'offer-retire'   // pinned and abandoned; ask before dropping ()
...
/**
 * the two marks that mean "come back to this", and what each one asks
...
const RESURFACE_MARKS: Readonly<Partial<Record<SignalType, string>>> = {
  ...
  /**
   *  — *"This run, the IAM topic is prioritised."*
   *
   * The still-shaky tap belongs here rather than in a mechanic of its own,
   * because it is the same statement  refresher mark makes, made at a
   * different moment: the learner has read something and told the product they
   * are not there yet. The register half of the product contract is the comfort model's
...
export function tend(input: GardenInput): readonly GardenDecision[] {
   ...
   *
   * The learner-lineup contract adds the second weight, and it is the same argument in the
  ...
  return topics.map((topic): GardenDecision => {
    ...
    /**
     * The learner-lineup contract — the X on the lineup, honoured for a week.
     *
    ...
    // the learner's own request, above every machine-derived read. A
    ...
    // regression outranks every other derived decision. Losing
    ...
    // pinned once, never engaged, long silence. Ask — do not just delete.
     ...
     *
     *  rule is that nothing infers more than the evidence carries, so
...
/**
 * honesty over filler. If there is genuinely not enough, say so and offer
...
export interface Pool {
  ...
  /**
   * what the revision offer is built FROM — "a 5-minute refresh on two
   * things from last week".
   *
   * the product contract calls the fallback mandatory and calls it something that must be
   ...
   * learner's own decision is honoured here as everywhere.
   *
...
/** Limit each revision session to two previously seen topics. */
