...
export interface ScoreableSession {
  ...
  /**
   * What the Verifier refused, for the verifier-withholding contract's check.
   *
 ...
 * ONE-SIDED on purpose: a session that runs long is the SB-05 failure that
...
export function scoreSession(session: ScoreableSession, board: ScoreBoard): Scorecard {
   ...
   *
   * The Composer material-budget contract made a section's budget the smaller of its register share and
   * what its topic's material earns, and deliberately added no state to carry
   ...
   * ruling. `budgetBasis` says which of the two a reader is looking at, because
  ...
  /**
   * The verifier-withholding contract — and the reason it is a hard check rather than a proxy.
   *
   * "The note may not name a section that was withheld" is a contract, not a
   ...
   * `card.hard` by id. This ruling moved the total from 16 to 17; the later
  ...
  const proxies: ProxyMetric[] = [
    ...
    proxy('register-spread', registersPresent.size / 3,
      'ratio', 'distinct registers over the three that exist — SB-25 as a number, not a verdict on whether they read as one voice'),
...
export async function boardFromStore(
  ...
  return {
    ...
    // SB-282: a modality question nobody has answered is not known about
