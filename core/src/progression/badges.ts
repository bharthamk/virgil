 ...
 *
 * Three sources: the still-shaky tap on a quick take (SB-61), the resurface
 * refresher (SB-62) — the learner asking for a topic back at a lower level —
 * and a regression the model detected, which is the product noticing the same
 * thing the learner would have said.
 *
 * The still-shaky tap is the one SB-67's walkthrough actually opens on
...
export function badgesFor(
   ...
   * something, which is their right (SB-37) and is not the same as having
  ...
  for (const flag of flags) {
    ...
    // event, and the courage badge is the one SB-67 exists for; showing both
    // would be the same fact celebrated twice, which is the confetti the story
    // rules out.
