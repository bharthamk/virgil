...
- Node.js 22 or 24 LTS and npm (CI and the production images use Node 24)
- Docker with `buildx`
- Google Cloud CLI and Firebase CLI, signed into the project you will own
- a Google Cloud project with billing or hackathon credits
...
2. Configure the OAuth consent screen, enable Firebase Authentication and its
   ...
   used for installation acceptance as a test user. Leave Email/Password and
   Anonymous sign-in disabled; Virgil has no product route that uses them.
...
Restrict the Firebase Web API key to Identity Toolkit and Secure Token. HTTP
referrer restrictions may not fit a package that serves both a Cloud Run origin
and a `chrome-extension://` origin, but API-target restrictions are mandatory.
...
`build.sh --plan` only prints the build and cloud commands; it writes nothing.
`apply.sh --plan` writes reviewable local `deploy/*.plan.yaml` but makes no cloud
change. A real apply writes `deploy/*.rendered.yaml`; neither plan overwrites the
last applied receipt.
...
publishes deny-all client rules, enables database delete protection, and grants
only the Firestore, Job-execution and named-secret access the direct Gemini API
route needs. The scripts refuse to create or bill resources without that exact
opt-in. The optional Vertex route is documented in `docs/GOOGLE_BACKEND.md` and
requires an explicit template and IAM change; it is not pre-authorized.

```bash
VIRGIL_DEPLOY=yes PROJECT_ID=your-project IMAGE_TAG=$(git rev-parse HEAD) ./deploy/build.sh
...
### Optional private Demo mode

Demo mode is compiled into the same build and is off by default. To give a
reviewer temporary product access without adding them to the Google member
directory, generate a strong 20–128 character password outside the repository,
calculate its lowercase SHA-256 hex digest, and pass only that digest when
applying:

```bash
JUDGE_DEMO_PASSWORD_SHA256=YOUR_64_CHARACTER_SHA256_DIGEST \
VIRGIL_DEPLOY=yes PROJECT_ID=your-project \
FIREBASE_API_KEY=your-public-web-api-key \
GOOGLE_WEB_CLIENT_ID=your-web-application-client-id \
OWNER_EMAIL=owner@example.com \
./deploy/apply.sh
...
Share the deployment's `/app/?judge=1` URL and the original password through a
private channel. The digest is deployment configuration; the password must not
be committed, written into a rendered manifest, or added to submission source.
The entrance is hidden when the digest is empty. A successful login creates a
30-day temporary session over one shared disposable Demo board, never the
owner's board. Personal Notebook/Drive connections begin off, and the checked-in
service template caps foreground Cloud/API use at 500,000 tokens per UTC day.
This route is separate from Firebase Email/Password authentication, which stays
disabled; ordinary `/app/` access remains Google-only.

The repository's public-safe judge story is not written to Firestore by build
or apply. Inspect the exact target and fixture digest first:

```bash
node scripts/apply-judge-story.mjs \
  --project your-project --board-id learner-judge-workspace-v1
```

Only the Demo-board operator should perform the replacement. The apply path
requires both exact environment confirmations plus explicit backup and receipt
files, refuses to run otherwise, and is idempotent when that story is already
present:

```bash
VIRGIL_DEPLOY=yes \
VIRGIL_REPLACE_JUDGE_STORY=learner-judge-workspace-v1 \
node scripts/apply-judge-story.mjs \
  --project your-project --board-id learner-judge-workspace-v1 --apply \
  --backup release/judge-board-before.json \
  --receipt release/judge-board-apply-receipt.json
```

Both output paths are under ignored `release/`; review and retain them in the
operator's private release records. This step spends no model tokens. It does
replace the named disposable board, so never point it at an owner's learner id.
...
id and writes the same id to `virgil-package.json` and `NEXT_STEP.txt`. The v2
receipt also records the exact source commit/tree, dirty-state declaration,
complete file inventory and hashes for the browser's identity-bearing runtime.
...
Both source and extension packagers refuse a dirty worktree by default. Use
`--source-ref EXACT_REF` to prove the checked-out clean commit is the requested
release, or `--allow-dirty` only for an explicitly non-release development
snapshot. A finished `virgil-package.json` is portable: every delivered file,
its size and SHA-256 hash can be verified without the Git repository.
